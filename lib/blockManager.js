...
blockQueue.drain = function () {
    console.log("blockQueue drained: unlocking remainder of blockManager functionality");
...
balanceQueue.drain = function () {
    console.log("balanceQueue drained");
    if (!block_unlock_callback) {
        console.error("block_unlock_callback is not defined");
        return;
    }
    if (!prev_balance_sum) {
        console.error("prev_balance_sum is not defined");
        return;
    }
	...
	let balance_sum = rows[0].amt;
        if (balance_sum > prev_balance_sum) {
            ...
            console.error("Balance not changed from " + prev_balance_sum + " to " + balance_sum);
        }
        block_unlock_callback = null;
        prev_balance_sum = null;
    });
