...
function calculatePPLNSPayments(block_height, block_reward, block_difficulty, unlock_callback) {
    ...
    async.doWhilst(function (callback) {
        ...
        global.mysql.query("SELECT SUM(amount) as amt FROM balance").then(function (rows) {
            ...
            console.log("PPLNS payout cycle complete on block: " + block_height + " Block Value: " + global.support.coinToDecimal(block_reward) + " Block Payouts: " + global.support.coinToDecimal(totalPayments) + " Payout Percentage: " + (totalPayments / default_window) * 100 + "% (precisely " + totalPayments + " / " + default_window + ")");
            if (totalPayments != default_window) {
                global.support.sendEmail(global.config.general.adminEmail,
                    "Block was not payed completely!",
                    "PPLNS payout cycle complete on block: " + block_height + " Block Value: " + global.support.coinToDecimal(block_reward) + " Block Payouts: " + global.support.coinToDecimal(totalPayments) + " Payout Percentage: " + (totalPayments / default_window) * 100 + "% (precisely " + totalPayments + " / " + default_window + ")"
                );
            }
        });
    });
