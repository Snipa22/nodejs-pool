...
let block_unlock_callback;
let prev_balance_sum;

balanceQueue.drain = function () {
    if (!block_unlock_callback) {
        debug("block_unlock_callback is not defined");
        return;
    }
    if (!prev_balance_sum) {
        debug("prev_balance_sum is not defined");
        return;
    }
    console.log("balanceQueue drained: performing block unlocking");
    global.mysql.query("SELECT SUM(amount) FROM pool.balance").then(function (rows) {
        if (typeof(rows[0]) === 'undefined') {
            full_stop("SELECT SUM(amount) as amt FROM pool.balance query returned undefined result");
	...
	let balance_sum = rows[0].amt;
        if (balance_sum > prev_balance_sum) {
            console.log("Total balance increased from " + global.support.coinToDecimal(prev_balance_sum) + " to " + global.support.coinToDecimal(balance_sum));
...
function calculatePPLNSPayments(block_height, block_reward, block_difficulty) {
    if (is_full_stop) {
        debug("Dropping all PPLNS payments");
        return;
    }
    ...
    if (balanceQueue.idle() === false || block_unlock_callback || prev_balance_sum) {
        debug("Skipping block unlocker run a balanceQueue is not empty");
        return;
    }
...
function altblockUnlocker() {
    ...
    if (balanceQueue.idle() === false || block_unlock_callback || prev_balance_sum) {
        debug("Skipping altblock unlocker run a balanceQueue is not empty");
        return;
    }
        ...
        case global.protos.POOLTYPE.PPLNS:
            global.coinFuncs.getBlockHeaderByHash(block.hash, function (err, header) {
                if (err === null && block.height === header.height && block.value === header.reward && block.difficulty === header.difficulty){
                    global.mysql.query("SELECT SUM(amount) FROM pool.balance").then(function (rows) {
                        if (typeof(rows[0]) === 'undefined') {
                            console.error("SELECT SUM(amount) as amt FROM pool.balance query returned undefined result");
                            return;
                        }
	                prev_balance_sum = rows[0].amt;
                        block_unlock_callback = function() {
                            console.log("Unlocking main block on " + block.height + " height with " + block.hash.toString('hex') + " hash");
                            global.database.unlockBlock(block.hash);
                        }
                        calculatePPLNSPayments(block.height, block.value, block.difficulty);
                    });
                } else {
                    ...
                    global.coinFuncs.getBlockHeaderByID(block.anchor_height, function (anchor_err, anchor_header) {
                        if (anchor_err === null){
	                        ...
	                        prev_balance_sum = rows[0].amt;
                               ...
                               calculatePPLNSPayments(block.anchor_height, block.pay_value, anchor_header.difficulty);
                            });
                        } else {
...
function initial_sync() {
    ...
    global.mysql.query("SELECT id, hex FROM block_log WHERE orphan = 0").then(function (rows) {
        ...
        blockUnlocker();
        setInterval(altblockUnlocker, 2*60*1000);
        altblockUnlocker();
