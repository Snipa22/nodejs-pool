...
let createBalanceQueue = async.queue(function (task, callback) {
    ...
    let query = "SELECT id FROM balance WHERE payment_address = ? AND payment_id is ? AND pool_type = ?";
    if (payment_id !== null) {
        query = "SELECT id FROM balance WHERE payment_address = ? AND payment_id = ? AND pool_type = ?";
    }
    let cacheKey = payment_address + pool_type + payment_id;
    debug("Processing a account add/check for:" + JSON.stringify(task));
    global.mysql.query(query, [payment_address, payment_id, pool_type]).then(function (rows) {
        if (rows.length === 0) {
            global.mysql.query("INSERT INTO balance (payment_address, payment_id, pool_type) VALUES (?, ?, ?)", [payment_address, payment_id, pool_type]).then(function (result) {
    ...
    global.coinFuncs.getLastBlockHeader(function(err, body){
        ...
        async.eachSeries(blockList, function(block, next) {
                ...
                if (block.pay_value !== 0) {
                    ...
                    global.coinFuncs.getPortBlockHeaderByHash(block.port, block.hash, (err, body) => {
                        if ( ( body.topoheight && body.topoheight === -1) ||
                            ...
                            return next();
                        } else if (err !== null && block.port != 8545 && block.port != 8645) {
            ...
            async.eachSeries(Object.keys(preCalcAnchorBlockHashes), function(anchor_height, next) {
                if (--maxPreCount < 0) return next();
		global.coinFuncs.getBlockHeaderByID(anchor_height, function (anchor_err, anchor_header) {
                    if (anchor_err === null){
