...
function calculatePPLNSPayments(block_height, block_reward, block_difficulty) {
    ...
    if (block_unlock_callback !== null || prev_balance_sum !== null) {
        debug("Skipping block unlocker run since balanceQueue is not empty");
        return;
    }
...
function altblockUnlocker() {
    ...
    if (block_unlock_callback !== null || prev_balance_sum !== null) {
        debug("Skipping altblock unlocker run since balanceQueue is not empty");
        return;
    }
