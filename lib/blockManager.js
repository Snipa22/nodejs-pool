...
let createBlockBalanceQueue = async.queue(function (task, callback) {
    ...
    task.hexes.forEach(function(block_hex) {
       if (!sqlq) sqlq = "REPLACE INTO block_balance (hex, payment_address, payment_id, amount) VALUES (?, ?, ?, ?)";
       else sqlq += ", (?, ?, ?, ?)";
       sqlp.push(block_hex);
       sqlp.push(task.payment_address);
       sqlp.push(task.payment_id);
       sqlp.push(task.amount);
    });
    global.mysql.query(sqlq, sqlp).then(function (result) {
        if (!result.hasOwnProperty("affectedRows") || result.affectedRows < task.hexes.length) {
           ...
           console.error("Can't do SQL block balance replace: REPLACE INTO block_balance (" + JSON.stringify(task.hexes) + ", " + task.payment_address + ", " + task.payment_id + ", " + task.amount + ");");
