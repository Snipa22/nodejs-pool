...
function calculatePPLNSPayments(block_hex, block_height, block_reward, block_difficulty, unlock_callback) {
    ...
    console.log("Performing PPLNS payout on block: " + block_height + " Block Value: " + global.support.coinToDecimal(rewardTotal));
    const blockDiff = block_difficulty;
    const windowPPLNS = blockDiff * global.config.pplns.shareMulti;
    ...
    async.doWhilst(function (callback) {
        ...
        global.mysql.query("SELECT SUM(amount) as amt FROM balance").then(function (rows) {
            ...
            let paymentDataCount = 0;
            Object.keys(paymentData).forEach(function (key) {
                if (paymentData[key].amount) ++ paymentDataCount;
            ...
            global.mysql.query("SELECT payment_address, payment_id, amount FROM block_balance WHERE hex = ?", [block_hex]).then(function (rows) {
                if (paymentDataCount == rows.length) {
                    rows.forEach(function (row) {
                        const userIdentifier = row.payment_id ? row.payment_address + "." + row.payment_id : row.payment_address;
                        if (userIdentifier in paymentData) {
                            const amount2 = Math.floor(row.amount * rewardTotal);
                            if (amount2 / paymentData[userIdentifier].amount < 0.99 || amount2 / paymentData[userIdentifier].amount > 1.01) {
                                console.error("Block " + block_hex + " has different payment for recipient " + userIdentifier + ": " + amount2 + " -> " + paymentData[userIdentifier].amount);
                            }
                        } else {
                            console.error("Block " + block_hex + " has unkown payment recipient " + userIdentifier);
                        }
                    });
                    console.log("Block " + block_hex + " pre calc payment was valided");
                } else {
                    console.error("Block " + block_hex + " has different number of precomputed payments " + rows.length + " vs " + paymentDataCount + " now");
                }
            });

            console.log("PPLNS payout cycle complete on block: " + block_height + " Block Value: " + global.support.coinToDecimal(rewardTotal) + " Payout Percentage: " + (totalPayments / pay_window) * 100 + "% (precisely " + totalPayments + " / " + pay_window + ")");
            if (is_need_correction) {
                console.warn("(This PPLNS payout cycle complete on block was corrected: " + block_height + " Block Value: " + global.support.coinToDecimal(rewardTotal) + " Payout Percentage: " + (totalPayments / default_window) * 100 + "% (precisely " + totalPayments + " / " + default_window + "))");
                global.support.sendEmail(global.config.general.adminEmail,
                    ...
                    "PPLNS payout cycle complete on block: " + block_height + " Block Value: " + global.support.coinToDecimal(rewardTotal) + " Payout Percentage: " + (totalPayments / pay_window) * 100 + "% (precisely " + totalPayments + " / " + pay_window + ")\n" +
                    "(This PPLNS payout cycle complete on block was corrected: " + block_height + " Block Value: " + global.support.coinToDecimal(rewardTotal) + " Payout Percentage: " + (totalPayments / default_window) * 100 + "% (precisely " + totalPayments + " / " + default_window + "))"
                );
            }
        });
    });
    ...
    global.coinFuncs.getLastBlockHeader(function(err, body){
        ...
        blockList.forEach(function (block) {
            global.coinFuncs.getBlockHeaderByID(block.height, (err, body) => {
                ...
                } else if (block.poolType == global.protos.POOLTYPE.PPLNS && !(block.hash in payReadyBlockHashCalc) && block.pay_ready !== true) {
                ...
                } else if (topBlockHeight - block.height > global.config.payout.blocksRequired /*&& (block.poolType != global.protos.POOLTYPE.PPLNS || block.pay_ready === true)*/) {
                    blockPayments(block);
                }
            });
    ...
    global.coinFuncs.getLastBlockHeader(function(err, body){
        ...
        async.eachSeries(blockList, function(block, next) {
            global.coinFuncs.getPortBlockHeaderByID(block.port, block.height, (err, body) => {
                ...
                } else if (true/*(block.poolType != global.protos.POOLTYPE.PPLNS || block.pay_ready === true)*/) {
        ...
        case global.protos.POOLTYPE.PPLNS:
            global.coinFuncs.getBlockHeaderByHash(block.hash, function (err, header) {
                if (err === null && block.height === header.height && block.value === header.reward && block.difficulty === header.difficulty){
                    ...
                    calculatePPLNSPayments(block.hash, block.height, block.value, block.difficulty, function() {
                ...
                if (err === null && block.height === header.height && block.value === header.reward && block.difficulty === header.difficulty){
                    global.coinFuncs.getBlockHeaderByID(block.anchor_height, function (anchor_err, anchor_header) {
                        if (anchor_err === null){
                            ...
                            paymentInProgress = true;
                            calculatePPLNSPayments(block.hash, block.anchor_height, block.pay_value, anchor_header.difficulty, function() {
