    ...
    global.coinFuncs.getLastBlockHeader(function(err, body){
        ...
        async.eachSeries(blockList, function(block, next) {
            ...
            if (is_pplns_block && !(block.hash in payReadyBlockHashCalc) && block.pay_ready !== true) {
                if (block.value) {
                    if (!(block.anchor_height in preCalcAnchorBlockHashes)) preCalcAnchorBlockHashes[block.anchor_height] = [];
                    preCalcAnchorBlockHashes[block.anchor_height].push(block.hash);
                    payReadyBlockHashCalc[block.hash] = 1;
            ...
            console.log("Running altblock prepayment for " + Object.keys(preCalcAnchorBlockHashes).length + " anchor heights");
