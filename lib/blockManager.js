...
function preCalculatePPLNSPayments(block_hex, block_height, block_difficulty, is_store_dump, done_callback) {
    ...
    async.doWhilst(function (callback) {
        ...
        for (let found = (cursor.goToRange(blockCheckHeight) === blockCheckHeight); found; found = cursor.goToNextDup()) {
		    ...
		    shares4dump.push(userIdentifier.slice(-16) + "\t" + shareData.timestamp.toString(16) + "\t" + shareData.shares + "\t" + shareData.share_num + "\t" +
        ...
        if (totalPayments == 0) {
            ...
            global.coinFuncs.getLastBlockHeader(function(err, body){
                ...
                return preCalculatePPLNSPayments(block_hex, topBlockHeight, block_difficulty, is_store_dump, done_callback);
            });
            return;
        } else {
            if (is_store_dump && fs.existsSync("./block_share_dumps/process.sh")) {
                shares4dump.sort();
                shares4dump.unshift("#last_16_chars_of_xmr_address\ttimestamp\traw_share_diff\tshare_count\tshare_coin\txmr_share_diff\txmr_share_diff_payed");
                ...
                fs.writeFile(fn, shares4dump.join("\n"), function(err) {
                    ...
                    child_process.exec("./block_share_dumps/process.sh " + fn, function callback(error, stdout, stderr) {
                        if (error) console.error("./block_share_dumps/process.sh " + fn + ": returned error exit code: " + error.code + "\n" + stdout + "\n" + stderr);
...
//    }
    ...
    global.coinFuncs.getLastBlockHeader(function(err, body){
        ...
        async.eachSeries(blockList, function(block, next) {
            ...
            const is_pplns_block = block.poolType == global.protos.POOLTYPE.PPLNS;
            console.log("Start " + block.port + " " + block.height);
            global.coinFuncs.getPortBlockHeaderByID(block.port, block.height, (err, body) => {
                console.log("Finish " + block.port + " " + block.height);
                ...
                if (is_valid_request && body.hash !== block.hash) {
                    ...
                    console.log("Pre " + block.port + " " + block.height);
                    global.coinFuncs.getBlockHeaderByID(block.anchor_height, function (anchor_err, anchor_header) {
                        if (anchor_err === null){
                            payReadyBlockHashCalc[block.hash] = 1;
                            preCalculatePPLNSPayments(block.hash, block.anchor_height, anchor_header.difficulty, (topBlockHeight - block.anchor_height) < 120, function(status) {
                                if (status) {
                    ...
                    if (block.pay_value !== 0) {
                        console.log("Pay " + block.port + " " + block.height);
...
function altblockPayments(block, cb) {
    ...
    switch (block.poolType) {
        case global.protos.POOLTYPE.PPLNS:
            console.log("Start2 " + block.port + " " + block.height);
            global.coinFuncs.getPortBlockHeaderByHash(block.port, block.hash, function (err, header) {
                console.log("Finish2 " + block.port + " " + block.height);
                if (err === null && block.height === header.height && block.value >= header.reward /*&& block.difficulty === header.difficulty*/){
                    //global.coinFuncs.getBlockHeaderByID(block.anchor_height, function (anchor_err, anchor_header) {
                        //if (anchor_err === null){
                            //if (paymentInProgress) {
