...
function doPPLNSPayments(block_hex, block_reward, unlock_callback) {
    ...
    global.mysql.query("SELECT SUM(amount) as amt FROM balance").then(function (rows) {
        ...
        global.mysql.query("SELECT payment_address, payment_id, amount FROM block_balance WHERE hex = ?", [block_hex]).then(function (rows) {
            if (rows.length) {
                global.mysql.query("INSERT INTO paid_blocks (hex, amount) VALUES (?,?)", [block_hex, block_reward]).then(function () {
    ...
    global.coinFuncs.getLastBlockHeader(function(err, body){
        ...
        async.eachSeries(blockList, function(block, next) {
            ...
            if (is_pplns_block && !(block.hash in payReadyBlockHashCalc) && block.pay_ready !== true) {
                if (block.value) global.coinFuncs.getBlockHeaderByID(block.anchor_height, function (anchor_err, anchor_header) {
                    if (anchor_err === null){
                        payReadyBlockHashCalc[block.hash] = 1;
                        preCalculatePPLNSPayments(block.hash, block.anchor_height, anchor_header.difficulty, true, function(status) {
                            if (status) {
