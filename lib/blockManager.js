...
function preCalculatePPLNSPayments(block_hex, block_height, block_difficulty, done_callback) {
    ...
    async.doWhilst(function (callback) {
        ...
        if (totalPayments == 0) {
            console.warn("This PPLNS payout cycle complete on block was redone for top height: " + block_height);
            global.support.sendEmail(global.config.general.adminEmail,
                "Warning: No shares to pay block, so it was corrected by using the top height",
                "PPLNS payout cycle complete on block: " + block_height
            );
