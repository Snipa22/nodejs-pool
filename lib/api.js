...
app.get('/user/:address', function (req, res) {
    global.mysql.query("SELECT payout_threshold, enable_email FROM users WHERE username = ?", [req.params.address]).then(function(row){
        return res.json({msg: {payout_threshold: row[0].payout_threshold, email_enabled: row[0].enable_email}});
    });
});

app.post('/user/toggleEmail', function (req, res) {
    console.log(JSON.stringify(req.body));
    const enabled = req.body.enabled;
    if (!enabled) return res.status(401).send({'success': false, 'msg': "Can't set enabled to a wrong value"});
...
app.post('/user/updateThreshold', function (req, res) {
    ...
    global.mysql.query("UPDATE users SET payout_threshold = ? WHERE username = ?", [threshold2, req.body.username]).then(function () {
