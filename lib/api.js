...
app.post('/user/subscribeEmail', function (req, res) {
    ...
    if (!from && to) {
        global.mysql.query("UPDATE users SET enable_email = ?, email = ? WHERE username = ? AND email = NULL", [enabled, to, username]).then(function (result) {
	    if (!result.hasOwnProperty("affectedRows") || result.affectedRows != 1) {
                ...
                global.mysql.query("INSERT INTO users (username, enable_email, email) VALUES (?, ?, ?)", [username, enabled, to]).then(function (result) {
	            if (!result.hasOwnProperty("affectedRows") || result.affectedRows != 1) {
