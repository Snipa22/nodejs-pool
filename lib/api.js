...
app.get('/user/:address', function (req, res) {
    global.mysql.query("SELECT payout_threshold, enable_email FROM users WHERE id = ?", [req.params.address]).then(function(row){
...
app.post('/user/toggleEmail', function (req, res) {
    global.mysql.query("UPDATE users SET enable_email = NOT enable_email WHERE id = ?", [req.body.address]).then(function () {
...
app.post('/user/updateThreshold', function (req, res) {
    ...
    console.log("UPDATE users SET payout_threshold = " + threshold + " WHERE id = " + req.body.username);
    global.mysql.query("UPDATE users SET payout_threshold = ? WHERE id = ?", [threshold, req.body.username]).then(function () {
