...
app.get('/config', function (req, res) {
    res.json({
        ...
        min_difficulty: global.config.pool.minDifficulty,
        max_difficulty: global.config.pool.maxDifficulty,
        payout_timer: global.config.payout.timer,
        payout_feeSlew: global.config.payout.feeSlewAmount,
        divisor: global.config.general.sigDivisor
    });
});
...
//gather last 3 days worth of blocks to generate some stats
//would like to cache results
//would like to show only every 5 instead of every 1
app.get('/network/difficulty/history', function (req, res) {
    let limit = typeof(req.query.limit) !== 'undefined' ? Number(req.query.limit) : 735;
    let page = typeof(req.query.page) !== 'undefined' ? Number(req.query.page) : 0;
