...
app.use(
    rateLimit({
        ...
        max: 500, // limit each IP to 500 requests per time window
    }),
);
            ...
            function (returnData, callback) {
                ...
                global.mysql.query(paidQuery, [address_pt, payment_id]).then(function (rows) {
                    if (typeof rows[0] === 'undefined') {
                        ...
                        returnData.amtPaid = rows[0].amt || 0;
...
app.get('/pool/altblocks/:pool_type', cache('10 seconds'), function (req, res) {
    let limit = typeof req.query.limit !== 'undefined' ? Number(req.query.limit) : 25;
    let page = typeof req.query.page !== 'undefined' ? Number(req.query.page) : 0;
    res.json(global.database.getAltBlockList(req.params.pool_type, page * limit, (page + 1) * limit));
});
...
app.get('/pool/altblocks', cache('10 seconds'), function (req, res) {
    let limit = typeof req.query.limit !== 'undefined' ? Number(req.query.limit) : 25;
    let page = typeof req.query.page !== 'undefined' ? Number(req.query.page) : 0;
    res.json(global.database.getAltBlockList(null, page * limit, (page + 1) * limit));
});
...
app.get('/pool/payments/:pool_type', cache('1 minute'), function (req, res) {
    ...
    let paymentIds = [];
    let query =
        'SELECT distinct(transaction_id) as txnID FROM payments WHERE pool_type = ? ORDER BY transaction_id DESC LIMIT ? OFFSET ?';
    let response = [];
    global.mysql
        .query(query, [pool_type, limit, page * limit])
        .then(function (rows) {
            ...
            rows.forEach(function (row, index, array) {
                ...
                if (array.length === paymentIds.length) {
                    global.mysql
                        .query('SELECT * FROM transactions WHERE id IN (' + paymentIds.join() + ') ORDER BY id DESC')
                        .then(function (txnIDRows) {
        ...
        .then(function (rows) {
            ...
            rows.forEach(function (row, index, array) {
                if (pool_list.length === 1 || (row.id in tx_pool_types && tx_pool_types[row.id] != '?')) {
                    ...
                    global.mysql
                        .query('SELECT pool_type FROM payments WHERE transaction_id = ? LIMIT 1', [row.id])
                        .then(function (ptRows) {
                            if (ptRows.length === 0) {
...
app.get('/miner/:address/payments', cache('1 minute'), function (req, res) {
    ...
    let payment_id = address_parts[1];
    let query =
        'SELECT amount as amt, pool_type, transaction_id, UNIX_TIMESTAMP(paid_time) as ts FROM ' +
    ...
    let response = [];
    global.mysql
        .query(query, [address, payment_id, limit, page * limit])
        .then(function (rows) {
            if (rows.length === 0) return res.json(response);
            rows.forEach(function (row, index, array) {
                debug(threadName + 'Got rows from initial SQL query: ' + JSON.stringify(row));
                global.mysql
                    .query('SELECT transaction_hash, mixin FROM transactions WHERE id = ? ORDER BY id DESC', [
                        row.transaction_id,
                    ])
                    .then(function (txnrows) {
...
app.get('/miner/:address/chart/hashrate/:identifier', cache('10 seconds'), function (req, res) {
    return res.json(
        global.database.getCache('history:' + req.params.address + '_' + req.params.identifier)['hashHistory'],
    );
});
...
app.get('/user/:address', function (req, res) {
    global.mysql
        .query('SELECT payout_threshold, enable_email FROM users WHERE username = ? LIMIT 1', [req.params.address])
        .then(function (row) {
            if (row.length == 1) {
                return res.json({ payout_threshold: row[0].payout_threshold, email_enabled: row[0].enable_email });
            } else {
                return res.json({
                    payout_threshold: global.support.decimalToCoin(global.config.payout.defaultPay),
                    email_enabled: 0,
                });
            }
        });
...
app.post('/user/subscribeEmail', function (req, res) {
    ...
    if (!('enabled' in req.body))
        return res.status(401).send({ success: false, msg: 'No "enabled" parameter was found' });
    ...
    if (from === '' && to === '') {
        global.mysql
            .query('UPDATE users SET enable_email = ? WHERE username = ?', [enabled, username])
        ...
        global.mysql
            ...
            .then(function (result) {
                if (!result.hasOwnProperty('affectedRows') || result.affectedRows != 1) {
                    if (global.database.getCache(username) === false)
                        return res.status(401).send({ success: false, msg: "Can't set email for unknown user" });
                    global.mysql
                        .query('INSERT INTO users (username, enable_email, email) VALUES (?, ?, ?)', [
                            username,
                            enabled,
                            to,
                        ])
        ...
        global.mysql
            .query('UPDATE users SET enable_email = ?, email = ? WHERE username = ? AND email = ?', [
                enabled,
                to,
                username,
                from,
...
app.get('/user/:address/unsubscribeEmail', function (req, res) {
    global.mysql
        .query('UPDATE users SET enable_email = 0 WHERE username = ?', [req.params.address])
        .then(function (result) {
            if (!result.hasOwnProperty('affectedRows') || result.affectedRows != 1) {
                return res.status(401).json({ error: 'This XMR address does not have email subscription' });
            } else {
...
app.post('/user/updateThreshold', function (req, res) {
    ...
    const username = req.body.username;
    if (!username || global.database.getCache(username) === false)
        return res.status(401).send({ success: false, msg: "Can't set threshold for unknown user" });
    const threshold2 = global.support.decimalToCoin(
        threshold < global.config.payout.walletMin ? global.config.payout.walletMin : threshold,
    );
    global.mysql
        .query("SELECT * FROM users WHERE username = ? AND payout_threshold_lock = '1'", [username])
        .then(function (rows) {
            if (rows.length === 0) {
                global.mysql
                    .query(
                        'INSERT INTO users (username, payout_threshold) VALUES (?, ?) ON DUPLICATE KEY UPDATE payout_threshold=?',
                        [username, threshold2, threshold2],
                    )
                    .then(function () {
                        return res.json({
                            msg: 'Threshold updated, set to: ' + global.support.coinToDecimal(threshold2),
                        });
                    });
            } else {
...
app.post('/authenticate', function (req, res) {
    ...
    global.mysql
        .query('SELECT * FROM users WHERE username = ? AND ((pass IS null AND email = ?) OR (pass = ?))', [
            req.body.username,
            req.body.password,
            hmac,
        ])
            ...
            if (rows.length === 0) {
                global.mysql.query('SELECT * FROM users WHERE username = ?', [req.body.username]).then(function (rows) {
                    if (rows.length === 0) {
                        return res
                            .status(401)
                            .send({ success: false, msg: 'Password is not set, so you can not login now.' });
                    }
                    global.mysql
                        .query('SELECT * FROM users WHERE username = ? AND pass IS null', [req.body.username])
                        .then(function (rows) {
                            if (rows.length !== 0) {
                                return res.status(401).send({
                                    success: false,
                                    msg:
                                        'Wrong password. Password equals to string after : character in your miner password field.',
                                });
                            }
                            return res.status(401).send({
                                success: false,
                                msg: 'Wrong password. Password was set by you in Dashboard Options before.',
                            });
                        });
                });
            } else {
                let token = jwt.sign({ id: rows[0].id, admin: rows[0].admin }, global.config.api.secKey, {
                    expiresIn: '1d',
                });
...
secureRoutes.get('/tokenRefresh', function (req, res) {
    let token = jwt.sign({ id: req.decoded.id, admin: req.decoded.admin }, global.config.api.secKey, {
        expiresIn: '1d',
    });
...
secureRoutes.get('/', function (req, res) {
    global.mysql
        .query('SELECT payout_threshold, enable_email, email FROM users WHERE id = ?', [req.decoded.id])
        .then(function (row) {
            return res.json({
                msg: {
                    payout_threshold: row[0].payout_threshold,
                    email_enabled: row[0].enable_email,
                    email: row[0].email,
                },
            });
        });
...
secureRoutes.post('/toggleEmail', function (req, res) {
    global.mysql
        .query('UPDATE users SET enable_email = NOT enable_email WHERE id = ?', [req.decoded.id])
        .then(function () {
...
secureRoutes.post('/changePayoutThreshold', function (req, res) {
    ...
    global.mysql
        .query('UPDATE users SET payout_threshold = ? WHERE id = ?', [threshold, req.decoded.id])
        .then(function () {
