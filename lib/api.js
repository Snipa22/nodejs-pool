...
function getAllWorkerStats(address, callback){
    ...
    let globalHistoryCache = global.database.getCache("history:" + address);
    if (globalHistoryCache === false) globalHistoryCache = globalCache; // for compatibility
    let returnData = {global: {
        lts: Math.floor(globalHistoryCache.lastHash / 1000),
        identifer: 'global',
        hash: globalHistoryCache.hash,
    ...
    identifiers.sort().forEach(function(identifier){
        ...
        let cachedHistoryData = global.database.getCache("history:" + address+"_"+identifier);
        if (cachedHistoryData === false) cachedHistoryData = cachedData; // for compatibility
        returnData[identifier] = {
            lts: Math.floor(cachedHistoryData.lastHash / 1000),
            identifer: identifier,
            hash: cachedHistoryData.hash,
...
function getAddressStats(address, extCallback){
    ...
    let cachedHistoryData = global.database.getCache("history:" + address);
    if (cachedHistoryData === false) cachedHistoryData = cachedData; // for compatibility
    ...
    async.waterfall([
        function (callback) {
            ...
            return callback(null, {hash: cachedHistoryData.hash, identifier: 'global', lastHash: Math.floor(cachedHistoryData.lastHash / 1000),
...
app.get('/pool/coin_altblocks/:coin_port', function (req, res) {
    let limit = typeof(req.query.limit) !== 'undefined' ? Number(req.query.limit) : 25;
    let page = typeof(req.query.page) !== 'undefined' ? Number(req.query.page) : 0;
    res.json(global.database.getAltBlockList(null, req.params.coin_port).slice(page*limit, (page + 1) * limit));
});
...
app.get('/miner/:address/stats/:identifier', function (req, res) {
    ...
    let cachedHistoryData = global.database.getCache("history:" + memcKey);
    if (cachedHistoryData === false) cachedHistoryData = cachedData; // for compatibility
    return res.json({
        lts: Math.floor(cachedHistoryData.lastHash / 1000),
        identifer: identifier,
