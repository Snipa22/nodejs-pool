...
app.post('/user/toggleEmail', function (req, res) {
    global.mysql.query("UPDATE users SET enable_email = NOT enable_email WHERE username = ?", [req.body.address]).then(function () {
...
app.post('/user/updateThreshold', function (req, res) {
    ...
    global.mysql.query("UPDATE users SET payout_threshold = ? WHERE username = ?", [threshold, req.body.username]).then(function () {
