...
app.get('/user/:address', function (req, res) {
    global.mysql.query("SELECT payout_threshold, enable_email FROM users WHERE username = ? LIMIT 1", [req.params.address]).then(function(row){
        if (row.length == 1) {
            ...
            return res.json({msg: {payout_threshold: global.support.decimalToCoin(global.config.payout.defaultPay), email_enabled: 0}});
        }
    });
});
...
app.post('/user/subscribeEmail', function (req, res) {
    ...
    if (!enabled) return res.status(401).send({'success': false, 'msg': "No \"enabled\" parameter was found"});
    global.mysql.query("UPDATE users SET enable_email = ? WHERE username = ?", [enabled, req.body.username]).then(function () {
        return res.json({'msg': 'Email preferences were updated'});
    });
});

app.post('/user/unsubscribeEmail', function (req, res) {
    global.mysql.query("UPDATE users SET enable_email = 0 WHERE username = ?", [req.body.username]).then(function (result) {
...
app.post('/user/updateThreshold', function (req, res) {
    ...
    if (!req.body.username || global.database.getCache(req.body.username) === false) return res.status(401).send({'success': false, 'msg': "Can't set threshold for unknown user"});
    ...
    global.mysql.query("INSERT INTO users (username, payout_threshold) VALUES (?, ?) ON DUPLICATE KEY UPDATE payout_threshold=?", [req.body.username, threshold2, threshold2]).then(function () {
