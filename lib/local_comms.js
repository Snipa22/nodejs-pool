...
function Database(){
    ...
    this.getLastBlock = function(blockType){
        ...
        cursor.close();
        txn.commit();
        debug("Done getting the most recent block for: "+ blockType + " at height: "+ highestBlock);
        return highestBlock;
    };
                ...
                global.coinFuncs.getLastBlockHeader(function(body){
                    if (oldestLockedBlock === null){
                        ...
                        if (global.config.pps.enable){
                            ...
                            callback(null, body.height - Math.floor(global.config.payout.blocksRequired/2), Math.floor(body.difficulty * global.pplns.shareMulti * 1.3));
                        } else {
                            // Otherwise, we can just start from the current height.  Woo!
                            callback(null, body.height, Math.floor(body.difficulty * global.pplns.shareMulti * 1.3));
                        }
                        ...
                        if (global.config.pps.enable && oldestLockedBlock.height > body.height - Math.floor(global.config.payout.blocksRequired/2)) {
                            ...
                            callback(null, oldestLockedBlock.height, Math.floor(oldestLockedBlock.difficulty * global.pplns.shareMulti * 1.3));
                        }
                    }
                });
            },
