...
function Database(){
    ...
    this.incrementCacheDataPart = function(key, data){
        ...
        if (cached !== null){
            ...
            if (!global.database.roundhashes_reset_times.hasOwnProperty(key)) {
                console.error("Unknown key " + key + " for global.database.roundhashes_reset_times!!!");
            }
            data.forEach(function(intDict){
                if (intDict.location === 'roundHashes' && intDict.value === false) {
                    cached[intDict.location] = 0;
                    let time_now = Date.now();
                    global.database.roundhashes_reset_times[key] = time_now;
                    txn.putString(this.cacheDB, key + '_roundhashes_reset_time', time_now.toString());

                } else if (intDict.location === 'roundHashes' && global.database.roundhashes_reset_times[key] < reset_time) {
                    cached[intDict.location] = intDict.value;
                        ...
                        if (cacheStore === null){
                            ...
                            if (cachedData[key].hasOwnProperty('goodShares')) {
                                ...
                                if (json_cache.hasOwnProperty('roundHashes')) {
                                    ...
                                    if (global.database.roundhashes_reset_times[key] < reset_time) {
