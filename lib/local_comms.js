...
function Database(){
    ...
    this.worker_key = function(key) {
      if (this.hasOwnProperty("worker_id")) {
        return key + "_" + this.worker_id.toString();
      } else {
    ...
    this.incrementCacheDataPart = function(key, data){
        this.refreshEnv();
        let txn = this.env.beginTxn();
        let cached = txn.getString(this.cacheDB, this.worker_key(key));
        if (cached !== null){
            cached = JSON.parse(cached);
            ...
            txn.putString(this.cacheDB, this.worker_key(key), JSON.stringify(cached));
            txn.commit();
        } else {
            txn.abort();
        }
            ...
            if(shareObject.length === shareCount){
                ...
                for (let key in cachedData){
                    if (cachedData.hasOwnProperty(key) && cachedData[key].totalHashes !== 0){
                        ...
                        let key2 = cachedData[key].hasOwnProperty('roundHashes') ? this.worker_key(key) : key;
    ...
    this.splitCache = function(cacheKey){
        ...
        try {
            ...
            if (cached !== null){
                ...
                txn.putString(this.cacheDB, this.worker_key(cacheKey), JSON.stringify(json_cache));
