    ...
    this.storeBulkShares = function(shareObject) {
        ...
        shareObject.forEach(function(share){
            ...
            if (typeof(share.shares) === "number") {
                ...
                let minerIDWithIdentifier = minerID + "_" + share.identifier;
                let port_suffix = typeof(share.port) !== 'undefined' ? "_" + share.port.toString() : "";
                let global_stats = "global_stats2" + port_suffix;
                let stats_type = poolTypeStr(share.poolType) + "_stats2" + port_suffix;
                if (!(global_stats in cachedData)) {
                    cachedData[global_stats] = { totalHashes: 0, roundHashes: 0 };
                }
                if (!(stats_type in cachedData)) {
                    cachedData[stats_type] = { totalHashes: 0, roundHashes: 0 };
                }
                if (typeof(share.port) !== 'undefined') {
    ...
    this.storeBlock = function(blockId, blockData, callback){
        ...
        try{
            ...
            global.coinFuncs.getBlockHeaderByHash(blockDataDecoded.hash, function(err, header){
                ...
                let port_str = "_" + global.config.daemon.port.toString();
                if (global.database.getCache(poolTypeStr(blockDataDecoded.poolType) + "_stats2" + port_str) === false) {
                    port_str = "";
                }
                let shares = global.database.getCache(poolTypeStr(blockDataDecoded.poolType) + "_stats2" + port_str);
                ...
                if (txn.getBinary(global.database.blockDB, blockId) !== null) {
                    txn.abort();
                    console.error("Can't store already stored block with " + blockId.toString() + " key and " + header.reward.toString() + " value");
                    return callback(true);
                }
                txn.putBinary(global.database.blockDB, blockId, blockData);
                txn.commit();
                global.database.incrementCacheData('global_stats2' + port_str, [{location: 'roundHashes', value: false}]);
                global.database.incrementCacheData(poolTypeStr(blockDataDecoded.poolType) + '_stats2' + port_str, [{location: 'roundHashes', value: false}]);
                return callback(true);
            });
        } catch (e) {
    ...
    this.storeAltBlock = function(blockId, blockData, callback){
        ...
        try{
            ...
            global.coinFuncs.getPortBlockHeaderByHash(blockDataDecoded.port, blockDataDecoded.hash, function(err, header){
                ...
                //blockData = global.database.calculateShares(blockDataDecoded, blockId);
                let port_str = "_" + blockDataDecoded.port.toString();
                let shares = global.database.getCache(poolTypeStr(blockDataDecoded.poolType) + "_stats2" + port_str);
                blockDataDecoded.shares = shares ? shares.roundHashes : 0;
                ...
                if (global.database.isAltBlockInDB(blockDataDecoded.port, blockDataDecoded.height)) {
                    console.error("Can't store already stored altblock with " + blockDataDecoded.port.toString() + " port and " + blockDataDecoded.height.toString() + " height and " + header.reward.toString() + " value");
                    return callback(true);
                }
                let txn = global.database.env.beginTxn();
