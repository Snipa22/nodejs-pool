...
function Database(){
    ...
    this.poolTypeStr = function(poolType) {
        switch (poolType) {
            case global.protos.POOLTYPE.PPLNS:
                return 'pplns';
            case global.protos.POOLTYPE.PPS:
    ...
    this.storeBulkShares = function(shareObject) {
        ...
        shareObject.forEach(function(share){
            ...
            if (typeof(share.shares) === "number") {
                ...
                let stats_type = this.poolTypeStr(share.poolType) + "_stats2" + port_suffix;
    ...
    this.storeBlock = function(blockId, blockData, callback){
        ...
        try{
            ...
            global.coinFuncs.getBlockHeaderByHash(blockDataDecoded.hash, function(err, header){
                ...
                if (global.database.getCache(this.poolTypeStr(blockDataDecoded.poolType) + "_stats2" + port_str) === false) {
                    port_str = "";
                }
                let shares = global.database.getCache(this.poolTypeStr(blockDataDecoded.poolType) + "_stats2" + port_str).roundHashes;
                ...
                global.database.incrementCacheData(this.poolTypeStr(blockDataDecoded.poolType) + '_stats2' + port_str, [{location: 'roundHashes', value: false}]);
