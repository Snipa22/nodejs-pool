    ...
    this.storeBlock = function(blockId, blockData, callback){
        ...
        try{
            ...
            global.coinFuncs.getBlockHeaderByHash(blockDataDecoded.hash, function(err, header){
                ...
                if (err && typeof(err.message) === 'string' && err.message.indexOf("can't get block by hash") > -1) {
                    let time_now = Date.now();
                    if (blockDataDecoded.hash in orphanBlocks) {
                        if (time_now - orphanBlocks[blockDataDecoded.hash] > 60*60*1000) return callback(true);
                    } else orphanBlocks[blockDataDecoded.hash] = time_now;
                }
