...
const {tryParseMergeMiningAddress} = require("../common");
...
function Coin(data) {
    ...
    this.validateAddress = function (address) {
        try {
            let {monero} = tryParseMergeMiningAddress(address);
            // TODO
            return true;

            if (this.validatePlainAddress(monero)) return true;
            // This function should be able to be called from the async library, as we need to BLOCK ever so slightly to verify the address.
            monero = new Buffer(monero);
            return cnUtil.address_decode_integrated(monero) === this.intPrefix;
        } catch (err) {
            console.error(err);
            return false;
        }
