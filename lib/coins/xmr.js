...
function Coin(data){
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        else if ("cn" in algos_perf)            coin_perf[""]     = coin_perf["LTHN"] = algos_perf["cn"];
        else if ("cn/1" in algos_perf)          coin_perf[""]     = coin_perf["LTHN"] = algos_perf["cn/1"];
    ...
    this.cryptoNight = function(convertedBlob, port, height) {
        switch (port) {
            ...
            case 48782: return multiHashing.cryptonight(convertedBlob, convertedBlob[0] >= 6 ? 13 : 8);       // Lethean
    ...
    this.algoShortTypeStr = function(port, version) {
        switch (port) {
            ...
            case 18081: return version >= 10 ? "cn/r" : "cn/2";          // XMR
            ...
            case 48782: return version >= 6 ? "cn/r" : "cn/2";          // Lethean
            default:    return "cn/2";
        }
    }
    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            if (majorv + minorv + minorv2 < 20900) {
                return "Please update your xmr-stak miner (" + agent + ") to v2.9.0+ (use monero in config now and cryptonight_r after fork) to support new cn/r Monero algo before March 9 Monero fork";
            }
        } else if (m = reXNP.exec(agent)) {
            const majorv = parseInt(m[1]) * 10000;
            const minorv = parseInt(m[2]) * 100;
