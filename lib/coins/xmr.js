...
function Coin(data){
    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            return "Please update your SRBminer (" + agent + ") to SRBminer-MULTI miner v0.1.5+ to support new rx/0 Monero algo";
    ...
    this.get_miner_agent_not_supported_notification = function(agent) {
        let m;
        if (m = reXMRSTAKRX.exec(agent)) {
            return false;
        } else if (m = reXMRSTAK.exec(agent)) {
            return "You must update your xmr-stak miner (" + agent + ") to xmr-stak-rx miner to support new rx/0 Monero algo";
        } else if (m = reSRB.exec(agent)) {
            return "You must update your SRBminer (" + agent + ") to SRBminer-MULTI miner v0.1.5+ to support new rx/0 Monero algo";
        }
        return false;
    };
