...
function Coin(data){
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        if      ("cn/wow" in algos_perf)        coin_perf[""]     = coin_perf["WOW"]  = algos_perf["cn/wow"];

        if      ("cn/4" in algos_perf)          coin_perf[""]     = algos_perf["cn/4"];

        if (!("" in coin_perf)) return "algo_perf set must include cn, cn/1, cn/2, cn/4 or cn/wow hashrate";
    ...
    this.cryptoNight = function(convertedBlob, port, height) {
        switch (port) {
            ...
            case 18081: return multiHashing.cryptonight(convertedBlob, convertedBlob[0] >= 10 ? 13 : 8, height);       // XMR
    ...
    this.algoShortTypeStr = function(port, version) {
        switch (port) {
            ...
            case 18081: return version >= 10 ? "cn/4" : "cn/2";          // XMR
    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            if (majorv + minorv < 21300) {
                return "Please update your XMRig miner (" + agent + ") to v2.13.0+ to support new cn/4 Monero algo before March 9 Monero fork";
            }
        /*} else if (m = reXMRSTAK2.exec(agent)) {
            ...
            const version = majorv + minorv + minorv2;
            if (version < 800) {
                 return "Please update your xmr-node-proxy (" + agent + ") to version v0.8.0+ by doing 'cd xmr-node-proxy && ./update.sh' (or check https://github.com/MoneroOcean/xmr-node-proxy repo)";
            }
        } /*else if (m = reCAST.exec(agent)) {
            const majorv = parseInt(m[1]) * 10000;
            const minorv = parseInt(m[2]) * 100;
