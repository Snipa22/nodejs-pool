...
function calcEthReward(block, tx_reciepts) {
    ...
    fee -= parseInt(block.baseFeePerGas) * parseInt(block.gasUsed);
...
function Coin(data){
        ...
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon(port, 'getblockheaderbyheight', {"height": blockId}, function (body) {
                if (body && body.hasOwnProperty('result')) {
                    if (port == 20206) body.result.block_header.timestamp /= 1000; // DERO is special here
    ...
    this.ethBlockCheck = function(miner_hex, nonce_hex, block_height_hex, callback) {
        const port = 8545;
    ...
    this.ethBlockFind = function(nonce_hex, callback) {
        const miner_hex = global.config.pool["address_" + 8545];
        global.coinFuncs.ethBlockCheck(miner_hex, nonce_hex, "latest", function(block_hash, block_height) {
            ...
            async.eachSeries(Array(32).fill().map((element, index) => block_height - index - 1), function(block_height, next) {
                global.coinFuncs.ethBlockCheck(miner_hex, nonce_hex, "0x" + block_height.toString(16), function(block_hash) {
...
//        if      ("rx/wow" in algos_perf)        coin_perf["WOW"]  = algos_perf["rx/wow"];
    ...
    this.slowHashBuff = function(convertedBlob, blockTemplate, nonce, mixhash) {
	    ...
	    case 19994: return multiHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), 2);	   // ArqMa
            ...
            case 20206: return multiHashing.astrobwt2(convertedBlob, 0);                                                    // Dero
