...
function Coin(data){
    ...
    this.convertBlob = function(blobBuffer, port) {
        ...
        if (this.blobTypeRaven(blob_type_num)) return cnUtil.convertRavenBlob(blobBuffer);
        let blob;
        try {
    ...
    this.constructNewBlob = function(blockTemplate, nonce, port, mixhash) {
        const blob_type_num = this.portBlobType(port, blockTemplate[0]);
        switch (blob_type_num) {
            case 100: return cnUtil.constructNewGrinBlob(blockTemplate, bignum(nonce, 10).toBuffer({endian: 'little', size: 4}));
            case 101: return cnUtil.constructNewRavenBlob(blockTemplate, bignum(nonce, 16).toBuffer({endian: 'little', size: 8}), bignum(mixhash, 16).toBuffer({endian: 'little', size: 32}));
            default:  return cnUtil.construct_block_blob(blockTemplate, new Buffer(nonce, 'hex'), blob_type_num);
        }
    };
    ...
    this.slowHashBuff = function(convertedBlob, blockTemplate, nonce, mixhash) {
        switch (blockTemplate.port) {
            case 8766:  return multiHashing.kawpow(convertedBlob, reverseBuffer(Buffer.from(nonce, 'hex')), Buffer.from(mixhash, 'hex')); // RVN
