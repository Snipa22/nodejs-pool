...
function Coin(data){
    ...
    this.getPortBlockTemplate = function(port, callback) {
        if (port == 11898) {
            global.support.rpcPortDaemon2(port, 'block/template', {
                address: global.config.pool[port == global.config.daemon.port ? "address" : "address_" + port.toString()],
    ...
    this.BlockTemplate = function(template) {
        ...
        this.blocktemplate_blob = template.blocktemplate_blob ? template.blocktemplate_blob : template.blob;
