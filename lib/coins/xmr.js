    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon2(port, '', { method: 'getblock', params: [ blockHash, 2 ] }, function (body) {
                ...
                if (port == 8766 || port == 19001) { // XNA, RVN
                  body.result.reward = 0;
                  for (const vout of body.result.tx[0].vout) body.result.reward += vout.value;
            ...
            const miner_tx_hash = port == 18981 ? "" : (body.result.miner_tx_hash == "" ? body.result.block_header.miner_tx_hash : body.result.miner_tx_hash);
