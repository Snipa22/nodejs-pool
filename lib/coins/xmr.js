...
let shareVerifyQueue = async.queue(function (task, queueCB) {
    ...
    const verify_shares_host = global.config.verify_shares_host instanceof String ? global.config.verify_shares_host :
                               global.config.verify_shares_host[next_verify_share_host++ % global.config.verify_shares_host.length];
...
function Coin(data){
    ...
    this.slowHashAsync = function(convertedBlob, blockTemplate, miner_address, cb) {
        ...
        return shareVerifyQueue.unshift({ jsonInput: jsonInput, cb: cb, time: Date.now(), miner_address: miner_address });
    }
