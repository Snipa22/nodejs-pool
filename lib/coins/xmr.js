...
global.config.verify_shares_host.forEach(function(verify_shares_host, index) {
    shareVerifyQueue[index] = async.queue(function (task, queueCB) {
        ...
        let timer = setTimeout(function() {
            socket.destroy();
            if (!shareVerifyQueueErrorTime[index] || Date.now() - shareVerifyQueueErrorTime[index] > 30*60*1000) {
    ...
    setInterval(function(queue_obj){
        if (queue_obj.length() > 1000) {
            ...
            console.error(global.database.thread_id + "Share verify queue state: " + queue_obj.length() + " items in the queue " + queue_obj.running() + " items being processed");
    ...
    }, 30*1000, shareVerifyQueue[index]);
});
...
function Coin(data){
    ...
    this.slowHashAsync = function(convertedBlob, blockTemplate, miner_address, cb) {
        ...
        const host_count = global.config.verify_shares_host.length;
        let max_attempts = host_count;
        do {
            ++ this.verify_share_host_index;
            if (this.verify_share_host_index >= host_count) this.verify_share_host_index = 0;
        } while (--max_attempts && shareVerifyQueueErrorTime[this.verify_share_host_index] && time_now - shareVerifyQueueErrorTime[this.verify_share_host_index] < 5*60*1000);
        return shareVerifyQueue[this.verify_share_host_index].unshift({ jsonInput: jsonInput, cb: cb, time: time_now, miner_address: miner_address });
    }
