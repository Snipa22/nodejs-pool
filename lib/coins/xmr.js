...
function Coin(data){
    ...
    this.cryptoNight = function(convertedBlob, port, height) {
        switch (port) {
            ...
            case 12211: return convertedBlob[0] >= 6 ? multiHashing.cryptonight(convertedBlob, 11) : multiHashing.cryptonight_heavy(convertedBlob, 0); // RYO
            ...
            case 34568: return convertedBlob[0] >= 11 ? multiHashing.cryptonight(convertedBlob, 12, height) : multiHashing.cryptonight(convertedBlob, 8);      // Wownero
    ...
    this.algoShortTypeStr = function(port, version) {
        switch (port) {
            ...
            case 12211: return version >= 6 ? "cn/gpu" : "cn-heavy/0";    // RYO
            ...
            case 34568: return version >= 11 ? "cn/wow" : "cn/2";          // Wownero
    ...
    this.variantValue = function(port, version) {
        switch (port) {
            ...
            case 12211: return version >= 6 ? "gpu" : "0";    // RYO
            ...
            case 34568: return version >= 11 ? "wow" : "2";    // Wownero
    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        /*if (m = reXMRig.exec(agent)) {
            ...
            const version = majorv + minorv + minorv2;
            if (version < 700) {
                 return "Please update your xmr-node-proxy (" + agent + ") to version v0.7.0+ by doing 'cd xmr-node-proxy && ./update.sh' (or check https://github.com/MoneroOcean/xmr-node-proxy repo)";
            }
        } /*else if (m = reCAST.exec(agent)) {
            const majorv = parseInt(m[1]) * 10000;
            const minorv = parseInt(m[2]) * 100;
