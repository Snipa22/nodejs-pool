                ...
                _this.getPortBlockHeaderByID(port, body.result.height, function(err, body_height) {
                    ...
                    // uncle block?
                        ...
                        _this.getPortBlockHeaderByID(port, block_height, function(err, body_height) {
                            ...
                            global.support.rpcPortDaemon2(port, '', { jsonrpc: "2.0", id: 1, method: 'eth_getUncleByBlockHashAndIndex', params: [ "0x" + blockHash, "0x" + uncleIndex.toString(16) ] }, function (body_uncle) {
                                if (!body_uncle || !body_uncle.result) {
                                    console.error("eth_getUncleByBlockHashAndIndex:" + JSON.stringify(body_uncle));
                                    return next();
                                }
                                return next((ETH_BASE_REWARD * (8 - (parseInt(body_height.number) - parseInt(body_uncle.number))) / 8) * ETH_MULTIPLIER);
                            });
                        });
