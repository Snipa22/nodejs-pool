...
const mm_port_set = { "22023": 11898, "33124": 11898 };
...
function Coin(data){
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        if      ("cn/r" in algos_perf)          coin_perf[""]     = coin_perf["SUMO"] = coin_perf["LTHN"] = coin_perf["WOW"]  = algos_perf["cn/r"];
        else if ("cn" in algos_perf)            coin_perf[""]     = coin_perf["SUMO"] = coin_perf["LTHN"] = coin_perf["WOW"]  = algos_perf["cn"];
        else if ("cn/4" in algos_perf)          coin_perf[""]     = coin_perf["SUMO"] = coin_perf["LTHN"] = coin_perf["WOW"]  = algos_perf["cn/4"];
        ...
        if      ("cn-pico"      in algos_perf)  coin_perf["XTNC"] = coin_perf["LOKI"] = coin_perf["TRTL"] = algos_perf["cn-pico"];
        else if ("cn-pico/trtl" in algos_perf)  coin_perf["XTNC"] = coin_perf["LOKI"] = coin_perf["TRTL"] = algos_perf["cn-pico/trtl"];
    ...
    this.cryptoNight = function(convertedBlob, blockTemplate) {
        switch (blockTemplate.port) {
            ...
            case 18081: return multiHashing.cryptonight(convertedBlob, 13, blockTemplate.height);			// XMR
            ...
            case 34568: return multiHashing.random_wow(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'));	// Wownero
    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            const version = majorv + minorv + minorv2;
            if (version < 1000) {
                 return "Please update your xmr-node-proxy (" + agent + ") to version v0.10.0+ by doing 'cd xmr-node-proxy && ./update.sh' (or check https://github.com/MoneroOcean/xmr-node-proxy repo) to support new rx/wow algo";
            }
        } else if (m = reSRB.exec(agent)) {
            const majorv = parseInt(m[1]) * 10000;
            const minorv = parseInt(m[2]) * 100;
