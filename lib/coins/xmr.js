...
setInterval(function(queue_obj){
    if (queue_obj.length() > 0) {
        let miner_address = {};
        queue_obj.remove(function(task) {
            console.log(task.miner_address + " " + task.time);
            if (!(task.miner_address in miner_address)) miner_address[task.miner_address] = 1;
            else ++ miner_address[task.miner_address];
            if (Date.now() - task.time > 1*60*1000) {
               task.cb(null);
               return true;
            }
        ...
        Object.keys(miner_address).forEach(function(key) {
            ...
            if (value > 0) console.error("Too many shares from " + key + ": " + value);
        });
    }
...
function Coin(data){
    ...
    this.slowHashAsync = function(convertedBlob, blockTemplate, miner_address, cb) {
        ...
        return shareVerifyQueue.unshift({ jsonInput: jsonInput, cb: cb, time: Date.now(), miner_address: "xxx" });
    }
