...
const pool_nonce_size = 16+1; // 1 extra byte for old daemon bug
const port2coin = {
    "11181": "AEON",
    "11898": "TRTL",
    "12211": "RYO",
...
function Coin(data){
    ...
    this.BlockTemplate = function(template) {
        ...
        const found_reserved_offset_template = blob.indexOf(extra_nonce_template_hex);
        ...
        if (found_reserved_offset_template !== -1) {
            ...
            if (is_mm) {
                ...
                if (found_reserved_offset != template.reserved_offset) {
                    console.error("INTERNAL ERROR: Found reserved offset " + found_reserved_offset + " do not match " + template.reserved_offset + " reported by daemon in block " + ": " + this.buffer.toString('hex'));
                }
                this.reserved_offset = found_reserved_offset;
            }
        } else {
            console.error("INTERNAL ERROR: Can not find reserved offset template '" + extra_nonce_template_hex + "' in block " + ": " + this.buffer.toString('hex'));
