...
function Coin(data){
    ...
    this.blobTypeStr = function(port) {
        switch (port) {
            case 12211: return "cryptonote_ryo"; // MSR
    ...
    this.get_miner_agent_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            if (majorv + minorv + minorv2 < 2) {
                return "Please update your xmr-node-proxy (" + agent + ") to version v0.2.1+ (from https://github.com/MoneroOcean/xmr-node-proxy repo)";
            }
    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        if (m = reXMRSTAK2.exec(agent)) {
            ...
            let minorv = parseInt(m[2]) * 100;
            let minorv2 = parseInt(m[3]);
            if (majorv + minorv + minorv2 < 201) {
                 return "Please update your xmr-node-proxy (" + agent + ") to version v0.2.1+ by doing 'cd xmr-node-proxy && ./update.sh' (or check https://github.com/MoneroOcean/xmr-node-proxy repo)";
            }
        }
        return false;
    };
