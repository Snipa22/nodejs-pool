...
function Coin(data){
    ...
    this.slowHashAsync = function(convertedBlob, blockTemplate, cb) {
        return cb(slowHash(convertedBlob, blockTemplate)); // !!!
        let jsonInput;
        switch (blockTemplate.port) {
            case 13102: 
            case 18081:
	    case 19994:
            ...
            case 48782:
                ...
                break;
            case 11181:
                return cb(slowHash(convertedBlob, blockTemplate)); // AEON K12 is too fast
            default:
        ...
        socket.on("end", function () {
            clearTimeout(timer);
                timer = null;
            try {
                const jsonOutput = JSON.parse(message);
                if (!("result" in jsonOutput)) return return_cb(null);
                return return_cb(Buffer.from(jsonOutput.result, 'hex'));
            } catch (e) {
                return return_cb(null);
            }
        });
