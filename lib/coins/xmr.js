...
if (global.config.verify_shares_host) global.config.verify_shares_host.forEach(function(verify_shares_host, index) {
    shareVerifyQueue[index] = async.queue(function (task, queueCB) {
        ...
        let timer = setTimeout(function() {
            socket.destroy();
            if (shareVerifyQueueErrorTime[index] && Date.now() - shareVerifyQueueErrorTime[index] < 10*1000) {
...
function Coin(data){
    ...
    this.slowHashAsync = function(convertedBlob, blockTemplate, miner_address, cb) {
        ...
        shareVerifyQueue.forEach(function(queue_obj, index) {
            if (shareVerifyQueueErrorTime[index] && time_now - shareVerifyQueueErrorTime[index] < 5*60*1000) return;
            if (min_queue_size === null || queue_obj.length() < min_queue_size) {
                best_index     = index;
                min_queue_size = queue_obj.length();
            }
        });
