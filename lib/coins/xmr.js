...
function Coin(data) {
    ...
    this.coinDevAddress =
        '44AFFq5kSiGBoZ4NMDwYtN18obc8AemS33DBLWs3H7otXft3XjrpDtQGv7SqSsaBYBb98uNbr2VBBEt7f2wfn3RVGQBEP3A'; // Monero Developers Address
    this.poolDevAddress =
        '499fS1Phq64hGeqV8p2AfXbf6Ax7gP6FybcMJq6Wbvg8Hw6xms8tCmdYpPsTLSaTNuLEtW4kF2DDiWCFcw4u7wSvFD8wFWE'; // MoneroOcean Address
    ...
    this.getPortBlockHeaderByID = function (port, blockId, callback) {
        if (port == 11898) {
            global.support.rpcPortDaemon2(port, 'block/' + blockId, null, function (err, body) {
                if (body) {
                    return callback(null, body);
                } else {
                    console.error(JSON.stringify(body));
                    return callback(err || true, body);
                }
            });
        } else if (port == 8766) {
            let _this = this;
            global.support.rpcPortDaemon2(
                port,
                '',
                { method: 'getblockhash', params: [blockId] },
                function (err, body) {
                    if (!body || !body.result) {
                        console.error(JSON.stringify(body));
    ...
    this.getPortAnyBlockHeaderByHash = function (port, blockHash, is_our_block, callback) {
        ...
        if (port == 11898) {
            global.support.rpcPortDaemon2(port, 'block/' + blockHash, null, function (err, body) {
            ...
            global.support.rpcPortDaemon2(port, '', { method: 'getblock', params: [blockHash] }, function (err, body) {
                if (!body || !body.result) {
                    console.error(JSON.stringify(body));
                    return callback(true, body);
                }
            ...
            global.support.rpcPortDaemon(port, 'getblockheaderbyhash', { hash: blockHash }, function (err, body) {
                if (
                    err ||
            ...
            global.support.rpcPortDaemon(port, 'getblock', { hash: blockHash }, function (err, body) {
                if (err || typeof body === 'undefined' || !body.hasOwnProperty('result')) {
                    console.error(JSON.stringify(body));
                    return callback(true, body);
                }
                ...
                if (/*port == 22023 ||*/ port == 33124 || port == 25182 || port == 13102 || port == 18181) {
                    // Loki / XtendCash / TUBE / Italocoin / XMC has reward as zero transaction
                ...
                const miner_tx_hash =
                    body.result.miner_tx_hash == ''
                        ? body.result.block_header.miner_tx_hash
                        : body.result.miner_tx_hash;

                if (is_our_block && body.result.hasOwnProperty('miner_tx_hash'))
                    global.support.rpcPortWalletShort(
                        port + 1,
                        ...
                        function (err, body2) {
                            if (
                                ...
                                body2.hasOwnProperty('error') ||
                                !body2.hasOwnProperty('result') ||
                                !body2.result.hasOwnProperty('transfer') ||
                                !body2.result.transfer.hasOwnProperty('amount')
                            ) {
                                console.error(
                                    port +
                                        ': block hash: ' +
                            ...
                            if (reward !== reward_check || reward == 0) {
                                if (port == 38081 && reward < reward_check /*&& reward != 0*/) {
                                    // MSR can have uncle block reward here
                                } else {
                                    console.error(
                                        port +
                                            ': block reward does not match wallet reward: ' +
                                            JSON.stringify(body) +
                                            '\n' +
                                            JSON.stringify(body2)
                                    );
    ...
    this.getPortLastBlockHeader = function (port, callback, no_error_report) {
        if (port == 11898) {
            global.support.rpcPortDaemon2(port, 'block/last', null, function (err, body) {
                if (err || !body || body.error) {
                    if (!no_error_report) console.error('Last block header invalid: ' + JSON.stringify(body), err);
                    return callback(true, body);
                }
            ...
            let _this = this;
            global.support.rpcPortDaemon2(port, '', { method: 'getbestblockhash' }, function (err, body) {
                if (!body || !body.result) {
                    console.error(JSON.stringify(body));
                    return callback(true, body);
                }
            ...
            global.support.rpcPortDaemon(port, 'getlastblockheader', [], function (err, body) {
                if (err || !body || !body.hasOwnProperty('result')) {
                    if (!no_error_report) console.error('Last block header invalid: ' + JSON.stringify(body) + err);
                    return callback(true, body);
                }
        ...
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon2(
                ...
                function (err, body) {
                    ...
                    return callback(
                        body && body.result
                            ? cnUtil.RavenBlockTemplate(body.result, global.config.pool['address_' + port.toString()])
                            : null
                    );
                }
            );
            ...
            global.support.rpcPortDaemon(
                ...
                'getblocktemplate',
                {
                    ...
                    wallet_address:
                        global.config.pool[
                            port === global.config.daemon.port ? 'address' : 'address_' + port.toString()
                        ],
                },
                function (err, body) {
                    if (err) {
    ...
    this.constructNewBlob = function (blockTemplate, params, port) {
        ...
        if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
            return cnUtil.construct_block_blob(
                blockTemplate,
            ...
            return cnUtil.constructNewRavenBlob(
                blockTemplate,
    ...
    this.BlockTemplate = function (template) {
        ...
        const blob = is_dero
            ? template.blockhashing_blob
            : is_mm
            ? template.parent_blocktemplate_blob
            : this.blocktemplate_blob;
        ...
        if (!is_dero) {
            const template_hex =
                template.port in mm_port_set && !is_mm ? extra_nonce_mm_template_hex : extra_nonce_template_hex;
    ...
    this.slowHashBuff = function (convertedBlob, blockTemplate, nonce, mixhash) {
        switch (blockTemplate.port) {
            case 8766:
                return multiHashing.kawpow(convertedBlob, Buffer.from(nonce, 'hex'), Buffer.from(mixhash, 'hex')); // RVN
            case 9231:
                return multiHashing.cryptonight(convertedBlob, 11); // XEQ
            case 11181:
                return multiHashing.k12(convertedBlob); // Aeon
            case 11898:
                return multiHashing.argon2(convertedBlob, 2); // TRTL
            case 12211:
                return multiHashing.cryptonight(convertedBlob, 11); // RYO
            case 13007:
                return multiHashing.cryptonight_pico(convertedBlob, 0); // Iridium
            case 16000:
                return multiHashing.cryptonight(convertedBlob, 11); // CCX
            case 17750:
                return multiHashing.cryptonight_heavy(convertedBlob, 1); // Haven
            case 18081:
                return multiHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), 0); // XMR
            case 18181:
                return multiHashing.cryptonight(convertedBlob, 0); // XMC
            case 18981:
                return multiHashing.cryptonight(convertedBlob, 14); // Graft
            case 19734:
                return multiHashing.cryptonight(convertedBlob, 13, blockTemplate.height); // SUMO
            case 19994:
                return multiHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), 2); // ArqMa
            case 11812:
                return multiHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), 3); // Scala
            case 20206:
                return multiHashing.astrobwt(convertedBlob, 0); // Dero
            case 34568:
                return multiHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), 17); // Wownero
            case 38081:
                return multiHashing.cryptonight(convertedBlob, 9); // MSR
            case 48782:
                return multiHashing.cryptonight(convertedBlob, 13, blockTemplate.height); // Lethean
            default:
    ...
    this.c29 = function (header, ring, port) {
        switch (port) {
            case 13102:
                return multiHashing.c29i(header, ring); // MoneroV
            case 19281:
                return multiHashing.c29v(header, ring); // MoneroV
            case 19950:
                return multiHashing.c29s(header, ring); // Swap
            case 25182:
                return multiHashing.c29b(header, ring); // TUBE
            case 33124:
                return multiHashing.c29s(header, ring); // XtendCash
            default:
