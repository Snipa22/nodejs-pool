...
function Coin(data){
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898 || port == 13007 || port == 48782 || port == 11181) {
            ...
            if (is_our_block && body.result.hasOwnProperty('miner_tx_hash')) global.support.rpcPortWallet(port+1, "get_transfer_by_txid", {"txid": body.result.block_header.miner_tx_hash}, function (body2) {
                if (typeof(body2) === 'undefined' || body2.hasOwnProperty('error') || !body2.hasOwnProperty('result') || !body2.result.hasOwnProperty('transfer') || !body2.result.transfer.hasOwnProperty('amount')) {
                    console.error(port + ": block hash: " + blockHash + ": txid " + body.result.block_header.miner_tx_hash + ": " + JSON.stringify(body2));
    ...
    this.cryptoNight = function(convertedBlob, blockTemplate) {
	    ...
	    case 19994: return multiHashing.cryptonight_pico(convertedBlob, 0);						// ArqMa
