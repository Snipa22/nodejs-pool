...
function Coin(data){
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        if      ("cn/r" in algos_perf)          coin_perf[""]     = algos_perf["cn/r"];
        else if ("cn/4" in algos_perf)          coin_perf[""]     = algos_perf["cn/4"];

        if (!("" in coin_perf)) return "algo_perf set must include cn, cn/1, cn/2, cn/r or cn/wow hashrate";
