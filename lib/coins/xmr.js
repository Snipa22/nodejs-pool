...
function Coin(data){
    ...
    this.getCOINS = function() {
        return [ "GRFT", "LTHN", "RYO", "LOKI", "TUBE", "XHV", "AEON", "MSR", "XTL" ];
    }
    ...
    this.get_miner_agent_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            let majorv = parseInt(m[1]) * 10000;
            let minorv = parseInt(m[2]) * 100;
            if (majorv + minorv < 20300) {
                return "Please update your xmr-stak miner (" + agent + ") to v2.5.0+ (and use monero in config)";
            }
            ...
            if (majorv + minorv + minorv2 < 2) {
                return "Please update your xmr-node-proxy (" + agent + ") to version v0.3.2+ (from https://github.com/MoneroOcean/xmr-node-proxy repo)";
            }
        } else if (m = reCCMINER.exec(agent)) {
            let majorv = parseInt(m[1]) * 10000;
            let minorv = parseInt(m[2]) * 100;
            if (majorv + minorv < 30000) {
