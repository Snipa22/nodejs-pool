...
function calcErgReward(block_tx) {
    let reward = 0;
    block_tx.forEach(function(tx) {
        if (tx.dataInputs.length) return;
        reward += tx.outputs[tx.outputs.length-1].value;
    });
    return reward;
}
...
function Coin(data){
    ...
    this.getPortBlockHeaderByID = function(port, blockId, callback){
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon2(port, 'blocks/at/' + blockId, null, function (body) {
                ...
                return _this.getPortAnyBlockHeaderByHash(port, body[0], false, callback);
            });
        } else {
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon2(port, 'blocks/' + blockHash, null, function (body) {
                ...
                body.header.reward = calcErgReward(body.blockTransactions.transactions);
    ...
    this.slowHashBuff = function(convertedBlob, blockTemplate, nonce, mixhash) {
        switch (blockTemplate.port) {
            ...
            case 9053:  return multiHashing.autolykos2_hash(convertedBlob, blockTemplate.height);                       // ERG
