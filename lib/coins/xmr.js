...
function Coin(data){
    ...
    this.constructNewBlob = function(blockTemplate, nonce, port, cycle, mixhash) {
        const blob_type_num = this.portBlobType(port, blockTemplate[0]);
        if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
            return cnUtil.construct_block_blob(blockTemplate, bignum(nonce, 10).toBuffer({endian: 'little', size: 4}), port, cycle);
        } else if (global.coinFuncs.blobTypeDero(blob_type_num)) {
            return cnUtil.constructNewDeroBlob(blockTemplate, new Buffer(nonce, 'hex'));
        } else if (global.coinFuncs.blobTypeRaven(blob_type_num)) {
            return cnUtil.constructNewRavenBlob(blockTemplate, bignum(nonce, 16).toBuffer({endian: 'little', size: 8}), bignum(mixhash, 16).toBuffer({endian: 'little', size: 32}));
        } else {
            return cnUtil.construct_block_blob(blockTemplate, new Buffer(nonce, 'hex'), blob_type_num);
        }
    };
