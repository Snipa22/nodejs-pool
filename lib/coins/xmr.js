    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        if (m = reXMRSTAK2.exec(agent)) {
            ...
            if (majorv + minorv + minorv2 < 103) {
                 return "Please update your xmr-node-proxy (" + agent + ") to version v0.1.3+ by doing 'cd xmr-node-proxy && git pull && npm update' (or check https://github.com/MoneroOcean/xmr-node-proxy repo)";
            }
        }
        return false;
    };
