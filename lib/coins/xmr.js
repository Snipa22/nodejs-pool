...
function Coin(data){
            ...
            if (is_our_block && body.result.hasOwnProperty('miner_tx_hash')) global.support.rpcPortWallet(port+1, "get_transfer_by_txid", {"txid": miner_tx_hash}, function (body2) {
                ...
                if (reward !== reward_check || reward == 0) {
                    if (port == 38081 && reward < reward_check/* && reward != 0*/) { // MSR can have uncle block reward here
                        ...
                        console.error("Port " + port + " block reward does not match wallet reward: " + JSON.stringify(body) + "\n" + JSON.stringify(body2));
                        return callback(true, body);
                    }
                }
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        if      ("cn-pico"      in algos_perf)  coin_perf["ARQ"] = coin_perf["IRD"] = algos_perf["cn-pico"];
        else if ("cn-pico/trtl" in algos_perf)  coin_perf["ARQ"] = coin_perf["IRD"] = algos_perf["cn-pico/trtl"];

        if      ("c29s" in algos_perf)          coin_perf["XTNC"] = algos_perf["c29s"];
    ...
    this.algoCheck = function(algos) {
        ...
        return "algo array must include at least one supported pool algo";
    }

    this.cryptoNight = function(convertedBlob, blockTemplate) {
        switch (blockTemplate.port) {
            case 11181: return blockTemplate.buffer[0] >= 8 ? multiHashing.k12(convertedBlob) :
                                                              multiHashing.cryptonight_light(convertedBlob, 1);         // Aeon
	    ...
	    case 13007: return multiHashing.cryptonight_pico(convertedBlob, 0);						// Iridium
            ...
            case 18081: return blockTemplate.buffer[0] >= 12 ? multiHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), 0) :
                                                               multiHashing.cryptonight(convertedBlob, 13, blockTemplate.height);	// XMR
	    ...
	    case 19994: return blockTemplate.buffer[0] >= 15 ? multiHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), 2) :
                                                               multiHashing.cryptonight_pico(convertedBlob, 0);		// ArqMa
            ...
            //case 33124: return multiHashing.cryptonight_pico(convertedBlob, 0);					// XtendCash???
            ...
            default:
		console.error("Unknown " + blockTemplate.port + " port for PoW type");
    ...
    this.blobTypeStr = function(port, version) {
        switch (port) {
            ...
            case 33124: return "cryptonote_loki"; // XtendCash
    ...
    this.get_miner_agent_warning_notification = function(agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            const majorv = parseInt(m[1]) * 10000;
            const minorv = parseInt(m[2]) * 100;
            if (majorv + minorv < 32000) {
                return "Please update your XMRig miner (" + agent + ") to v3.2.0+ to support new rx/0 Monero algo before November 30 Monero fork";
            }
            if (majorv + minorv >= 40000 && majorv + minorv < 42000) {
                return "Please update your XMRig miner (" + agent + ") to v4.2.0+ to support new rx/0 Monero algo before November 30 Monero fork";
            }
            ...
            const version = majorv + minorv + minorv2;
            if (version < 1400) {
                 return "Please update your xmr-node-proxy (" + agent + ") to version v0.14.0+ by doing 'cd xmr-node-proxy && ./update.sh' (or check https://github.com/MoneroOcean/xmr-node-proxy repo) to support new rx/0 Monero algo before November 30 Monero fork";
            }
        } else if (m = reSRB.exec(agent)) {
            const majorv = parseInt(m[1]) * 10000;
            const minorv = parseInt(m[2]) * 100;
