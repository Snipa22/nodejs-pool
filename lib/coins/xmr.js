...
function Coin(data){
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        // TRTL does not get getblock and XTL / LTHN / AEON have composite tx, GRFT does not have miner_tx_hash
        if (port == 11898 || port == 20189 || port == 48782 || port == 11181 || port == 18981) {
            ...
            if (typeof(body) === 'undefined' || !body.hasOwnProperty('result') || !body.result.hasOwnProperty('miner_tx_hash')) {
                console.error(JSON.stringify(body));
                return callback(true, body);
            }
            ...
            if (is_our_block) global.support.rpcPortWallet(port+1, "get_transfer_by_txid", {"txid": body.result.miner_tx_hash}, function (body2) {
