...
function Coin(data){
    ...
    this.constructMMParentBlockExtraNonce = function(parentTemplateBuffer, port, childTemplateBuffer) {
        console.log("MERGED MINING: constructMMParentBlockExtraNonce");
        return cnUtil.construct_mm_parent_block_extra_nonce(parentTemplateBuffer, this.portBlobType(port, parentTemplateBuffer[0]), childTemplateBuffer);
    };
...
console.log("DEBUG: BT4");
console.log(this.child_template_buffer);
console.log(this.child_template_buffer.toString('hex'));
console.log(JSON.stringify(this.child_template_buffer));
}
            this.blocktemplate_blob    = this.blocktemplate_blob.substring(0, this.reserved_offset * 2) +
                                         template.parent_extra_nonce_hex +
                                         this.blocktemplate_blob.substring(this.reserved_offset * 2 + template.parent_extra_nonce_hex.length);
        }
        ...
        if (!("prev_hash" in template)) {  // Get prev_hash from this.blocktemplate_blob
