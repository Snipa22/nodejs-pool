...
let shareVerifyQueue = async.queue(function (task, queueCB) {
    ...
    socket.connect(2222, "127.0.0.1", function () {
    ...
    socket.on("end", function () {
        ...
        timer = null;
        try {
            const jsonOutput = JSON.parse(buffall.toString());
            if (!("result" in jsonOutput)) return return_cb(null);
...
function Coin(data){
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898 || port == 13007 || port == 48782 || port == 11181 || port == 20206) {
            ...
            if (port == 22023 || port == 33124 || port == 24182 || port == 13102) { // Loki / XtendCash / TUBE / Italocoin has reward as zero transaction
    ...
    this.blobTypeGrin = function(blob_type_num) { return blob_type_num == 8 || blob_type_num == 9; }

    this.blobTypeDero = function(blob_type_num) { return blob_type_num == 10; }
    ...
    this.slowHashAsync = function(convertedBlob, blockTemplate, cb) {
        return cb(this.slowHash(convertedBlob, blockTemplate)); // !!!
        let jsonInput;
        switch (blockTemplate.port) {
            case 13102:
            case 18081:
        ...
        return shareVerifyQueue.unshift({"jsonInput": jsonInput, "cb": cb});
    }
