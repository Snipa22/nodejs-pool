...
function get_coin2port(port2coin) {
    let coin2port = {};
    for (port in port2coin) coin2port[port2coin[port]] = port;
    return coin2port;
}
const coin2port = get_coin2port(port2coin);
function get_coins(port2coin) {
    let coins = [];
    for (port in port2coin) coins.push(port2coin[port]);
    return coins;
}
const coins = get_coins(port2coin);
function get_mm_child_port_set(mm_port_set) {
    let mm_child_port_set = {};
    for (port in mm_port_set) {
        if (!(port in mm_child_port_set[port])) mm_child_port_set[port] = {};
...
function Coin(data){
    ...
    this.BlockTemplate = function(template, activeBlockTemplates) {
        ...
        const is_mm = port in mm_port_set;
        ...
        const child_coin = port2coin[mm_port_set[port]];
        if (is_mm && child_coin in activeBlockTemplates) {
            ...
            this.buffer = cnUtil.construct_mm_parent_block_blob(
                this.buffer, this.portBlobType(template.port, this.buffer[0]), this.child_template.buffer
            );
        }
