...
function Coin(data){
    ...
    this.BlockTemplate = function(template) {
        ...
        if (is_mm) {
            if (this.buffer[this.reserved_offset - 2] != 2 || this.buffer[this.reserved_offset - 1] != 17) {
                console.error("INTERNAL ERROR: Parent block after serialization changed reserved_offset " + this.reserved_offset + ": " + this.buffer.toString('hex'));
            }
        }
