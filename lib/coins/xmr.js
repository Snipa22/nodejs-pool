...
const xhvUtil = require('haven-nodejs-wrapper');
...
function Coin(data){
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898 || port == 13007 || port == 48782 || port == 11181 || port == 20206 || port == 16000) {
            ...
            if (port == 22023 || port == 33124 || port == 24182 || port == 13102 || port == 18181) { // Loki / XtendCash / TUBE / Italocoin / XMC has reward as zero transaction
    ...
    this.blobTypeGrin = function(blob_type_num) { return blob_type_num == 8 || blob_type_num == 9 || blob_type_num == 10; }

    this.c29ProofSize = function(blob_type_num) { return blob_type_num == 10 ? 40 : 32; }

    this.nonceSize = function(blob_type_num) { return blob_type_num == 7 ? 8 : 4; } 

    this.blobTypeDero = function(blob_type_num) { return blob_type_num == 100; }

    this.blobTypeHaven = function(blob_type_num) { return false /*blob_type_num == 101*/; }
    ...
    this.convertBlob = function(blobBuffer, port){
        ...
        let blob;
        try {
            blob = this.blobTypeHaven(blob_type_num) ? xhvUtil.convert_blob(blobBuffer) : cnUtil.convert_blob(blobBuffer, blob_type_num);
        } catch (e) {
    ...
    this.constructNewBlob = function(blockTemplate, NonceBuffer, port, ring){
        ...
        if (this.blobTypeDero(blob_type_num)) {
            NonceBuffer.copy(blockTemplate, 39, 0, 4);
            return blockTemplate;
        } else if (this.blobTypeHaven(blob_type_num)) {
          return xhvUtil.construct_block_blob(blockTemplate, NonceBuffer);
        } else {
    ...
    this.getBlockID = function(blockBuffer, port){
        const blob_type_num = this.portBlobType(port, blobBuffer[0]);
        return this.blobTypeHaven(blob_type_num) ? xhvUtil.get_block_id(blockBuffer) : cnUtil.get_block_id(blockBuffer, blob_type_num);
    };
    ...
    this.BlockTemplate = function(template) {
        ...
        const blob = is_dero ? template.blockhashing_blob : (is_mm ? template.parent_blocktemplate_blob : template.blocktemplate_blob);
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        if      ("rx/0" in algos_perf)          coin_perf[""]     = coin_perf["LOKI"] = coin_perf["XTA"] = algos_perf["rx/0"];
        ...
        if      ("cn/ccx" in algos_perf)        coin_perf["CCX"] = algos_perf["cn/ccx"];
    ...
    this.slowHashBuff = function(convertedBlob, blockTemplate) {
	    ...
	    case 13007: return multiHashing.cryptonight_pico(convertedBlob, 0);						// Iridium
            case 13102: return multiHashing.randomx(convertedBlob, Buffer.from(blockTemplate.seed_hash, 'hex'), 0);	// Italocoin
            case 16000: return multiHashing.cryptonight(convertedBlob, 17, blockTemplate.height);			// Conceal
    ...
    this.slowHash = function(convertedBlob, blockTemplate) {
        return this.slowHashBuff(convertedBlob, blockTemplate).toString("hex");
    }
    ...
    this.c29 = function(header, ring, port) {
        switch (port) {
            ...
            case 24182: return multiHashing.c29b(header, ring);	// TUBE
    ...
    this.blobTypeStr = function(port, version) {
        switch (port) {
            ...
            //case 24182: return "cryptonote_tube"; // TUBE
