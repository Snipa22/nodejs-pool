...
function Coin(data){
    ...
    console.log(global.config.pool_id);
    console.log((global.config.pool_id % (1<<10)));
    console.log(((global.config.pool_id % (1<<10)) << 22));
console.log((process.pid % (1<<22)));
console.log(((global.config.pool_id % (1<<10)) << 22) + (process.pid % (1<<22)));
    instanceId.writeUInt32LE( ((global.config.pool_id % (1<<10)) << 22) + (process.pid % (1<<22)) );
    ...
    this.getPortLastBlockHeader = function(port, callback, no_error_report) {
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon2(port, '', { method: 'getbestblockhash' }, function (body) {
                if (typeof(body) === 'object' && body.result) {
                    body.hash = body.result;
                    return callback(null, body);
                } else {
                    if (!no_error_report) console.error("Last block header invalid: " + JSON.stringify(body));
                    return callback(true, body);
    ...
    this.convertAlgosToCoinPerf = function(algos_perf) {
        ...
        if      ("kawpow/rvn" in algos_perf)    coin_perf["RVN"]  = algos_perf["kawpow/rvn"];
    ...
    this.slowHashBuff = function(convertedBlob, blockTemplate, job) {
        switch (blockTemplate.port) {
            case 8766:  return multiHashing.kawpow(blockTemplate.height, convertedBlob, Buffer.from(job.target_hex, 'hex')); // RVN
