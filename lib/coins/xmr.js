...
function calcEthReward(txs) {
    let fee = 0;
    txs.forEach(function(tx) {
        fee += parseInt(Number(tx.gas), 10) * parseInt(Number(tx.gasPrice), 10);
    });
    return 2 + fee / 1000000000000000000;
}
...
function Coin(data){
    ...
    this.getPortBlockHeaderByID = function(port, blockId, callback){
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon2(port, '', { method: 'eth_getBlockByNumber', params: [ "0x" + blockId.toString(16), true ] }, function (body) {
                if (!body || !body.result) {
                    console.error(JSON.stringify(body));
                    return callback(true, body);
                }
                body.result.reward = calcEthReward(body.result.transactions);
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon2(port, '', { jsonrpc: "2.0", method: 'eth_getBlockByHash', params: [ "0x" + blockHash, true ] }, function (body) {
                if (!body || !body.result) {
                    console.error(JSON.stringify(body));
                    return callback(true, body);
                }
