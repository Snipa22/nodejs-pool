...
function get_coin2port(port2coin) {
    let coin2port = {};
    for (let port in port2coin) coin2port[port2coin[port]] = port;
    return coin2port;
}
const coin2port = get_coin2port(port2coin);
function get_coins(port2coin) {
    let coins = [];
    for (let port in port2coin) coins.push(port2coin[port]);
    return coins;
}
...
function get_mm_child_port_set(mm_port_set) {
    ...
    for (let port in mm_port_set) {
        if (!(port in mm_child_port_set)) mm_child_port_set[port] = {};
        mm_child_port_set[port][mm_port_set[port]] = 1;
    }
...
function Coin(data){
    ...
    this.getPortBlockTemplate = function(port, callback){
        global.support.rpcPortDaemon(port, 'getblocktemplate', {
            reserve_size: port in mm_port_set ? mm_nonce_size + 16 + 1 : 16 + 1,
    ...
    this.BlockTemplate = function(template, activeBlockTemplates) {
        ...
        if (!("prev_hash" in template)) console.error("NOT DEFINED PREV_HASH FOR " + template.port);
        ...
        this.idHash = crypto.createHash('md5').update(this.blocktemplate_blob).digest('hex');

        const is_mm = this.port in mm_port_set;

        // Set this.reserveOffset to the byte location of the reserved offset.
        this.reserveOffset = template.reserved_offset + (is_mm ? mm_nonce_size : 0);
        // Set this.buffer to the binary decoded version of the BT blocktemplate_blob.
        this.buffer = new Buffer(this.blocktemplate_blob, 'hex');

        const child_coin = port2coin[mm_port_set[this.port]];
        if (is_mm && child_coin in activeBlockTemplates) {
            this.child_template = activeBlockTemplates[child_coin];
            this.buffer = cnUtil.construct_mm_parent_block_blob(
                this.buffer, global.coinFuncs.portBlobType(this.port, this.buffer[0]), this.child_template.buffer
            );
        }
        ...
        this.nextBlob = function () {
            ...
            // Convert the blocktemplate_blob into something hashable.
            return global.coinFuncs.convertBlob(this.buffer, this.port).toString('hex');
        };
        // Make it so you can get the raw block blocktemplate_blob out.
        this.nextBlobWithChildNonce = function () {
            // Write a 32 bit integer, big-endian style to the 0 byte of the reserve offset.
            this.buffer.writeUInt32BE(++this.extraNonce, this.reserveOffset);
            // Don't convert the blocktemplate_blob to something hashable.  You bad.
            return this.buffer.toString('hex');
        };
    };

    this.getCOINS          = function() { return coins; }
    this.getPORT2COIN      = function() { return port2coin; }
    this.getMM_PORTS       = function() { return mm_port_set; }
    this.getMM_CHILD_PORTS = function() { return get_mm_child_port_set; }
