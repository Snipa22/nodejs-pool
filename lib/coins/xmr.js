...
function Coin(data){
    ...
    this.getPortBlockHeaderByHash = function(port, blockHash, callback){
        if (port != 11898) global.support.rpcPortDaemon(port, 'getblock', {"hash": blockHash}, function (body) {
            if (typeof(body) === 'undefined' || !body.hasOwnProperty('result')) {
                console.error(JSON.stringify(body));
                return callback(true, body);
            }

            /*global.support.rpcPortWallet(port+1, "get_transfer_by_txid", {"txid": body.result.miner_tx_hash}, function (body2) {
                if (typeof(body2) === 'undefined' || body2.hasOwnProperty('error') || !body2.hasOwnProperty('result') || !body2.result.hasOwnProperty('transfer') || !body2.result.transfer.hasOwnProperty('amount')) {
                    console.error(port + ": " + JSON.stringify(body) + "\n" + JSON.stringify(body2));
                    return callback(true, body2);
                }*/
                //body.result.block_header.reward = body2.result.transfer.amount;

		// Stellite / Lethean / Aeon / Loki / Saronite have composite based miner_tx
                if (port != 11898 && port != 20189 && port != 48782 && port != 11181 && port != 22023 && port != 31014) {
                    const blockJson = JSON.parse(body.result.json);
                    body.result.block_header.reward2 = 0;

                    const minerTx = blockJson.miner_tx;

                    for (var i=0; i<minerTx.vout.length; i++) {
                        if (minerTx.vout[i].amount > body.result.block_header.reward2) {
                            body.result.block_header.reward2 = minerTx.vout[i].amount;
                        }
                    }
                    body.result.block_header.reward = body.result.block_header.reward2;

                } else if (port == 22023 || port == 31014) { // Loki / Saronite has reward as zero transaction
                    const blockJson = JSON.parse(body.result.json);
                    body.result.block_header.reward2 = 0;

                    const minerTx = blockJson.miner_tx;

                    body.result.block_header.reward2 = minerTx.vout[0].amount;
                    body.result.block_header.reward = body.result.block_header.reward2;
                }

                /*if (body.result.block_header.reward !== body.result.block_header.reward2) {
                    console.error("Block reward does not match wallet reward!!!");
                    return callback(true, body);
                }
                 */
                return callback(null, body.result.block_header);
        ...
        }); else global.support.rpcPortDaemon(port, 'getblockheaderbyhash', {"hash": blockHash}, function (body) { // TRTL is very special
            if (typeof(body) === 'undefined' || !body.hasOwnProperty('result')) {
                console.error(JSON.stringify(body));
                return callback(true, body);
            }
    ...
    this.cryptoNight = function(convertedBlob, port) {
        switch (port) {
            ...
            case 31014: return multiHashing.cryptonight_heavy(convertedBlob, 0); // Saronite
    ...
    this.algoShortTypeStr = function(port, version) {
        switch (port) {
            ...
            case 31014: return "cn-heavy/0";    // Saronite
