...
function Coin(data){
    ...
    this.slowHashAsync = function(convertedBlob, blockTemplate, miner_address, cb) {
        ...
        do {
            ++ verify_share_host_index;
            if (verify_share_host_index >= host_count) verify_share_host_index = 0;
        } while (--max_attempts && shareVerifyQueueErrorTime[verify_share_host_index] && time_now - shareVerifyQueueErrorTime[verify_share_host_index] < 5*60*1000);
        return shareVerifyQueue[verify_share_host_index].unshift({ jsonInput: jsonInput, cb: cb, time: time_now, miner_address: miner_address });
    }
