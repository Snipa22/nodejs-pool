...
function Coin(data){
    ...
    this.getPortBlockHeaderByID = function(port, blockId, callback){
        ...
        } else if (port == 8766 || port == 9998 || port == 10225 || port == 19001) {
    ...
    this.getPortAnyBlockHeaderByHash = function(port, blockHash, is_our_block, callback){
        ...
        if (port == 11898) {
            ...
            global.support.rpcPortDaemon2(port, '', { method: 'getblock', params: [ blockHash ] }, function (body) {
                if (!body || !body.result || !(block.result.tx instanceof Array) || block.result.tx.length < 1) {
                    console.error("getPortBlockHeaderByHash(" + port + ", " + blockHash + "): " + JSON.stringify(body));
                    return callback(true, body);
                }
                global.support.rpcPortDaemon2(port, '', { method: 'getrawtransaction', params: [ block.result.tx[0], true ] }, function (body2) {
                    if (!body2 || !body2.result || !(body2.result.vout instanceof Array)) {
                        console.error("getPortBlockHeaderByHash(" + port + ", " + blockHash + "): " + JSON.stringify(body2));
                        return callback(true, body2);
                    }
                    const wallet_address = global.config.pool["address_" + port.toString()];
                    body.result.reward = 0;
                    for (vout of body2.result.vout) {
                      if (vout.addresses && vout.addresses instanceof Array && vout.addresses.length == 1 && vout.addresses[0] == wallet_address)
                        body.result.reward += vout.valueSat;
                    }
    ...
    this.getPortBlockTemplate = function(port, callback) {
		...
		method: 'getblocktemplate',
                ...
                if (body && body.result) switch (parseInt(port)) {
                  ...
                  case 10225: return callback(cnUtil.RtmBlockTemplate(body.result, global.config.pool["address_" + port.toString()]));
                  case 19001: return callback(cnUtil.RavenBlockTemplate(body.result, global.config.pool["address_" + port.toString()]));
...
//        if      ("rx/wow" in algos_perf)        coin_perf["WOW"]  = algos_perf["rx/wow"];
        ...
        if      ("kawpow" in algos_perf)        coin_perf["RVN"]  = coin_perf["XNA"] = algos_perf["kawpow"];
