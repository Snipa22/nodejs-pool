...
let paymentQueue = async.queue(function (paymentDetails, callback) {
    ...
    global.support.rpcWallet('getbalance', [], function (body) {
        if (body.result) {
            ...
            if (body.result.unlocked_balance < amountToPay) {
                console.error("Wallet only has " + body.result.unlocked_balance + " unlocked balance, can't pay " + amountToPay + " worth of ITNS. Retrying in " + timerRetry + " minutes!");
    ...
    global.support.rpcWallet(transferFunc, paymentDetails, function (body) {
        ...
        if (paymentDetails.hasOwnProperty('payment_id')) {
            console.log("Payment made to " + paymentDetails.destinations[0].address + " with PaymentID: " + paymentDetails.payment_id + " For: " + global.support.coinToDecimal(paymentDetails.destinations[0].amount) + " ITNS with a " + global.support.coinToDecimal(body.result.fee) + " ITNS Mining Fee");
                    ...
                    while (paymentDestinations.length > 0) {
                        ...
                        paymentQueue.push(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (body.fee && body.fee > 10) {
                                ...
                                if (global.config.general.discordPayoutAnnounce === true) {
                                    global.support.poolBotMsg("Pool paid out: " + global.support.coinToDecimal(totalAmount)  + " ITNS to " + paymentDetails.destinations.length + " miners");
                                }
                            } else {
                                console.error("Unknown error from the wallet.");
                            }
