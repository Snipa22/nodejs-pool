        ...
        async.eachSeries(rows, function(row, next) {
            ...
            global.mysql.query("SELECT payout_threshold FROM users WHERE username = ?", [payee.id]).then(function (userRow) {
                ...
                if (payee.amount >= threshold) {
                    payee.setFeeAmount();
                    if (payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.address.length !== 106) {
                    ...
                    } else if (payee.paymentID === null && payee.amount !== 0 && payee.amount > 0 && payee.address.length === 106 && (payee.amount >= global.support.decimalToCoin(global.config.payout.exchangeMin) || (payee.amount > threshold && custom_threshold))) {
                        ...
                        console.error("Unsupported payment to " + payee.id + " was found");
                    }
                }
                return next();
            }).catch(function (error) {
