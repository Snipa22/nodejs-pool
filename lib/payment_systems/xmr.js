                        ...
                        paymentQueue.push(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (body.fee && body.fee > 10) {
                                ...
                                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                                    ...
                                    if (err) {
                                       console.error("Can't do INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (0, null, null," + totalAmount + ", " + body.tx_hash.match(hexChars)[0] + ", " + global.config.payout.mixIn + ", " + body.fee + ", " + paymentDetails.destinations.length + ")");
                                       full_stop(err);
                                       return;
                                    }
                                    paymentDetails.destinations.forEach(function (payeeItem) {
