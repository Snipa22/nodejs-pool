...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (body.fee && body.fee > 10) {
                console.log("[*] Successful payment to " + identifier + " of " + global.support.coinToDecimal(amount) + " XMR (fee " + global.support.coinToDecimal(fee) + " - " + global.support.coinToDecimal(body.fee) + " = " + global.support.coinToDecimal(fee - body.fee) + ") with tx_hash " + body.tx_hash.match(hexChars)[0] + " with tx_key " + body.tx_key);
