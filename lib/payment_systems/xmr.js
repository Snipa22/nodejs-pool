...
function Payee(amount, address, paymentID, bitcoin) {
    ...
    this.trackPayment = function () {
        ...
        global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_email IS true limit 1", [this.id]).then(function (rows) {
            if (rows.length === 0) return;
            // toAddress, subject, body
            let emailData = {
                payment_amount: this.amount - this.fee,
                amount: this.amount,
                fee: this.fee,
            ...
            global.support.sendEmail(rows[0].email,
                sprintf("Your %(payment_amount)s payment was just performed", emailData),
                sprintf(
                    "Your payment of %(payment_amount)s (with fee %(fee)s) was just performed and total due was decreased by %(amount)s.\n" +
