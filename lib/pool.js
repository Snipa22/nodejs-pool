...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_hr) {
    ...
    this.selectBestAlgo = function() {
        ...
        ALGOS.forEach(function(algo) {
            if (!(global.coinFuncs.algoTypeStr(global.config.daemon[activePortAlgo(algo)]) in this.algos)) return;
            if (!(global.coinFuncs.algoShortTypeStr(global.config.daemon[activePortAlgo(algo)]) in this.algos)) return;
