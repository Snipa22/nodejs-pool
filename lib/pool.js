...
function setNewBlockTemplate(template) {
    ...
    if (cluster.isMaster) {
        ...
        console.log('[*] New ' + algo_str + 'block to mine at ' + template.height + ' height with ' + template.difficulty + ' difficulty and ' + template.port + ' port');
    } else {
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_hr) {
    ...
    this.selectBestAlgo = function() {
        ...
        ALGOS.forEach(function(algo) {
            if (!(global.coinFuncs.algoTypeStr(global.config.daemon["activePort" + algo]) in miner.algos)) return;
            if (!(global.coinFuncs.algoShortTypeStr(global.config.daemon["activePort" + algo]) in miner.algos)) return;
...
if (cluster.isMaster) {
    ...
    if (!global.config.daemon.activePort) {
        ...
        ALGOS.forEach(function(algo) {
            if (global.config.daemon["activePort" + algo]) {
