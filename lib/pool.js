...
// Share times of miners (payout:identifier) that never submitted any good share
...
function get_miner_agent_notification(agent) {
    let m;
    if (m = reXMRig.exec(agent)) {
        let majorv = parseInt(m[1]);
        let minorv = parseInt(m[2]);
        if (majorv < 2 || (majorv === 2 && minorv < 5)) {
            return "Please update your XMRig miner (" + agent + ") to v2.5.0+ before hardfork on April 6";
        }
    } else if (m = reXMRSTAK.exec(agent)) {
        let majorv = parseInt(m[1]);
        let minorv = parseInt(m[2]);
        if (majorv < 2 || (majorv === 2 && minorv < 3)) {
            return "Please update your xmr-stak miner (" + agent + ") to v2.3.0+ before hardfork on April 6 (and use monero7 in config)";
        }
        ...
        if (majorv === 0 && minorv === 0 && minorv2 === 1) {
            return "Please update your xmr-node-proxy (" + agent + ") to version v0.0.2+ before hardfork on April 6 (check https://github.com/MoneroOcean/xmr-node-proxy repo)";
        }
    } else if (m = reCCMINER.exec(agent)) {
        let majorv = parseInt(m[1]);
        let minorv = parseInt(m[2]);
        if (majorv < 3) {
            return "Please update ccminer-cryptonight miner to v3.00+ before hardfork on April 6";
        }
    }
    return false;
}
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        case 'login':
            ...
            let time_now = Date.now();
            let miner_id = miner.payout + ":" + miner.identifier;
            if (miner_id in badMinerLastShareTime) {
                if (time_now - badMinerLastShareTime[miner_id] < 10*60*1000) {
                    sendReply("You miner " + miner.identifier + " is currently banned for submitting wrong result (up to 10 minutes)");
                    return;
                } else {
                    delete badMinerLastShareTime[miner_id];
                }
            }
            let miner_agent_notification = params.agent ? get_miner_agent_notification(params.agent) : false;
            let miner_notification = miner_agent_notification ? miner_agent_notification : get_miner_notification(miner.payout);
            ...
            if (global.config.pool.trustedMiners) {
                if (shareAccepted) {
                    ...
                    if (miner.validShares == 0) {
                        badMinerLastShareTime[miner.payout + ":" + miner.identifier] = Date.now();
                    }
                }
            }
