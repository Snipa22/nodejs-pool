...
function handleMinerData(socket, method, params, ip, portData, sendReply, sendFinalReply, pushMessage) {
    ...
    switch (method) {
        case 'login':
            ...
            const is_prev_main_algo_miner = !global.coinFuncs.algoMainCheck(miner.algos) && global.coinFuncs.algoPrevMainCheck(miner.algos);
            const miner_agent_not_supported_notification = is_prev_main_algo_miner ? global.coinFuncs.get_miner_agent_not_supported_notification(params.agent) : false;
            const miner_agent_notification = is_prev_main_algo_miner ? global.coinFuncs.get_miner_agent_warning_notification(params.agent) : false;
            const miner_notification = miner_agent_not_supported_notification ? miner_agent_not_supported_notification :
                                       (miner_agent_notification ? miner_agent_notification : get_miner_notification(miner.payout));
