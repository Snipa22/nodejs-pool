...
function get_hash_factor(coin) {
    return coin === "" ? 1.0 : global.config.daemon["coinHashFactor" + coin];
}
            ...
            if (coin !== "") {
                global.support.getCoinHashFactor(coin, function (newCoinHashFactor) {
                    if (newCoinHashFactor === null) {
                        ...
                        if (newActivePort == 0 || newCoinHashFactor == 0) coinHashFactorUpdate(coin, 0);
                        else global.config.daemon["coinHashFactor" + coin] = newCoinHashFactor;
...
function coinHashFactorUpdate(coin, coinHashFactor) {
    if (coin === "") return;
    if (global.config.daemon["coinHashFactor" + coin] == 0 && coinHashFactor == 0) return;
    if (cluster.isMaster) {
...
function templateUpdate(coin, repeating) {
    ...
    if (activePort) global.coinFuncs.getPortLastBlockHeader(activePort, function (err, body) {
        ...
        if (err === null) {
            const coinHashFactor = get_hash_factor(coin);
...
function setNewCoinHashFactor(coin, coinHashFactor, check_height) {
    ...
    const isHashFactorChange = !(coin in lastCoinHashFactor) || !(coin in activeBlockTemplate) || coinHashFactor == 0 ||
                               (Math.abs(lastCoinHashFactor[coin] - coinHashFactor) / coinHashFactor > 0.05);

    const time_before = Date.now();

    if (isHashFactorChange) {
        debug(threadName + "Full BT update for coin " + coin + " with hash factor changed from " + lastCoinHashFactor[coin] + " to " + coinHashFactor);
        lastCoinHashFactor[coin] = coinHashFactor;
        ...
        debug(threadName + "Fast BT update for coin " + coin + " with the same " + coinHashFactor + " hash factor");
        if (check_height) {
            for (var [minerId, miner] of activeMiners) {
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    this.selectBestCoin = function() {
        if (miner.debugMiner) console.log(threadName + miner.logString + ": current coin is " + this.curr_coin);
        if (typeof(this.curr_coin) !== 'undefined' && this.curr_coin_time && get_hash_factor(this.curr_coin) &&
        ...
        COINS.forEach(function(coin) {
            ...
            const coinHashFactor = global.config.daemon["coinHashFactor" + coin];
            if (!coinHashFactor) {
