...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    this.selectBestCoin = function() {
        ...
        if (typeof(this.curr_coin) === 'undefined' || this.curr_coin != best_coin) {
            const curr_hash_factor = typeof(this.curr_coin) === 'undefined' ? 1 : lastCoinHashFactor[this.curr_coin];
            const factor = curr_hash_factor / lastCoinHashFactor[best_coin];
            if (factor != 1) {
                //if (factor > 10) factor = 10;
                //else if (factor < 0.1) factor = 0.1;
                if (factor < 0.1) console.error("!!! " + this.curr_coin + " to " + best_coin + " hash factor: " + factor + " (" +  curr_hash_factor + " / " + lastCoinHashFactor[best_coin] + ")");
