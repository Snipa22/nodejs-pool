...
function Miner(id, login, pass, ipAddress, startingDiff, pushMethodMessage, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    this.pushMethodMessage = pushMethodMessage;
    ...
    if (protoVersion === 1) {
        this.getCoinJob = function (coin, params) {
            ...
            if (!this.proxy) {
                ...
                if (blob_type_num == 8) this.cachedJob = {
                    pre_pow:    blob,
                    algo:       "cuckaroo",
                    edgebits:   29,
		    proofsize:  32,
        ...
        this.sendCoinJob = function(coin, params) {
            ...
            if (job === null) return;
            return this.protocol === "grin" ? pushMethodMessage("getjobtemplate", job) : this.pushMethodMessage('job', job);
        };
...
function processShare(miner, job, blockTemplate, params) {
    ...
    const resultHash    = params.result; // can be undefined for blob_type_num == 8 (and will not be used in submit_block since isTrustedShare = false)
...
function handleMinerData(socket, id, method, params, ip, portData, sendReply, sendReplyFinal, sendReplyMethod, pushMethodMessage) {
    switch (method) {
        case 'login': { // grin and default
            ...
            let miner = new Miner(
                minerId, params.login, params.pass, ip, difficulty, pushMethodMessage, 1, portData.portType, portData.port, params.agent,
        ...
        case 'getjobtemplate': { // grin only
            ...
            miner.heartbeat();
            sendReplyMethod("getjobtemplate", miner.getBestCoinJob());
            break;
        }
        ...
        case 'submit': { // grin and default
            ...
            const is_bad_nonce = job.blob_type_num == 8 ?
                                 (typeof params.nonce !== 'number') || (typeof params.pow !== 'array') || (params.pow.length != 32) :
            ...
            if (miner.proxy) {
                ...
                nonce_test = job.blob_type_num == 8 ? params.pow.join(':') + `_${params.poolNonce}_${params.workerNonce}`
                                                    : `${params.nonce}_${params.poolNonce}_${params.workerNonce}`;
            } else {
                nonce_test = job.blob_type_num == 8 ? params.pow.join(':') : params.nonce;
            }
...
if (cluster.isMaster) {
    ...
    async.each(global.config.ports, function (portData) {
        ...
        let handleMessage = function (socket, jsonData, pushMethodMessage) {
            if (!jsonData.id) {
            ...
            handleMinerData(socket, jsonData.id, jsonData.method, jsonData.params, socket.remoteAddress, portData, sendReply, sendReplyFinal, sendReplyMethod, pushMethodMessage);
        };
            ...
            socket.on('data', function (d) {
                ...
                if (dataBuffer.indexOf('\n') !== -1) {
                    ...
                    for (let i = 0; i < messages.length; i++) {
                        ...
                        handleMessage(socket, jsonData, pushMethodMessage);
                    }
