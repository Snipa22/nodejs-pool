...
// algo can be "", "Heavy", "Light"
function algoHashFactor(algo) { return "algoHashFactor" + algo; }
function activePortAlgo(algo) { return "activePort" + algo; }
...
function retargetMiners() {
    ...
    if (elapsed > 500) console.error("retargetMiners() consumed " + elapsed + " ms");
}
...
function checkAliveMiners() {
    ...
    const elapsed = Date.now() - time_before;
    if (elapsed > 500) console.error("checkAliveMiners() consumed " + elapsed + " ms");
}

// global.config.daemon[activePortAlgo(algo)] is only updated in master thread
function updateActivePort(algo) {
    global.support.getActivePort(algo, function (newActivePort) {
        const oldActivePort = global.config.daemon[activePortAlgo(algo)];
        if (newActivePort === null) {
            if (algo === "" && oldActivePort != global.config.daemon.port) {
                ...
                console.error("Error getting " + activePortAlgo(algo));
                ...
                global.support.getAlgoHashFactor(algo, function (newAlgoHashFactor) {
                    if (newAlgoHashFactor === null) {
                        ...
                        if (oldActivePort !== newActivePort) {
                            console.log("Changing " + activePortAlgo(algo) + " from " + oldActivePort + " to " + newActivePort);
                            global.config.daemon[activePortAlgo(algo)] = newActivePort;
                        }
                    }
                });
            } else if (oldActivePort !== newActivePort) {
                if (!(newActivePort in lastPortErrorTime) || Date.now() - lastPortErrorTime[newActivePort] > 30*1000) {
                    console.log("Changing " + activePortAlgo(algo) + " from " + oldActivePort + " to " + newActivePort);
                    global.config.daemon[activePortAlgo(algo)] = newActivePort;
                } else {
                    console.warn("Avoiding changing recently problem " + activePortAlgo(algo) + " from " + oldActivePort + " to " + newActivePort);
                }
            }
        }
    });
...
function templateUpdateReal(algo, activePort, algoHashFactor) {
    global.coinFuncs.getPortBlockTemplate(activePort, function (rpcResponse) {
        if (activePort !== global.config.daemon[activePortAlgo(algo)]) {
            console.log("Aborting " + activePort + " last block template request because " + activePortAlgo(algo) + " was already changed to " + global.config.daemon[activePortAlgo(algo)] + " port");
            return;
        }
        if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
            ...
            if (algo === "") {
                if (activePort != global.config.daemon.port) {
                    console.warn("Returning to " + global.config.daemon.port + " port.");
                    lastPortErrorTime[global.config.daemon.activePort] = Date.now();
                    global.config.daemon.activePort = global.config.daemon.port;
                }
            } else {
...
function templateUpdate(algo, repeating) {
    let activePort = global.config.daemon[activePortAlgo(algo)];
    global.coinFuncs.getPortLastBlockHeader(activePort, function (err, body) {
        if (activePort !== global.config.daemon[activePortAlgo(algo)]) {
            console.log("Aborting " + activePort + " last block header request because " + activePortAlgo(algo) + " was already changed to " + global.config.daemon[activePortAlgo(algo)] + " port");
        ...
        if (err === null) {
            ...
            console.error("Last block header request for " + global.config.daemon[activePortAlgo(algo)] + " port failed!");
            if (algo === "") {
                if (activePort != global.config.daemon.port) {
                    console.warn("Returning to " + global.config.daemon.port + " port.");
                    lastPortErrorTime[global.config.daemon.activePort] = Date.now();
...
function setNewAlgoHashFactor(algo, algoHashFactor, check_height) {
    ...
    if (algo !== "") global.config.daemon[algoHashFactor(algo)] = algoHashFactor;
    ...
    const elapsed = Date.now() - time_before;
    if (elapsed > 500) console.error("setNewAlgoHashFactor() consumed " + elapsed + " ms");
}
...
function setNewBlockTemplate(template) {
    ...
    if (algo in activeBlockTemplate) {
        ...
        if (!(algo in pastBlockTemplates)) pastBlockTemplates[algo] = global.support.circularBuffer(4);
        activeBlockTemplate[algo].timeOutdate = Date.now() + 4*1000;
        pastBlockTemplates[algo].enq(activeBlockTemplate[algo]);
    ...
    const elapsed = Date.now() - time_before;
    if (elapsed > 500) console.error("setNewBlockTemplate() consumed " + elapsed + " ms");
}
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_hr) {
    ...
    if (typeof(addressSplit[1]) !== 'undefined') {
        if (addressSplit[1].length === 64 && hexMatch.test(addressSplit[1])) {
    ...
    if (algos && algos_hr) {
        const status = this.setAlgos(algos, algos_hr);
        if (status != "") {
            this.error = status;
            this.valid_miner = false;
            return;
    ...
    this.setAlgos = function(algos, algos_hr) {
        if (algos && (algos instanceof Array) && (algos.includes("cn/1") || algos.includes("cryptonight/1"))) {
            this.algos = {};
            for (let algo in algos) this.algos[algo] = 1;
        } else {
            return "algo array should include cn/1 or cryptonight/1";
        }
        if (algos_hr && (algos_hr instanceof Object)) {
            this.algos_hr = {};
            for (let algo in algos_hr) {
                if (algo.includes("heavy")) this.algos_hr["Heavy"] = algos_hr[algo];
                else if (algo.includes("lite")) this.algos_hr["Light"] = algos_hr[algo];
                else this.algos_hr[""] = algos_hr[algo];
            }
            if (!this.algos_hr[""]) return "algo_hr set should include non heavy and lite hashrate";
        } else {
            return "algo_hr set should be present";
        }
        return "";
    };
    ...
    this.selectBestAlgo = function() {
        ...
        let algo = "";
        let algo_hr = this.algos_hr[""];
        if ("Heavy" in this.algos_hr && this.algos_hr["Heavy"] * global.config.daemon[algoHashFactor("Heavy")] > algo_hr) {
            algo = "Heavy";
            algo_hr = this.algos_hr[algo] * global.config.daemon[algoHashFactor(algo)];
        }
        if ("Light" in this.algos_hr && this.algos_hr["Light"] * global.config.daemon[algoHashFactor("Light")] > algo_hr) {
            algo = "Light";
        }
        return algo;
    }
    ...
    this.calcNewDiff = function () {
        ...
        if (proxyMinerName in proxyMiners) {
            hashes = proxyMiners[proxyMinerName].hashes;
            target = 5;
            period = (Date.now() - proxyMiners[proxyMinerName].connectTime) / 1000;
        } else if (this.payout in minerWallets && minerWallets[this.payout].last_ver_shares >= MAX_VER_SHARES_PER_SEC * VER_SHARES_PERIOD) {
            hashes = minerWallets[this.payout].hashes;
            target = 5;
            period = (Date.now() - minerWallets[this.payout].connectTime) / 1000;
        } else {
            hashes = this.hashes;
            target = this.proxy ? 5 : global.config.pool.targetTime;
            period = (Date.now() - this.connectTime) / 1000;
        }
        if (hashes === 0) {
        ...
        return Math.floor(hashes * target / period);
    };
    ...
    if (protoVersion === 1) {
        ...
        this.getJob = function () {
            ...
            if (!this.proxy) {
                ...
                this.cachedJob = {
                    blob: blob,
                    algo: global.coinFuncs.algoTypeStr(bt.port),
        ...
        function submit_block(retry) {
            global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [shareBuffer.toString('hex')], function (rpcResult) {
                if (rpcResult.error) {
                    ...
                    if (isNotifyAdmin) global.support.sendEmail(global.config.general.adminEmail,
                        ...
                        threadName + "Error submitting block at height " + job.height + " (active block template height: " + activeBlockTemplate[blockTemplate.algo].height + ") from " + miner.logString + ", share type: " + shareType + " error: " + JSON.stringify(rpcResult.error)
                    );
                    if (global.config.pool.trustedMiners) {
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        case 'login':
            ...
            let minerId = uuidV4();
            let miner = new Miner(minerId, params.login, params.pass, ip, difficulty, pushMessage, 1, portData.portType, portData.port, params.agent, params.algo, params["algo-hr"]);
            let time_now = Date.now();
            ...
            break;
        case 'getjob':
            let miner = activeSmartMiners[params.id];
            ...
            miner.heartbeat();
            if (miner.algos && params.algo && params["algo-hr"]) {
                const status = miner.setAlgos(params.algo, params["algo-hr"]);
                if (status != "") {
                    sendReply(status);
                    return;
                }
        ...
        case 'submit':
            ...
            if (activeBlockTemplate[miner.jobLastAlgo].idHash !== job.blockHash) {
                ...
                blockTemplate = activeBlockTemplate[miner.jobLastAlgo];
            }
...
if (cluster.isMaster) {
    ...
    if (!global.config.daemon.activePort) {
        ...
        if (global.config.daemon.activePortHeavy && global.config.daemon.activePortLight) {
            setInterval(updateActivePort, 5*1000, "Heavy");
            setInterval(updateActivePort, 5*1000, "Light");
            templateUpdate("Heavy");
            templateUpdate("Light");
            setTimeout(templateUpdate, 50, "Heavy", true);
            setTimeout(templateUpdate, 50, "Light", true);
        } else {
            console.warn("global.config.daemon.activePortHeavy/activePortLight are not defined, so ignoring their algos");
        }
    }
