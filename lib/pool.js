...
function templateUpdate(coin, repeating) {
    ...
    if (activePort) global.coinFuncs.getPortLastBlockHeader(activePort, function (err, body) {
        ...
        if (err === null) {
            ...
            const isHashFactorChange = !(coin in lastCoinHashFactor) || (!coinHashFactor && lastCoinHashFactor[coin]) ||
                                       (coinHashFactor && Math.abs(lastCoinHashFactor[coin] - coinHashFactor) / coinHashFactor > 0.05);
            if ((!(coin in lastBlockHash) || body.hash !== lastBlockHash[coin]) && coinHashFactor) {
