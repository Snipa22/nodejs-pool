...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    for (let index = 1; index < login_div_split.length - 1; ++ index) {
        const percent_raw = parseInt(login_div_split[index]);
        const percent = isNaN(percent_raw) ? 1 : percent_Raw;
        if (percent < 1) {
            this.error = "Your payment divide split " + percent + " is below 1% and can't be processed";
            this.valid_miner = false;
            return;
        }
        if (percent > 99) {
            this.error = "Your payment divide split " + percent + " is above 99% and can't be processed";
            this.valid_miner = false;
            return;
        }
        payout_percent_left -= percent;
        if (payout_percent_left < 1) {
            this.error = "Your summary payment divide split exceeds 99% and can't be processed";
            this.valid_miner = false;
            return;
        }
...
function storeShareDiv(miner, share_reward, share_reward2, share_num, worker_name, bt_port, bt_height, bt_difficulty, isBlockCandidate, isTrustedShare) {
    ...
    if (miner.payout_div === null) {
        ...
        for (let payout in miner.payout_div) {
            ...
            const shares         = share_reward  * 100 / payoutPercent;
            const shares2        = share_reward2 * 100 / payoutPercent;
            global.database.storeShare(bt_height, global.protos.Share.encode({
