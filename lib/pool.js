...
function submit_block(miner, job, blockTemplate, shareBuffer, hashDiff, shareType, retry) {
    global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [shareBuffer.toString('hex')], function (rpcResult) {
        if (rpcResult.error) {
            ...
            if (retry) {
                setTimeout(submit_block, 500, miner, job, blockTemplate, shareBuffer, hashDiff, shareType, false);
            } else {
...
function processShare(miner, job, blockTemplate, params) {
    ...
    if (hashDiff.ge(blockTemplate.difficulty)) { // Submit block to the RPC Daemon.
        ...
        submit_block(miner, job, blockTemplate, shareBuffer, hashDiff, shareType, true);
    ...
    if (is_mm && hashDiff.ge(blockTemplate.child_template.difficulty)) { // Submit child block to the RPC Daemon.
        ...
        submit_block(miner, job, blockTemplate.child_template, shareBuffer2, hashDiff, shareType, true);
        is_block_diff_matched = true;
    }
