...
const baseEthDiff   = global.coinFuncs.baseEthDiff();

const BLOCK_NOTIFY_PORT = 2223;
const DAEMON_POLL_MS = 500;

const id_min = 100000000000000;
let id = id_min;

function get_new_id() {
    if (++id > 999999999999999) id = id_min;
    return id.toString();
};

const eth_job_id_min = 0x1000;
let eth_job_id = eth_job_id_min;

function get_new_eth_job_id() {
    if (++eth_job_id > 0xFFFF) eth_job_id = eth_job_id_min;
    return eth_job_id.toString(16);
};

let UNIQUE_WORKER_ID;
const eth_extranonce_id_min = 0x1000;
let eth_extranonce_id = eth_extranonce_id_min;

function get_new_eth_extranonce_id() {
    if (++eth_extranonce_id > 0xFFFF) eth_extranonce_id = eth_extranonce_id_min;
    return UNIQUE_WORKER_ID.toString(16) + eth_extranonce_id.toString(16);
};
...
function process_rpc_template(rpc_template, coin, port, coinHashFactor, isHashFactorChange, latest_block_body) {
    ...
    const blob_type_num = global.coinFuncs.portBlobType(template.port);
    if (global.coinFuncs.blobTypeEth(blob_type_num)) template.height = parseInt(latest_block_body.number) + 1;
...
function templateUpdateReal(coin, port, coinHashFactor, isHashFactorChange, latest_block_body) {
    global.coinFuncs.getPortBlockTemplate(port, function (body_bt) {
        ...
        if (body_bt) {
            const template = process_rpc_template(body_bt, coin, port, coinHashFactor, isHashFactorChange, latest_block_body);
                ...
                if (port in global.coinFuncs.getMM_CHILD_PORTS()) {
                    ...
                    for (let parent_port in parent_ports) {
                        ...
                        if (parent_coin in activeBlockTemplates) {
                            const parent_template = process_rpc_template(activeBlockTemplates[parent_coin], parent_coin, parent_port, lastCoinHashFactor[parent_coin], false, latest_block_body);
            ...
            setTimeout(templateUpdateReal, 3000, coin, port, coinHashFactor, isHashFactorChange, latest_block_body);
        }
    });
}
    ...
    if (coinHashFactor) global.coinFuncs.getPortLastBlockHeader(port, function (err, body) {
        if (!newCoinHashFactor[coin]) {
            ...
            if (!(coin in lastBlockHash) || body.hash !== lastBlockHash[coin]) {
                ...
                templateUpdateReal(coin, port, coinHashFactor, isHashFactorChange, body);
...
function getEthTargetHex(diff) {
    const size2 = 64;
    const diff2 = (baseEthDiff / diff).toString(16).substr(0, size2);
    return '0'.repeat(size2 - diff2.length) + diff2;
};
    ...
    if (protoVersion === 1) {
        ...
        this.sendCoinJob = function(coin, params) {
            ...
            if (this.protocol == "grin") {
                ...
                const target = global.coinFuncs.blobTypeEth(blob_type_num) ? getEthTargetHex(this.difficulty) : job[3];
                if (!this.last_target || this.last_target !== target) {
                    this.pushMessage({method: "mining.set_target", params: [ target ]});
...
function submit_block(miner, job, blockTemplate, blockData, resultHash, isTrustedShare, isParentBlock, isRetrySubmitBlock, submit_blockCB) {
    let reply_fn = function (rpcResult, rpcStatus) {
        ...
        // Success! Submitted a block without an issue.
            ...
            const blockFastHash = global.coinFuncs.blobTypeDero(blob_type_num) ? rpcResult.result.blid :
                                  (  global.coinFuncs.blobTypeRaven(blob_type_num) || global.coinFuncs.blobTypeEth(blob_type_num) ? resultHash.toString('hex') :
                                     global.coinFuncs.getBlockID(blockData, blockTemplate.port).toString('hex')
                                  );
            ...
            if (isRetrySubmitBlock) {
                ...
                setTimeout(submit_block, 500, miner, job, blockTemplate, blockData, resultHash, isTrustedShare, isParentBlock, false, submit_blockCB);
            } else {
...
function hashEthBuffDiff(hash) { // float as result
    return baseEthDiff / bignum.fromBuffer(hash).toNumber();
}
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    let verifyShare = function(verifyShareCB) {
        if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
            ...
            const resultHash = global.coinFuncs.c29_cycle_hash(params.pow, blob_type_num);
            return verifyShareCB(hashBuffDiff(resultHash), resultHash, blockData, false, true);
            ...
            const resultHash = global.coinFuncs.slowHashBuff(convertedBlob, blockTemplate, params.nonce, params.mixhash);
            return verifyShareCB(hashRavenBuffDiff(resultHash), resultHash, blockData, false, true);
            ...
            const resultHash = hashes[0].toString('hex');
            const blockData = [ "0x" + params.nonce, "0x" + blockTemplate.hash, "0x" + hashes[1].toString('hex') ];
            return verifyShareCB(hashEthBuffDiff(resultHash), resultHash, blockData, false, true);
        }
        ...
        if ( global.config.pool.trustedMiners &&
            ...
            if (miner.lastSlowHashAsyncDelay) {
                setTimeout(function() { return verifyShareCB(hashDiff, resultHash, blockData, true, true); }, miner.lastSlowHashAsyncDelay);
                debug("[MINER] Delay " + miner.lastSlowHashAsyncDelay);
            } else {
                return verifyShareCB(hashDiff, resultHash, blockData, true, true);
            }
            ...
            if (isBlockDiffMatched) {
                if (miner.validShares || (miner.payout in minerWallets && minerWallets[miner.payout].hashes)) {
                    submit_block(miner, job, blockTemplate, blockData, resultHash, true, true, true, function(block_submit_result) {
                        ...
                        return verifyShareCB(hashDiff, resultHash, blockData, false, false);
                    });
               } else {
                    const hash = global.coinFuncs.slowHash(convertedBlob, blockTemplate);
                    if (hash !== resultHash) {
                    ...
                    walletTrust[miner.payout] += job.rewarded_difficulty2;
                    return verifyShareCB(hashDiff, resultHash, blockData, false, true);
               }
            } else {
                const time_now = Date.now();
                global.coinFuncs.slowHashAsync(convertedBlob, blockTemplate, function(hash) {
    ...
    verifyShare(function(hashDiff, resultHash, blockData, isTrustedShare, isNeedCheckBlockDiff) {
        if (isNeedCheckBlockDiff && ge(hashDiff, blockTemplate.difficulty)) { // Submit block to the RPC Daemon.
            ...
            submit_block(miner, job, blockTemplate, blockData, resultHash, isTrustedShare, true, true);
        }
    ...
    switch (method) {
        ...
        case 'login': { // Grin and default
            ...
            if (method === 'mining.authorize') {
                socket.eth_extranonce = socket.eth_extranonce ? socket.eth_extranonce : get_new_eth_extranonce_id();
                miner.eth_extranonce = socket.eth_extranonce;
            }
            ...
            if (id === "Stratum") {
                ...
                if (coin !== false) {
                    ...
                    const blob_type_num = global.coinFuncs.portBlobType(global.coinFuncs.COIN2PORT(coin));
                    if (global.coinFuncs.blobTypeRaven(blob_type_num)) { // xmrig specifics
	    ...
	    sendReply(null, [ [ "mining.notify", get_new_id(), "EthereumStratum/1.0.0" ], socket.eth_extranonce ]);
            break;
        }
...
} else getUniqueWorkerID(function(uniqueWorkerID) {
    UNIQUE_WORKER_ID = uniqueWorkerID;
    console.log(threadName + "Starting pool worker with " + UNIQUE_WORKER_ID + " unique id");
