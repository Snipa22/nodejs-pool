...
function pad_hex(num, bytes) {
    return ("00".repeat(bytes) + num.toString(16)).substr(-bytes*2);
}
...
function getEthTargetHex(diff, size) {
    const size2 = size * 2;
    const diff2 = baseDiff.div(diff).toBuffer().toString('hex').substr(0, size2);
    return '0'.repeat(size2 - diff2.length) + diff2;
};
    ...
    if (protoVersion === 1) {
        ...
        this.sendCoinJob = function(coin, params) {
            ...
            if (this.protocol == "grin") {
                ...
                const target = global.coinFuncs.blobTypeEth(blob_type_num) ? getEthTargetHex(this.difficulty, 8) : job[3];
