...
function updateCoinHashFactor(coin) {
    global.support.getCoinHashFactor(coin, function (coinHashFactor) {
        if (coin === "RVN") coinHashFactor = 2000000; // !!!
...
function recordShareData(miner, job, isBlockCandidate, hashHex, isTrustedShare, blockTemplate) {
...
function submit_block(miner, job, blockTemplate, shareBuffer, resultHash, isTrustedShare, isParentBlock, isRetrySubmitBlock) {
    let reply_fn = function (rpcResult, rpcStatus) {
        if (rpcResult.error) {
            // Did not manage to submit a block.  Log and continue on.
            recordShareData(miner, job, false, null, isTrustedShare, blockTemplate);
            ...
            recordShareData(miner, job, true, blockFastHash, isTrustedShare, blockTemplate);
            ...
            if (isRetrySubmitBlock) {
                ...
                setTimeout(submit_block, 500, miner, job, blockTemplate, shareBuffer, resultHash, isTrustedShare, isParentBlock, false);
            } else {
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    let verifyShare = function(verifyShareCB) {
        if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
            ...
            return verifyShareCB(hashBuffDiff(global.coinFuncs.c29_cycle_hash(params.pow, blob_type_num)), shareBuffer, false, null);
        }
        ...
        if (global.coinFuncs.blobTypeRaven(blob_type_num)) {
            ...
            const hash = global.coinFuncs.slowHashBuff(convertedBlob, blockTemplate, params.nonce, params.mixhash);
            return verifyShareCB(hashRavenBuffDiff(hash), shareBuffer, false, null);
        }
    ...
    verifyShare(function(hashDiff, shareBuffer, isTrustedShare, isBlockDiffMatched) {
        isBlockDiffMatched = isBlockDiffMatched === null ? ge(hashDiff, blockTemplate.difficulty) : isBlockDiffMatched;
        ...
        if (!ge(hashDiff, job.difficulty)) {
            ...
            if (is_mm) {
                ...
                recordShareData(miner, job, false, null, isTrustedShare, blockTemplate.child_template);
            }
