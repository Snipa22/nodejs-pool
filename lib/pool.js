...
function process_rpc_template(rpc_template, coin, port, coinHashFactor, isHashFactorChange) {
    ...
    if (port in global.coinFuncs.getMM_PORTS()) {
        ...
        if (child_coin in activeBlockTemplates) {
            ...
            console.log("SSSSSSSSSSSSSSS1 " + coin + " " + activeBlockTemplates[coin].child_template_prev_hash);
            template.child_template_prev_hash = template.child_template.prev_hash;
            console.log("SSSSSSSSSSSSSSS2 " + child_coin + " " + template.child_template_prev_hash);
                ...
                if (activePort in global.coinFuncs.getMM_CHILD_PORTS()) {
                    ...
                    for (let parent_port in parent_ports) {
                        ...
                        if (parent_coin in activeBlockTemplates) {
                            const parent_template = process_rpc_template(activeBlockTemplates[parent_coin], parent_coin, parent_port, currCoinHashFactor[coin], false);
...
function setNewBlockTemplate(template) {
    ...
    if (coin in activeBlockTemplates) {
        if (activeBlockTemplates[coin].prev_hash === template.prev_hash) {
            if ("child_template" in template && cluster.isMaster) {
                console.log(threadName + "XXXXXXX0 " + template.child_template.prev_hash);
                console.log(threadName + "XXXXXXX1 " + template.child_template_prev_hash);
                if ("child_template" in activeBlockTemplates[coin] && cluster.isMaster) {
                console.log(threadName + "XXXXXXX2 " + activeBlockTemplates[coin].child_template.prev_hash);
                console.log(threadName + "XXXXXXX3 " + activeBlockTemplates[coin].child_template_prev_hash);
                }
                if ("child_template" in activeBlockTemplates[coin] && activeBlockTemplates[coin].child_template_prev_hash === template.child_template_prev_hash) {
                    console.log(threadName + "XXXXXXX " + activeBlockTemplates[coin].child_template_prev_hash);
