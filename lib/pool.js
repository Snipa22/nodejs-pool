...
function getUniqueWorkerID(cb) {
    global.mysql.query("INSERT INTO pool_workers (pool_id, worker_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE id=id", [global.config.pool_id, process.env['WORKER_ID']], function(err,rows) {
        if (err) {
...
if (cluster.isMaster) {
    ...
    console.log(threadName + "Starting pool worker with " + uniqueWorkerID + " unique id");
