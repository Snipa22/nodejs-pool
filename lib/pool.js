...
if (cluster.isMaster) {
    ...
    threadName = "(Worker " + cluster.worker.id2 + " - " + process.pid + ") ";
...
function retargetMiners() {
    ...
    process.send({type: 'minerPortCount', data: { worker_id: cluster.worker.id2, ports: minerCount } });
}
...
// miner agent strings (for cluster.worker.id2 == 1)
let minerAgents = {};
...
function handleMinerData(socket, id, method, params, ip, portData, sendReply, sendReplyFinal, pushMessage) {
    switch (method) {
        ...
        case 'login': { // grin and default
            ...
            if (params.agent && cluster.worker.id2 == 1) minerAgents[params.agent] = 1;
            let time_now = Date.now();
...
setInterval(function dump_vars() {
   const fn = "dump" + (cluster.isMaster ? "" : "_" + cluster.worker.id2.toString());
...
if (cluster.isMaster) {
    ...
    cluster.on('exit', function (worker, code, signal) {
        ...
        const prev_id = worker.id;
        worker = cluster.fork();
        worker.id2 = prev_id;
        worker.on('message', messageHandler);
        global.support.sendEmail(global.config.general.adminEmail, "FYI: Started new worker " + worker.id2,
            "Hello,\r\nMaster thread of " + global.config.hostname + " starts new worker with id " + worker.id2);
    });
    ...
    setInterval(function () {
       ...
       fs.access(extra_wallet_verify_fn, fs.F_OK, function(err) {
          ...
          const fn = "extra_verify_wallet_hashes_" + cluster.worker.id2.toString();
