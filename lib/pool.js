...
function templateUpdate2(coin, port, coinHashFactor, isHashFactorChange, body_header) {
    if (port == 8545) {
        ...
        if (!body_bt) {
            ...
            return setTimeout(templateUpdate2, 3000, coin, port, coinHashFactor, isHashFactorChange);
        }
    ...
    if (coinHashFactor) global.coinFuncs.getPortLastBlockHeader(port, function (err, body) {
        if (!newCoinHashFactor[coin]) {
            ...
            if (!(coin in lastBlockHash) || body.hash !== lastBlockHash[coin]) {
                ...
                templateUpdate2(coin, port, coinHashFactor, isHashFactorChange, body);
        ...
        this.getCoinJob = function (coin, params) {
		    ...
		    noncebytes: 4,
	            ...
	            newJob.id,
                    ...
                    getTargetHex(coin_diff, global.coinFuncs.nonceSize(blob_type_num)),
                    "",
                    true
                ]; else this.cachedJob = {
                    blob:       blob_hex,
...
function submit_block(miner, job, blockTemplate, blockData, resultBuff, isTrustedShare, isParentBlock, isRetrySubmitBlock, submit_blockCB) {
    let reply_fn = function (rpcResult, rpcStatus) {
        ...
        // Success! Submitted a block without an issue.
                          ...
                          ( rpcResult.response !== 'rejected' ) || // ERG
            ...
            } else if (global.coinFuncs.blobTypeRvn(blob_type_num) || global.coinFuncs.blobTypeErg(blob_type_num)) {
    ...
    } else if (global.coinFuncs.blobTypeRvn(job.blob_type_num)) {
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    let shareThrottled = function() {
        if (miner.payout in minerWallets && ++minerWallets[miner.payout].last_ver_shares >= global.config.pool.minerThrottleSharePerSec * global.config.pool.minerThrottleShareWindow) {
            if (minerWallets[miner.payout].last_ver_shares === global.config.pool.minerThrottleSharePerSec * global.config.pool.minerThrottleShareWindow) {
    ...
    let verifyShare = function(verifyShareCB) {
        if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
            ...
            return verifyShareCB(hashEthBuffDiff(hashes[1]), hashes[0], params.nonce, false, true);
        }
    ...
    switch (method) {
        ...
        case 'login': { // Grin and default
            ...
            if (!miner.proxy) {
                ...
                if ((params.agent && params.agent.includes('proxy')) || (proxyMinerName in proxyMiners)) {
                    if (!addProxyMiner(miner)) {
                        return sendReplyFinal("Temporary (one hour max) ban since you connected too many workers. Please use proxy (https://github.com/MoneroOcean/xmrig-proxy)", 600);
                    }
            ...
            if (id === "Stratum") { // if grin miner is connected directly to the pool
                ...
                if (coin !== false) {
                    const params = getCoinJobParams(coin);
                    const blob_type_num = global.coinFuncs.portBlobType(global.coinFuncs.COIN2PORT(coin));
                    if (global.coinFuncs.blobTypeRvn(blob_type_num) || global.coinFuncs.blobTypeEth(blob_type_num)) { // xmrig specifics
                        const new_id = socket.eth_extranonce_id ? socket.eth_extranonce_id : get_new_eth_extranonce_id();
                        if (new_id !== null) {
                            socket.eth_extranonce_id = new_id;
                            miner.eth_extranonce     = eth_extranonce(new_id);
        ...
        case 'submit': { // grin and default
            ...
            if (method === 'mining.submit') {
                ...
                } else if (global.coinFuncs.blobTypeEth(blob_type_num) && params.raw_params.length >= 5) {
            ...
            processShare(miner, job, blockTemplate, params, function(shareAccepted) {
                ...
                if (miner.protocol === "grin") {
                    sendReply(null, "ok");
                } else if (global.coinFuncs.blobTypeRvn(blob_type_num) || global.coinFuncs.blobTypeEth(blob_type_num)) {
                    sendReply(null, true);
                } else {
    ...
    async.each(global.config.ports, function (portData) {
        ...
        function socketConn(socket) {
            ...
            socket.on('data', function (d) {
                ...
                if (socket.miner_ids) socket.miner_ids.forEach(miner_id => removeMiner(activeMiners.get(miner_id)));
