...
function templateUpdate2(coin, port, isHashChange, coinHashFactor, isHashFactorChange, body_header) {
    if (port == 8545 || port == 8645) {
        ...
        const isRewardChange = body_bt.expected_reward && (!(coin in lastBlockReward) || (lastBlockReward[coin] && body_bt.expected_reward / lastBlockReward[coin] > 1.01));

        if ( isHashChange || isTimeChange || isRewardChange) {
            console.log(port + ": " + isHashChange + " " + isTimeChange + " " + isRewardChange + " " + lastBlockReward[coin] + " " + body_bt.expected_reward);
