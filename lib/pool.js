...
let walletAcc            = {};
// number of worker_name for wallet_key (so we do not count the by iteration)
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate) {
    ...
    if (blockCandidate) {
        ...
        let worker_name = safe_worker_name in wallet || walletWorkerCount[wallet_key] < 100 ? safe_worker_name : "all_other_workers";
        ...
        if (height !== job.height || difficulty !== activeBlockTemplate.difficulty || time_now - worker["time"] > 60*1000) {
            let acc = worker["acc"];
            if (acc != 0) {
            ...
            if (time_now - walletWorkerScanTime[wallet_key] > 60*1000) {
                ...
                for (let worker_name2 in wallet) {
                    ...
                    if (time_now - worker2["time"] > 5*60*1000) {
                        let acc2 = worker2["acc"];
                        if (acc2 != 0) {
                            //console.log("!!! " + wallet_key + " / " + worker_name2  + ": storing old worker share " + height2 + " " + difficulty2 + " " + time_now + " " + acc2);
                            global.database.storeShare(worker2["height"], global.protos.Share.encode({
	                        shares: acc2,
	                        paymentAddress: miner.address,
	                        paymentID: miner.paymentID,
	                        foundBlock: blockCandidate,
