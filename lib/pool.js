...
function templateUpdate(coin, repeating) {
    ...
    if (activePort) global.coinFuncs.getPortLastBlockHeader(activePort, function (err, body) {
        ...
        if (err === null) {
            ...
            } else if ( !(coin in lastCoinHashFactor) || (!coinHashFactor && lastCoinHashFactor[coin]) ||
                        (coinHashFactor && Math.abs(lastCoinHashFactor[coin] - coinHashFactor) / coinHashFactor > 0.05)
            ) {
                lastCoinHashFactor[coin] = coinHashFactor;
...
function setNewCoinHashFactor(coin, coinHashFactor, check_height) {
    ...
    if (isHashFactorChange) {
        ...
        if (cluster.isMaster) console.log(threadName + "Full BT update for coin " + coin + " with hash factor changed from " + lastCoinHashFactor[coin] + " to " + coinHashFactor);
        lastCoinHashFactor[coin] = coinHashFactor;
        ...
        if (cluster.isMaster) console.log(threadName + "Fast BT update for coin " + coin + " with the same " + coinHashFactor + " hash factor (" + lastCoinHashFactor[coin] + " base)");
        if (check_height) {
            for (var [minerId, miner] of activeMiners) {
