...
function templateUpdate(coin, repeating) {
    ...
    if ((activePort && coinHashFactor) || (coin in failedPortLastBlockHeaderCoinHashFactor)) global.coinFuncs.getPortLastBlockHeader(activePort, function (err, body) {
        if (coin in failedPortLastBlockHeaderCoinHashFactor) {
              console.error("!!!1 " + activePort + " port: " + currCoinHashFactor[coin] + " " + lastCoinHashFactor[coin] + " " + coinHashFactor);
            if (!coinHashFactor) {
              coinHashFactor = failedPortLastBlockHeaderCoinHashFactor[coin];
            }
              console.error("!!!2 " + activePort + " port: " + currCoinHashFactor[coin] + " " + lastCoinHashFactor[coin] + " " + coinHashFactor);
        ...
        if (activePort !== global.config.daemon["activePort" + coin]) {
            ...
            if (!(coin in lastBlockHash) || body.hash !==
 lastBlockHash[coin]) {
