...
function processShare(miner, job, blockTemplate, params) {
    ...
    const reward_diff  = job.rewarded_difficulty2;
    const miner_wallet = miner.payout;
    const is_safe_to_trust = reward_diff < 400000 && (
        (miner_wallet in walletTrust && reward_diff * 1000 < walletTrust[miner_wallet]) ||
        (miner.trust.threshold <= 0 && crypto.randomBytes(1).readUIntBE(0, 1) > miner.trust.probability)
    );
    if (global.config.pool.trustedMiners && is_safe_to_trust && miner.trust.check_height !== job.height) {
        try {
        ...
        case 'submit':
            ...
            if (global.config.pool.trustedMiners) {
                if (shareAccepted) {
                    ...
                    if (miner.trust.probability < global.config.pool.trustMin) miner.trust.probability = global.config.pool.trustMin;
