...
function updateActivePort() {
    global.mysql.query("SELECT item_value FROM pool.config WHERE module = 'daemon' and item = 'activePort'").then(function (rows) {
        ...
        let newActivePort = parseInt(rows[0].item_value);
        if (global.config.daemon.activePort && global.config.daemon.activePort !== newActivePort) {
            if (isActivePortUpdateBlocked) {
                console.error("Blocked change of active port from " + global.config.daemon.activePort + " to " + newActivePort); 
            } else {
...
function templateUpdate(repeating) {
    if (global.config.general.allowStuckPoolKill && cluster.isMaster && lastBlockHashUpdateTime && Date.now() - lastBlockHashUpdateTime > 60*60*1000) {
...
function newBlockTemplate(template) {
    ...
    if (activeBlockTemplate && activeBlockTemplate.port != template.port && global.config.pool.trustedMiners) {
        for (let minerId in activeMiners) {
            if (activeMiners.hasOwnProperty(minerId)) {
                let miner = activeMiners[minerId];
                if (miner.trust.threshold === 0) miner.trust.threshold = 1;
            }
        }
    }
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent) {
    ...
    if (typeof(addressSplit[1]) !== 'undefined' && global.coinFuncs.validatePlainAddress(addressSplit[0].length) && addressSplit[1].length === 64 && hexMatch.test(addressSplit[1])) {
