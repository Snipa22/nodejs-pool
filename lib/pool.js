...
function templateUpdateReal(coin, port, coinHashFactor, isHashFactorChange) {
    global.coinFuncs.getPortBlockTemplate(port, function (rpcResponse) {
        ...
        if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
            const template = process_rpc_template(rpcResponse.result, coin, port, coinHashFactor, isHashFactorChange);
...
function submit_block(miner, job, blockTemplate, shareBuffer, resultHash, hashDiff, isTrustedShare, isParentBlock, isRetrySubmitBlock) {
    let reply_fn = function (rpcResult) {
        if (rpcResult.error) {
