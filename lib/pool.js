...
function Miner(id, login, pass, ipAddress, startingDiff, sendReplyMethod, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    this.sendReplyMethod = sendReplyMethod;
    ...
    if (protoVersion === 1) {
        this.getCoinJob = function (coin, params) {
		    ...
		    noncebytes: 4,
                    height:     bt.height,
                    job_id:     newJob.id,
                    difficulty: getTargetHex(this.difficulty),
                    id:         this.id
                }; else this.cachedJob = {
                    blob:       blob,
        ...
        this.sendCoinJob = function(coin, params) {
            ...
            if (job === null) return;
            return this.protocol === "grin" ? sendReplyMethod("getjobtemplate", job) : this.sendReplyMethod('job', job);
        };
...
function handleMinerData(socket, id, method, params, ip, portData, sendReply, sendReplyFinal, sendReplyMethod) {
    switch (method) {
        case 'login': { // grin and default
            ...
            let miner = new Miner(
                minerId, params.login, params.pass, ip, difficulty, sendReplyMethod, 1, portData.portType, portData.port, params.agent,
...
if (cluster.isMaster) {
    ...
    async.each(global.config.ports, function (portData) {
        ...
        let handleMessage = function (socket, jsonData, sendReplyMethod) {
            if (!jsonData.id) {
            ...
            handleMinerData(socket, jsonData.id, jsonData.method, jsonData.params, socket.remoteAddress, portData, sendReply, sendReplyFinal, sendReplyMethod);
        };
            ...
            socket.on('data', function (d) {
                ...
                if (dataBuffer.indexOf('\n') !== -1) {
                    ...
                    for (let i = 0; i < messages.length; i++) {
                        ...
                        handleMessage(socket, jsonData, sendReplyMethod);
                    }
