...
function templateUpdate(coin, repeating) {
    ...
    if ((activePort && coinHashFactor) || (coin in failedPortLastBlockHeader)) global.coinFuncs.getPortLastBlockHeader(activePort, function (err, body) {
        delete failedPortLastBlockHeader[coin];
        if (activePort !== global.config.daemon["activePort" + coin]) {
            ...
            failedPortLastBlockHeader[coin] = 1;
            console.error(threadName + "Last block header request for " + global.config.daemon["activePort" + coin] + " port failed!");
            coinHashFactorUpdate(coin, 0);
