...
let lastBlockGoodTime = {}; // time when blocks were checked to be in line with other nodes
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent) {
    ...
    if (this.valid_miner && global.coinFuncs.exchangeAddresses.indexOf(this.address) !== -1 && !(this.paymentID)) {
        this.error = "Exchange addresses need 64 char long payment IDs. Please specify it after your wallet address as follows: wallet.paymentid";
        this.valid_miner = false;
    }
        ...
        global.mysql.query("SELECT blockID, port FROM pools WHERE last_checkin > date_sub(now(), interval 30 minute)").then(function (rows) {
            ...
            if (top_height) {
               if (height < top_height - 3) {
                   ...
                   if (!(port in lastBlockGoodTime)) lastBlockGoodTime[port] = Date.now();

                   if (global.config.general.allowStuckPoolKill && port == global.config.daemon.port && Date.now() - lastBlockGoodTime[port] > 30*60*1000) {
                       global.support.sendEmail(global.config.general.adminEmail,
                   ...
                   if (Date.now() - lastBlockFixTime[port] > 10*60*1000) {
                       global.support.sendEmail(global.config.general.adminEmail,
                   ...
                   lastBlockFixTime[port] = lastBlockGoodTime[port] = Date.now();
               }
            } else {
