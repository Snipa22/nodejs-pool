...
function templateUpdate(repeating) {
    global.coinFuncs.getLastBlockHeader(function (err, body) {
        if (err === null) {
            ...
            if (!lastBlockHash || body.hash !== lastBlockHash || !lastBlockTemplateUpdateTime || time_now - lastBlockTemplateUpdateTime > 10*1000) {
            ...
            console.error("Last block header request failed failed!");
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent) {
    ...
    if (global.config.pool.trustedMiners) {
        if (!(this.payout in walletTrust)) walletTrust[this.payout] = 0;
        let is_trusted_wallet = this.difficulty <= 16001 && this.payout in walletTrust && walletTrust[this.payout] > global.config.pool.trustThreshold * 100;
        this.trust = {
...
function processShare(miner, job, blockTemplate, params) {
    ...
    if (global.config.pool.trustedMiners && miner.difficulty < 500000 && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 &&
...
if (cluster.isMaster) {
    ...
    for (let i = 1; i <= numWorkers; ++ i) {
        var lineReader = require('readline').createInterface({
            input: fs.createReadStream("wallet_trust_" + i.toString())
        });
        lineReader.on('line', function (line) {
            let parts = line.split(/\t/);
            if (parts.length != 2) {
                console.error("Error line " + line + " ignored from wallet_trust_" + i.toString() + " file");
                return;
            }
            let wallet = parts[0];
            let trust  = parseInt(parts[1], 10);
            if (!(wallet in walletTrust) || trust < walletTrust[wallet]) {
    ...
    setInterval(function () {
       let str = "";
       for (let wallet in walletTrust) str += wallet + "\t" + walletTrust[wallet].toString() + "\n";
