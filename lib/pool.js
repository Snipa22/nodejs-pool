...
function process_rpc_template(rpc_template, coin, port, coinHashFactor, isHashFactorChange) {
    let template = rpc_template;

    template.coin               = coin;
    template.port               = activePort;
    ...
    if (port in global.coinFuncs.getMM_PORTS()) {
        const child_coin = global.coinFuncs.getPORT2COIN[global.coinFuncs.getMM_PORTS()[port]];
        if (child_coin in activeBlockTemplates) {
            console.log("MERGED MINING: cnUtil.construct_mm_parent_block_blob");
            template.child_template            = activeBlockTemplates[child_coin];
            template.child_template_buffer     = template.child_template.buffer;
            template.parent_blocktemplate_blob = cnUtil.construct_mm_parent_block_blob(
                new Buffer(rpc_template.blocktemplate_blob, 'hex'), global.coinFuncs.portBlobType(this.port, this.buffer[0]), template.child_template_buffer
            ).toString('hex');
        }
    }
...
function templateUpdateReal(coin, activePort, coinHashFactor, isHashFactorChange) {
    global.coinFuncs.getPortBlockTemplate(activePort, function (rpcResponse) {
        ...
        if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
            ...
            const template = process_rpc_template(rpc_template, coin, port, coinHashFactor, isHashFactorChange);
...
function setNewBlockTemplate(template) {
    ...
    if (coin in activeBlockTemplates) {
        if (activeBlockTemplates[coin].prev_hash === template.prev_hash) {
            if (template.port in global.coinFuncs.getMM_PORTS()) {
                const child_coin_port = global.coinFuncs.getMM_PORTS()[template.port];
                const child_coin      = global.coinFuncs.getPORT2COIN()[child_coin_port];
                if (child_coin in activeBlockTemplates && "child_template" in template) {
                  const child_template = activeBlockTemplates[child_coin];
                  if (child_template.prev_hash === template.child_template.prev_hash) {
    ...
    if (template.port in global.coinFuncs.getMM_CHILD_PORTS()) {
        const parent_ports = global.coinFuncs.getMM_CHILD_PORTS()[template.port];
        for (let parent_port in parent_ports) {
            const parent_coin = global.coinFuncs.getPORT2COIN()[parent_port];
            if (parent_coin in activeBlockTemplates) {
                const coinHashFactor = coin in currCoinHashFactor ? currCoinHashFactor[coin] : 0;
                const template = process_rpc_template(activeBlockTemplates[parent_coin], parent_coin, parent_port, coinHashFactor, false);
                process.send ({type: 'newBlockTemplate', data: template});
                sendToWorkers({type: 'newBlockTemplate', data: template});
            }
        }
    }
...
function processShare(miner, job, blockTemplate, params) {
    ...
    if ("child_template" in blockTemplate && hashDiff.ge(blockTemplate.child_template.difficulty)) { // Submit child block to the RPC Daemon.
        ...
        try {
            console.log("MERGED MINING: cnUtil.construct_mm_child_block_blob");
            shareBuffer2 = cnUtil.construct_mm_child_block_blob(
                shareBuffer, this.portBlobType(blockTemplate.port, blockTemplate.buffer[0]), blockTemplate.child_template_buffer
            );
        } catch (e) {
            const err_str = "Can't construct_mm_child_block_blob with " + shareBuffer.toString('hex') + " parent block and " + blockTemplate.child_template.buffer.toString('hex') + " child block share buffers from " + miner.logString + ": " + e;
            console.error(err_str);
