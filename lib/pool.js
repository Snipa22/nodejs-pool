...
function handleMinerData(method, params, ip, portData, sendReply, sendFinalReply, pushMessage) {
    ...
    if (ip in bannedIPs) {
        ...
        sendFinalReply("This IP address is temporarily suspended from mining", 20);
        return;
    }
    let miner;
    switch (method) {
        case 'login':
            if (!params.login) {
                sendFinalReply("No login specified", 10);
                return;
            }
            ...
            if (miner_notification) {
                if (!(miner.payout in lastMinerNotifyTime) || time_now - lastMinerNotifyTime[miner.payout] > 60*60*1000) {
                    ...
                    sendFinalReply(miner_notification + " (miner will connect after several attempts)", 10);
                    return;
                }
            }
            if (miner_agent_notification) {
                ...
                sendFinalReply(miner_agent_notification, 10);
                return;
            }
...
if (cluster.isMaster) {
    ...
    async.each(global.config.ports, function (portData) {
        ...
        let handleMessage = function (socket, jsonData, pushMessage) {
            ...
            let sendFinalReply = function (error, delay_time) {
                setTimeout(function() {
        ...
        function socketConn(socket) {
            ...
            socket.on('data', function (d) {
                ...
                if (err.code !== 'ECONNRESET') {
                    console.warn(threadName + "Socket Error " + err.code + " from " + socket.remoteAddress + " Error: " + err);
                }
            }).on('close', function () {
