...
let minerWallets = {};
const MAX_VER_SHARES_PER_SEC = 20; // per thread
...
if (cluster.isMaster) {
    ...
    setInterval(function () {
        ...
        console.log(`>>> Processed Trusted=${trustedShares}(${trustedSharesPercent}%) / Validated=${normalShares}(${normalSharesPercent}%) / Invalid=${invalidShares}(${invalidSharesPercent}%) / Outdated=${outdatedShares}(${outdatedSharesPercent}%) / Throttled=${throttledShares}(${throttledSharesPercent}%) / Total=${totalShares} shares in the last 30 seconds`);
        totalShares     = 0;
        trustedShares   = 0;
        normalShares    = 0;
        invalidShares   = 0;
...
function processShare(miner, job, blockTemplate, params) {
    ...
    if (global.config.pool.trustedMiners && miner.difficulty < 400000 && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 &&
        ...
        if (miner.payout in minerWallets && ++minerWallets[miner.payout].last_ver_shares > MAX_VER_SHARES_PER_SEC * VER_SHARES_PERIOD) {
            console.error(threadName + "Throttled down miner share submission from " + miner.logString);
    ...
    switch (method) {
        ...
        case 'submit':
            ...
            if (shareAccepted === null) {
                sendReply('Throttle down share submission (please use xmr-node-proxy)');
                return;
            }
