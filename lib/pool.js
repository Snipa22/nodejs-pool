...
let lastBlockHash;
let lastBlockCheckTime  = Date.now(); // time when block was checked to be in line with other nodes or when ./fix_daemon.sh was attempted
let lastBlockCheckTime2 = Date.now(); // time when block was checked to be in line with other nodes
...
function retargetMiners() {
    ...
    const elapsed = Date.now() - time_now;
...
function checkAliveMiners() {
    ...
    const time_now = Date.now();
    const deadline = time_now - global.config.pool.minerTimeout * 1000;
    for (let minerId in activeMiners) {
        if (activeMiners.hasOwnProperty(minerId)) {
            let miner = activeMiners[minerId];
...
function updateActivePort() {
    global.support.getActivePort(function (newActivePort) {
        if (!global.config.daemon.activePort) return;
        if (newActivePort === null) {
            if (global.config.daemon.activePort !== global.config.daemon.port) {
                console.error("Error getting active port, so rolling back to main port");
                global.config.daemon.activePort = global.config.daemon.port;
            }
        } else if (global.config.daemon.activePort !== newActivePort) {
        ...
        function submit_block(retry) {
            global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [shareBuffer.toString('hex')], function (rpcResult) {
                if (rpcResult.error) {
                    ...
                    console.error(threadName + "Error submitting block at height " + job.height + " (active block template height: " + activeBlockTemplate.height + ") from " + miner.logString + ", share type: " + shareType + ", valid: " + isNotifyAdmin + " error: " + JSON.stringify(rpcResult.error));
...
if (cluster.isMaster) {
    ...
    setInterval(function () {
        if (!activeBlockTemplate) continue;

        global.mysql.query("SELECT blockID, port FROM pools WHERE last_checkin > date_sub(now(), interval 30 minute)").then(function (rows) {
            let top_height = 0;
            rows.forEach(function (row) {
                if (row.port != activeBlockTemplate.port) continue;
                if (row.blockID > top_height) top_height = row.blockID;
            });
            if (top_height) {
               if (activeBlockTemplate.height < top_height) {
                   if (Date.now() - lastBlockCheckTime2 > 30*60*1000 && global.config.general.allowStuckPoolKill && activeBlockTemplate.port == global.config.daemon.port) {
                       global.support.sendEmail(global.config.general.adminEmail,
                       ...
                       console.error("Block height was not updated for a long time. Check your monerod. Exiting...");
                       fs.closeSync(fs.openSync("block_template_is_stuck", 'w'));
                       process.exit();
                   }
                   if (Date.now() - lastBlockCheckTime > 10*60*1000) {
                       console.error("Current block height " + activeBlockTemplate.height + " is stuck compared to top height (" + top_height + ") amongst other leaf nodes for " + activeBlockTemplate.port + " port");
                       global.support.sendEmail(global.config.general.adminEmail,
                           ...
                           activeBlockTemplate.height + " is stuck compared to top height (" + top_height + ") amongst other leaf nodes for " +
                           activeBlockTemplate.port + " port"
                       );
                       if (fs.existsSync("./fix_daemon.sh")) {
                           child_process.exec("./fix_daemon.sh", function callback(error, stdout, stderr) {
                               console.log("> ./fix_daemon.sh");
                               console.log(stdout);
                               console.error(stderr);
                               if (error) console.error("./fix_daemon.sh script returned error exit code: " + error.code);
                           });
                       } else {
	                   console.error("No ./fix_daemon.sh script was found to fix stuff");
                       }
                       lastBlockCheckTime = Date.now();
                   }
               } else if (activeBlockTemplate.height == top_height) {
                   lastBlockCheckTime = lastBlockCheckTime2 = Date.now();
               } else {
                   console.error("Current block height " + activeBlockTemplate.height + " is somehow greater than top height (" + top_height + ") amongst other leaf nodes for " + activeBlockTemplate.port + " port");
                   lastBlockCheckTime = lastBlockCheckTime2 = Date.now();
               }
            } else {
               console.error("Can't get top height amongst all leaf nodes for " + activeBlockTemplate.port + " port");
               lastBlockCheckTime = lastBlockCheckTime2 = Date.now();
            }
        });
    }, 60*1000);
