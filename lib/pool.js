...
function submit_block(miner, job, blockTemplate, shareBuffer, hashDiff, isTrustedShare, isParentBlock, isRetrySubmitBlock) {
    global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [shareBuffer.toString('hex')], function (rpcResult) {
        if (rpcResult.error) {
            ...
            console.error(threadName + "Error submitting block at height " + blockTemplate.height + " (active block template height: " + activeBlockTemplates[blockTemplate.coin].height + ") from " + miner.logString + ", is trusted share: " + isTrustedShare + ", valid: " + isNotifyAdmin + " error: " + JSON.stringify(rpcResult.error));
            ...
            if (isNotifyAdmin) setTimeout(function() { // only alert if block height is not changed in the nearest time
                global.coinFuncs.getPortLastBlockHeader(blockTemplate.port, function(err, body) {
                    ...
                    if (blockTemplate.height == body.height + 1) global.support.sendEmail(global.config.general.adminEmail,
                        ...
                        threadName + "Error submitting block at " + blockTemplate.height + " height from " + miner.logString + ", is trusted share: " + isTrustedShare + " error: " + JSON.stringify(rpcResult.error)
                    );
               });
            }, 2*1000);
            ...
            console.log(threadName + "Block " + blockFastHash.substr(0, 6) + " found at height " + blockTemplate.height + " by " + miner.logString +
                ", is trusted share: " + isTrustedShare + " - submit result: " + JSON.stringify(rpcResult.result)
            );
            recordShareData(miner, job, hashDiff.toString(), true, blockFastHash, isTrustedShare, blockTemplate);
        } else {
            if (isRetrySubmitBlock) {
                setTimeout(submit_block, 500, miner, job, blockTemplate, shareBuffer, hashDiff, isTrustedShare, isParentBlock, false);
            } else {
...
function processShare(miner, job, blockTemplate, params) {
    ...
    if (hashDiff.ge(blockTemplate.difficulty)) { // Submit block to the RPC Daemon.
        ...
        submit_block(miner, job, blockTemplate, shareBuffer, hashDiff, isTrustedShare, true, true);
    ...
    if (is_mm && hashDiff.ge(blockTemplate.child_template.difficulty)) { // Submit child block to the RPC Daemon.
        ...
        submit_block(miner, job, blockTemplate.child_template, shareBuffer2, hashDiff, isTrustedShare, false, true);
        is_block_diff_matched = true;
    }
    ...
    if (is_block_diff_matched) { // Do nothing here
        ...
        recordShareData(miner, job, hashDiff.toString(), false, null, isTrustedShare, blockTemplate);
        if (is_mm) recordShareData(miner, job, hashDiff.toString(), false, null, isTrustedShare, blockTemplate.child_template);
    }
