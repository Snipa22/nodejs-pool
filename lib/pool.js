...
function updateActivePort(algo) {
    global.support.getActivePort(algo, function (newActivePort) {
        if (newActivePort === null) {
            if (algo === "" && global.config.daemon.activePort !== global.config.daemon.port) {
            ...
            if (algo !== "") {
                global.support.getAlgoHashFactor(algo, function (newAlgoHashFactor) {
                    if (newAlgoHashFactor === null) {
                        ...
                        if (global.config.daemon[activePortAlgo(algo)] !== newActivePort) {
                            console.log("Changing " + activePortAlgo(algo) + " from " + global.config.daemon[activePortAlgo(algo)] + " to " + newActivePort);
...
function setNewBlockTemplate(template) {
    ...
    if (algo in activeBlockTemplate) {
        ...
        activeBlockTemplace[algo].timeOutdate = Date.now() + 4*1000;
