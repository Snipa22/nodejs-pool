...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_perf) {
    ...
    this.setAlgos = function(algos, algos_perf) {
        ...
        if (algos_perf && (algos_perf instanceof Object)) {
            this.algos_perf = {};
            for (let algo in algos_perf) {
                if (algo.includes("heavy")) this.algos_perf["Heavy"] = algos_perf[algo];
                else if (algo.includes("lite")) this.algos_perf["Light"] = algos_perf[algo];
                else if (algo.includes("fast")) this.algos_perf["Fast"] = algos_perf[algo];
                else this.algos_perf[""] = algos_perf[algo];
            }
            if (!this.algos_perf[""]) return "algo_perf set should include non heavy/lite/fast hashrate";
        } else {
            return "algo_perf set should be present";
        }
        return "";
    };

    if (algos && algos_perf) {
        console.error(JSON.stringify(algos));
        console.error(JSON.stringify(algos_perf));
        const status = this.setAlgos(algos, algos_perf);
        if (status != "") {
            this.error = status;
            this.valid_miner = false;
            return;
    ...
    this.selectBestAlgo = function() {
        ...
        ALGOS.forEach(function(algo) {
            console.error(miner.logString + ": " + algo + "??");
            ...
            console.error(miner.logString + ": " + algo + "? " + miner.algos_perf[algo] * algoHashFactor);
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        case 'login':
            ...
            let minerId = uuidV4();
            miner = new Miner(minerId, params.login, params.pass, ip, difficulty, pushMessage, 1, portData.portType, portData.port, params.agent, params.algo, params["algo-perf"]);
            let time_now = Date.now();
        ...
        case 'getjob':
            ...
            miner.heartbeat();
            if (miner.algos && params.algo && params["algo-perf"]) {
                const status = miner.setAlgos(params.algo, params["algo-perf"]);
                if (status != "") {
                    sendReply(status);
                    return;
                }
