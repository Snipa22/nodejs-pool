...
function submit_block(miner, job, blockTemplate, shareBuffer, hashDiff, isTrustedShare, isParentBlock, isRetrySubmitBlock) {
    global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [shareBuffer.toString('hex')], function (rpcResult) {
        if (rpcResult.error) {
            ...
            console.error(threadName + "Error submitting block at height " + blockTemplate.height + " (active block template height: " + activeBlockTemplates[blockTemplate.coin].height + ") from " + miner.logString + ", isTrustedShare: " + isTrustedShare + ", valid: " + isNotifyAdmin + " error: " + JSON.stringify(rpcResult.error));
            ...
            if (isNotifyAdmin) setTimeout(function() { // only alert if block height is not changed in the nearest time
                global.coinFuncs.getPortLastBlockHeader(blockTemplate.port, function(err, body) {
                    ...
                    if (blockTemplate.height == body.height + 1) global.support.sendEmail(global.config.general.adminEmail,
                        ...
                        threadName + "Error submitting block at " + blockTemplate.height + " height from " + miner.logString + ", isTrustedShare: " + isTrustedShare + " error: " + JSON.stringify(rpcResult.error)
                    );
               });
            }, 2*1000);
