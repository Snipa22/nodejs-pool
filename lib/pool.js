...
function templateUpdateReal() {
    global.coinFuncs.getBlockTemplate(global.config.pool.address, function (rpcResponse) {
        if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
            ...
            let new_hash = crypto.createHash('md5').update(rpcResponse.blocktemplate_blob).digest('hex');
            if (!activeBlockTemplate || activeBlockTemplate.idHash !== new_hash) {
...
function templateUpdate(repeating) {
    if (global.config.general.allowStuckPoolKill && lastBlockHashUpdateTime && Date.now() - lastBlockHashUpdateTime > 30*60*1000) {
    ...
    global.coinFuncs.getLastBlockHeader(function (err, body) {
        if (err === null) {
            ...
            if (is_block_hash_updated /*|| !lastBlockTemplateUpdateTime || time_now - lastBlockTemplateUpdateTime > 30*1000*/) {
...
function newBlockTemplate(template) {
    if (activeBlockTemplate) {
        if (activeBlockTemplate.idHash === crypto.createHash('md5').update(template.blocktemplate_blob).digest('hex')) {
