...
function setNewCoinHashFactor(coin, coinHashFactor, check_height) {
    ...
    if (isHashFactorChange) {
        console.log(threadName + "Full BT update for coin " + coin + " with hash factor changed from " + lastCoinHashFactor[coin] + " to " + coinHashFactor);
        lastCoinHashFactor[coin] = coinHashFactor;
        ...
        console.log(threadName + "Fast BT update for coin " + coin + " with the same " + coinHashFactor + " hash factor");
        if (check_height) {
            for (var [minerId, miner] of activeMiners) {
