...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    this.selectBestAlgo = function() {
        ...
        ALGOS.forEach(function(algo) {
            ...
            if (!global.coinFuncs.isMinerSupportPortAlgo(port, miner.algos, activeBlockTemplate[algo].blob[0])) return;
    ...
    if (protoVersion === 1) {
        ...
        this.getJob = function () {
            ...
            if (!this.proxy) {
                ...
                this.cachedJob = {
                    blob: blob,
                    algo: global.coinFuncs.algoShortTypeStr(bt.port, blob[0]),
                    variant: global.coinFuncs.variantValue(bt.port, blob[0]),
                    job_id: newJob.id,
                    target: target,
                    id: this.id
                };
                ...
                this.cachedJob = {
                    ...
                    blob_type: global.coinFuncs.blobTypeStr(bt.port, blob[0]),
                    algo: global.coinFuncs.algoShortTypeStr(bt.port, blob[0]),
                    variant: global.coinFuncs.variantValue(bt.port, blob[0]),
