...
function templateUpdate(repeating) {
    global.coinFuncs.getBlockTemplate(global.config.pool.address, function (rpcResponse) {
        if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
            ...
            if (!activeBlockTemplate || (new_hash.toString("hex") !== activeBlockTemplate.previous_hash.toString("hex") && rpcResponse.height > activeBlockTemplate.height) || ((rpcResponse.blocktemplate_blob.length > activeBlockTemplate.blob.length) && Date.now() - 10000 > lastUpdateTime)) {
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent) {
    ...
    this.validJobs = global.support.circularBuffer(8);
    ...
    if (protoVersion === 1) {
        ...
        this.sendNewJob = function() {
            ...
            let tempJob = this.sentJobs.toarray().filter(function (intJob) {
                return intJob.job_id === job.job_id || intJob.blockHash === job.blockHash;
            })[0];
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        ...
        case 'submit':
            ...
            let blockTemplate = activeBlockTemplate.idHash === job.blockHash ? activeBlockTemplate : pastBlockTemplates.toarray().filter(function (t) {
                return job.blockHash === t.idHash;
            })[0];
