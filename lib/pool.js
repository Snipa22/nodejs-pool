...
function get_miner_agent_notification(agent, minerLogString) {
    let m;
    if (m = reXMRig.exec(agent)) {
        ...
        if (majorv + minorv + minorv2 < 101) {
            console.error("Old proxy was detected for " + minerLogString + ": xmr-node-proxy (" + agent + ")");
        }
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        case 'login':
            ...
            let miner_agent_notification = params.agent ? get_miner_agent_notification(params.agent, miner.logString) : false;
