...
function templateUpdate2(coin, port, coinHashFactor, isHashFactorChange, body_header) {
    if (port == 8545 || port == 8645) {
        ...
        if (coin === "") {
          ...
          if (body_bt.prev_hash === lastBlockHash[coin]) {
            // replace body_header.hash (lastBlockHash) caching logic by time caching
            if ( global.config.daemon.maxBlockTemplateKeepTime && prevMainBlockTime &&
                 time_now - prevMainBlockTime < global.config.daemon.maxBlockTemplateKeepTime * 1000
               ) return;
            // skip frequent updates of low fee block templates for miners
            if ( global.config.daemon.highFeeBlock && body_bt.expected_reward &&
                 body_bt.expected_reward < global.support.decimalToCoin(global.config.daemon.highFeeBlock)
               ) return;
          } else {
            lastBlockHash[coin] = body_bt.prev_hash;
          }
          prevMainBlockTime = time_now;
        }
...
function templateUpdate(coin, repeating) {
    ...
    if (coinHashFactor) {
      if (coin === "") { // separate logic for XMR (main coin) to allow several block tempaltes per height
        templateUpdate2(coin, port, coinHashFactor, false, null);
        if (repeating === true) setTimeout(templateUpdate, DAEMON_POLL_MS, coin, repeating);
      } else global.coinFuncs.getPortLastBlockHeader(port, function (err, body) {
        if (!newCoinHashFactor[coin]) {
