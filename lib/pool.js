...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_hr) {
    ...
    this.selectBestAlgo = function() {
        ...
        ALGOS.forEach(function(algo) {
            console.error("> " + best_algo + " " + best_algo_hr); // !!! FOR DEBUGGING PURPOSES
            if (!(algo in activeBlockTemplate)) return;
            const port = activeBlockTemplate[algo].port;
            if (!(global.coinFuncs.algoTypeStr(port) in miner.algos)) return;
            if (!(global.coinFuncs.algoShortTypeStr(port) in miner.algos)) return;
            const algoHashFactor = global.config.daemon["algoHashFactor" + algo];
            console.error(">> " + miner.algos_hr[algo] + " " + (miner.algos_hr[algo] * algoHashFactor)); // !!! FOR DEBUGGING PURPOSES
            if (algo in miner.algos_hr && miner.algos_hr[algo] * algoHashFactor > best_algo_hr) {
                best_algo = algo;
                best_algo_hr = miner.algos_hr[algo] * algoHashFactor;
            }
        });
        return best_algo;
    }
