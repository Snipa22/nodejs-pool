...
function setNewCoinHashFactor(isHashFactorChange, coin, coinHashFactor, check_height) {
    ...
    const elapsed = Date.now() - time_before;
    if (elapsed > 50) console.error(threadName + "setNewCoinHashFactor() consumed " + elapsed + " ms for " + activeMiners.size + " miners");
}
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    if (protoVersion === 1) {
        ...
        this.getCoinJob = function (coin, bt) {
        ...
        this.getJob = function () {
            ...
            return this.getCoinJob(coin, activeBlockTemplates[coin]);
        };

        this.sendNewCoinJob = function(coin, bt) {
            const job = this.getCoinJob(coin, bt);
            if (job === null) return;
            return this.messageSender('job', job);
        };
        this.sendNewJob = function() {
            const coin = this.selectBestCoin();
            return this.sendNewCoinJob(coin, activeBlockTemplates[coin]);
        };
    }
}
