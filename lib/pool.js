...
function templateUpdate2(coin, port, isHashChange, coinHashFactor, isHashFactorChange, body_header) {
    if (port == 8545 || port == 8645) {
        ...
        const highFeeBlock     = global.support.decimalToCoin("highFeeBlock" + coin in global.config.daemon ? global.config.daemon["highFeeBlock" + coin] : 0);
        ...
        const isValueChange = highFeeBlock && body_bt.expected_reward && body_bt.expected_reward >= highFeeBlock;

        if ( isHashChange || isTimeChange || isValueChange) {
