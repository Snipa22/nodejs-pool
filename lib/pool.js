...
function templateUpdate(repeating) {
    if (global.config.general.allowStuckPoolKill && cluster.isMaster && lastBlockHashUpdateTime && Date.now() - lastBlockHashUpdateTime > 30*60*1000) {
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate) {
    ...
    if (job.difficulty >= 1000000 || blockCandidate) {
        ...
        //console.log("!!! " + wallet_key + " / " + worker_name  + ": accumulating share " + job.height + " " + activeBlockTemplate.difficulty + " " + worker["time"] + " " + worker["acc"] + " (+" +  job.rewarded_difficulty + ")");
    }
        ...
        case 'submit':
            ...
            if (activeBlockTemplate.height != job.height && lastBlockTemplateUpdateTime) {
                ...
                if (late_time < max_late_time) {
                    job.rewarded_difficulty = Math.floor(job.difficulty * (max_late_time - late_time) / max_late_time);
            ...
            if (!blockTemplate || is_outdated) {
                ...
                console.warn(threadName + err_str + ', Height: ' + job.height + ' from ' + miner.logString);
                if (miner.incremented === false) {
                    miner.newDiff = miner.difficulty + 1;
                    miner.incremented = true;
                } else {
