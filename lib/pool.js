...
function Miner(id, login, pass, ipAddress, startingDiff, pushMessage, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    this.selectBestCoin = function() {
        ...
        if (typeof(this.curr_coin) === 'undefined' || this.curr_coin != best_coin) {
            this.curr_min_diff = global.coinFuncs.portBlobType(global.coinFuncs.COIN2PORT(best_coin)) == 8 ? 1 : global.config.pool.minDifficulty;
...
function handleMinerData(socket, id, method, params, ip, portData, sendReply, sendReplyFinal, sendReplyMethodResult, pushMessage) {
    switch (method) {
        ...
        case 'getjobtemplate': { // grin only
            ...
            miner.heartbeat();
            sendReplyMethodResult("getjobtemplate", miner.getBestCoinJob());
            break;
        }
