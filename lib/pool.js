    ...
    switch (method) {
        case 'login':
            ...
            if (!miner.valid_miner) {
                let time_now = Date.now();
                if (!(miner.payout in lastMessageTime) || time_now - lastMessageTime[miner.payout] > 10*60*1000) {
                    console.log("Invalid miner " + miner.logString + " [" + miner.email + "], disconnecting due to: " + miner.error);
                    lastMessageTime[miner.payout] = time_now;
                }
                sendReply(miner.error);
                return;
            }
            if (miner.payout in notifyAddresses) {
                sendReply(notifyAddresses[miner.payout]);
            }
