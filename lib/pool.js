                ...
                if (port in global.coinFuncs.getMM_CHILD_PORTS()) {
                    ...
                    for (let parent_port in parent_ports) {
                        ...
                        if (parent_coin in activeBlockTemplates) {
                            const parent_template = process_rpc_template(activeBlockTemplates[parent_coin], parent_coin, parent_port, newCoinHashFactor[parent_coin], false);
...
function templateUpdate(coin, repeating) {
    const port           = global.coinFuncs.COIN2PORT(parent_port);
        ...
        if (global.config.pool.trustedMiners &&
            ...
            if (miner.payout in extra_wallet_verify) {
                ...
                if (shareBuffer !== null) {
                    ...
                    const hash2 = global.coinFuncs.slowHash(convertedBlob, blockTemplate);
                    if (hash2 !== resultHash) {
