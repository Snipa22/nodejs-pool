...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    let verifyShare = function(verifyShareCB) {
        if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
            ...
            return verifyShareCB(hash_buff_diff(global.coinFuncs.c29_cycle_hash(params.pow, blob_type_num)), shareBuffer, false, null);
        }
        ...
        const hashDiff = hash_buff_diff(resultBuffer);
...
function handleMinerData(socket, id, method, params, ip, portData, sendReply, sendReplyFinal, pushMessage) {
    switch (method) {
        ...
        case 'submit': { // grin and default
            ...
            const blob_type_num = job.blob_type_num;
            const is_bad_nonce = global.coinFuncs.blobTypeGrin(blob_type_num) ?
                                 (typeof params.nonce !== 'number') || !(params.pow instanceof Array) || (params.pow.length != global.coinFuncs.c29ProofSize(blob_type_num)) :
                                 (typeof params.nonce !== 'string') || !(global.coinFuncs.nonceSize(blob_type_num) == 8 ? nonceCheck64.test(params.nonce) : nonceCheck32.test(params.nonce) );
