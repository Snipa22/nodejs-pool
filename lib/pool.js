...
function is_safe_to_trust(reward_diff, miner_wallet, miner_trust) {
    return reward_diff < 400000 && miner_trust != 0 && (
        (   miner_wallet in walletTrust &&
            ...
            reward_diff * global.config.pool.trustThreshold < miner_trust &&
            crypto.randomBytes(1).readUIntBE(0, 1) > Math.max(256 - miner_trust / reward_diff, global.config.pool.trustMin)
        )
    );
}
