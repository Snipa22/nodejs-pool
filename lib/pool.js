...
let ethExtraonceId = 0;

function get_new_eth_extranonce_id() {
    if (++ethExtraonceId >= 1 << (16 - uniqueWorkerIdBits)) {
      ethExtraonceId = 0;
    ...
    return pad_hex(((ethExtraonceId << uniqueWorkerIdBits) + uniqueWorkerId).toString(16), 2);
};
...
function Miner(id, login, pass, ipAddress, startingDiff, pushMessage, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    this.selectBestCoin = function() {
        ...
        COINS.forEach(function(coin) {
            ...
            if (miner.difficulty * factor > bt.difficulty * 3) {
                if (miner.debugMiner) console.log(threadName + miner.logString + " [WALLET DEBUG] Rejected best " + coin + " coin due to high diff " + miner.difficulty + " " + factor + " " + bt.difficulty);
                return;
            }
    ...
    if (agent && agent.includes('NiceHash')) {
        this.fixed_diff = true;
        if (this.difficulty < global.coinFuncs.niceHashDiff) this.difficulty = global.coinFuncs.niceHashDiff;
    }
    ...
    if (protoVersion === 1) {
        this.getCoinJob = function (coin, params) {
            ...
            if (coin_diff < this.curr_min_diff) coin_diff = this.curr_min_diff;
...
function recordShareData(miner, job, isTrustedShare, blockTemplate) {
    const normalized_diff = job.difficulty * miner.curr_coin_hash_factor;
    miner.hashes += normalized_diff;
    let proxyMinerName = miner.payout; // + ":" + miner.identifier;
    if (proxyMinerName in proxyMiners) proxyMiners[proxyMinerName].hashes += normalized_diff;
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    const normalized_diff = job.difficulty * miner.curr_coin_hash_factor;
    if (miner.payout in minerWallets) minerWallets[miner.payout].hashes += normalized_diff;
    ...
    let shareThrottled = function() {
        if (miner.payout in minerWallets && ++minerWallets[miner.payout].last_ver_shares >= MAX_VER_SHARES_PER_SEC * VER_SHARES_PERIOD) {
            ...
            addProxyMiner(miner);
            const proxyMinerName = miner.payout; // + ":" + miner.identifier;
            proxyMiners[proxyMinerName].hashes += normalized_diff;
    ...
    switch (method) {
        ...
        case 'login': { // Grin and default
            ...
            if (method === 'mining.authorize') {
                miner.eth_extranonce = socket.eth_extranonce = socket.eth_extranonce ? socket.eth_extranonce : get_new_eth_extranonce_id();
            }
