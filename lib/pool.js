    ...
    if (agent) {
        ...
        if (m = reXMRig.exec(agent)) {
            ...
            if (majorv < 2 || (majorv === 2 && minorv < 5)) {
                return "Please update XMRig miner (" + agent + ") to at least v2.5.0 before hardfork on March 28";
            }
        } else if (m = reXMRSTAK.exec(agent)) {
            let majorv = parseInt(m[1]);
            let minorv = parseInt(m[2]);
            if (majorv < 2 || (majorv === 2 && minorv <= 2)) {
                return "Please update xmr-stak miner (" + agent + ") to version AFTER v2.2.0 before hardfork on March 28";
            }
        }
        ...
        //    let minorv = parseInt(m[2]);
        //    if (majorv < 3) {
        //        return "Please update ccminer-cryptonight miner to at least v3.00 before hardfork on March 28";
        //    }
        //}
    }
    return false;
