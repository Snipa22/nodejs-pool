...
function pad_hex(num, bytes) { // up to 8 bytes
    return ("000000000000000" + num.toString(16)).substr(-bytes*2);
}

let eth_job_id = 0;

function get_new_eth_job_id() {
    if (++eth_job_id > 0xFFFF) eth_job_id = 0;
    return pad_hex(eth_job_id, 2);
};

let UNIQUE_WORKER_ID;
let eth_extranonce_id = 0;

function get_new_eth_extranonce_id() {
    if (++eth_extranonce_id > 0xFFFF) eth_extranonce_id = 0;
    return pad_hex(UNIQUE_WORKER_ID, 1) + pad_hex(eth_extranonce_id, 2);
};
