...
let activePortLastUpdateTime;

// global.config.daemon.activePort is only updated in master thread
function updateActivePort(callback) {
    let time_now = Date.now();
    if (!isActivePortUpdateBlocked && (!activePortLastUpdateTime || time_now - activePortLastUpdateTime > 3*1000)) {
        activePortLastUpdateTime = time_now;
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent) {
    ...
    this.hashes = 0;
    this.logString = this.address + " (" + this.identifier + "/" + this.ipAddress + ")";
        ...
        case 'login':
            ...
            if (!miner.valid_miner) {
                ...
                if (!lastMessageTime || time_now - lastMessageTime > 30*1000) {
                    console.log("Invalid miner " + miner.logString + ", disconnecting due to: " + miner.error);
