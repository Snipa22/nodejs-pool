...
let totalShares = 0, trustedShares = 0, normalShares = 0, invalidShares = 0, outdatedShares = 0;
...
if (cluster.isMaster) {
    ...
    setInterval(function () {
        ...
        console.log(`>>> Processed Trusted ${trustedShares} (${trustedSharesPercent}%) / Validated ${normalShares} (${normalSharesPercent}%) / Invalid ${invalidShares} (${invalidSharesPercent}%) / Outdated ${outdatedShares} (${outdatedSharesPercent}%) / Total shares ${totalShares} in the last 30 seconds`);
        totalShares = 0;
        trustedShares = 0;
        normalShares = 0;
        invalidShares = 0;
...
// wallet " " proxy miner name -> { connectTime, count, hashes }
...
function updateActivePort() {
    global.mysql.query("SELECT item_value FROM pool.config WHERE module = 'daemon' and item = 'activePort'").then(function (rows) {
        ...
        if (global.config.daemon.activePort && global.config.daemon.activePort !== row.item_value) {
            if (isActivePortUpdateBlocked) {
                console.error("Blocked change of active port from " + global.config.daemon.activePort + " to " + row.item_value); 
            } else {
                console.log("Changing active port from " + global.config.daemon.activePort + " to " + row.item_value); 
                global.config.daemon.activePort = row.item_value;
            }
        }
    });
}
...
function templateUpdateReal() {
    global.coinFuncs.getPortBlockTemplate(global.config.daemon.activePort, function (rpcResponse) {
        if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
            rpcResponse = rpcResponse.result;
            rpcResponse.port = global.config.daemon.activePort;
            ...
            console.error("Block template request failed for " + global.config.daemon.activePort + " port");
...
function templateUpdate(repeating) {
    if (!cluster.isMaster) {
        console.error("templateUpdate should not be called from a worker thread!");
        return;
    }
    if (global.config.general.allowStuckPoolKill && lastBlockHashUpdateTime && Date.now() - lastBlockHashUpdateTime > 60*60*1000) {
        if (global.config.daemon.activePort === global.config.daemon.port) {
            ...
            console.error("Block height was not updated for an hour. Check your monerod. Exiting...");
            fs.closeSync(fs.openSync("block_template_is_stuck", 'w'));
            process.exit();
        } else {
           global.support.sendEmail(global.config.general.adminEmail,
               "Pool server " + global.config.hostname + " was switched to main chain",
               "The pool server: " + global.config.hostname + " with IP: " + global.config.bind_ip + " was switch to main chain due to block tempalte stuck on " + global.config.daemon.activePort + " port"
           );
           console.error("Alt block height on " + global.config.daemon.activePort + " port was not updated for an hour. Switching to main chain....");
    ...
    global.coinFuncs.getPortLastBlockHeader(global.config.daemon.activePort, function (err, body) {
        if (err === null) {
            ...
            if (is_block_hash_updated) {
                lastBlockHashUpdateTime = time_now;
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent) {
    ...
    this.setNewDiff = function (difficulty) {
        ...
        if (this.fixed_diff) {
            console.log("Dropped low fixed diff " + this.difficulty + " for " + this.payout + ":" + this.identifier + " miner to " + this.newDiff + " dynamic diff");
            this.fixed_diff = false;
        }
        this.sendNewJob();
    };
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate) {
    ...
    if (job.difficulty >= 1000000 || blockCandidate) {
        ...
        //console.log("!!! " + wallet_key + " / " + worker_name  + ": accumulating share " + db_job_height + " " + activeBlockTemplate.difficulty + " " + worker["time"] + " " + worker["acc"] + " (+" +  job.rewarded_difficulty + ")");
    }
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        case 'login':
            ...
            if (!miner.proxy && params.agent && params.agent.includes('proxy')) {
                let proxyMinerName = miner.payout + ":" + miner.identifier;
                if (!(proxyMinerName in proxyMiners)) {
                    proxyMiners[proxyMinerName] = {};
                    proxyMiners[proxyMinerName]["connectTime"] = Date.now();
                    proxyMiners[proxyMinerName]["count"] = 0;
...
if (cluster.isMaster) {
    ...
    if (!global.config.daemon.activePort) {
        ...
        setInterval(updateActivePort, 60*1000);
    }
