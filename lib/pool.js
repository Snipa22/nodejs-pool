    ...
    global.support.getActivePort(algo, function (newActivePort) {
        ...
        if (newActivePort === null) {
            ...
            if (algo !== "") {
                ...
                if (newActivePort != 38081 || newActivePort != 18081) return; // !!! FOR DEBUGGING PURPOSES ONLY
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_hr) {
    ...
    algos = [ "cn/1", "cn-heavy" ]; // !!! FOR DEBUGGING PURPOSES ONLY
    algos_hr = { "cn/1": 1000, "cn-heavy": 700 }; // !!! FOR DEBUGGING PURPOSES ONLY
    ...
    this.selectBestAlgo = function() {
        ...
        let best_algo = "";
        let best_algo_hr = this.algos_hr[""];
        for (let algo in ALGOS) {
            if (!(global.coinFuncs.algoTypeStr(global.config.daemon[activePortAlgo(algo)]) in this.algos)) continue;
            if (!(global.coinFuncs.algoShortTypeStr(global.config.daemon[activePortAlgo(algo)]) in this.algos)) continue;
            if (algo in this.algos_hr && this.algos_hr[algo] * global.config.daemon[algoHashFactor(algo)] > best_algo_hr) {
                best_algo = algo;
                best_algo_hr = this.algos_hr[algo] * global.config.daemon[algoHashFactor(algo)];
            }
        }
        return best_algo;
    }
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        case 'login':
            ...
            let minerId = uuidV4();
            miner = new Miner(minerId, params.login, params.pass, ip, difficulty, pushMessage, 1, portData.portType, portData.port, params.agent, params.algo, params["algo-hr"]);
            let time_now = Date.now();
...
if (cluster.isMaster) {
    ...
    if (!global.config.daemon.activePort) {
        ...
        for (let algo in ALGOS) {
            if (global.config.daemon[activePortAlgo(algo)]) {
                setInterval(updateActivePort, 5*1000, algo);
                templateUpdate(algo);
                setTimeout(templateUpdate, 50, algo, true);
            } else {
                console.warn("global.config.daemon." + activePortAlgo(algo) + " is not defined, so ignoring its algo changes");
            }
        }
    }
    ...
    if (global.config.daemon.activePortHeavy && global.config.daemon.activePortLight) {
        for (let algo in ALGOS) templateUpdate(algo);
    }
    anchorBlockUpdate();
