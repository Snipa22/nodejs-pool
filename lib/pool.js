...
function updateCoinHashFactor(coin) {
    global.support.getCoinHashFactor(coin, function (coinHashFactor) {
        if (coin === "RVN") coinHashFactor = 100; // !!!
...
function getTargetHex(diff, size, diff1) {
    const size2 = size * 2;
    const diff2 = diff1.div(diff).toBuffer({endian: 'little', size: size}).toString('hex').substr(0, size2);
...
function Miner(id, login, pass, ipAddress, startingDiff, pushMessage, protoVersion, portType, port, agent, algos, algos_perf, algo_min_time) {
    ...
    this.setNewDiff = function (difficulty) {
        console.log("!!! DEBUG DIFF: " + difficulty);
    ...
    if (protoVersion === 1) {
        this.getCoinJob = function (coin, params) {
            ...
            console.log("!!! DEBUG DIFF2: " + bt.difficulty);
		    ...
		    noncebytes: 4,
                    ...
                    difficulty: this.difficulty,
                    id:         this.id
                }; else if (global.coinFuncs.blobTypeRaven(blob_type_num)) this.cachedJob = [
	            newJob.id,
                    blob,
                    bt.seed_hash,
                    getTargetHex(this.difficulty, global.coinFuncs.nonceSize(blob_type_num), baseRavenDiff),
                    true,
                    bt.height,
                    bt.bits
                ]; else this.cachedJob = {
                    ...
                    seed_hash:  bt.seed_hash,
                    job_id:     newJob.id,
                    target:     getTargetHex(this.difficulty, global.coinFuncs.nonceSize(blob_type_num), baseDiff),
                    id:         this.id
                };
            } else {
        ...
        this.sendCoinJob = function(coin, params) {
            ...
            switch (this.protocol) {
                case "grin":  return this.pushMessage({method: "getjobtemplate", result: job});
                case "raven": return this.pushMessage({method: "mining.notify", params: job});
                default:      return this.pushMessage({method: "job", params: job});
            }
        };
...
function recordShareData(miner, job, shareDiff, isBlockCandidate, hashHex, isTrustedShare, blockTemplate) {
    ...
    if (job.difficulty >= 100000000 || isBlockCandidate) {
...
function getShareBuffer(miner, job, blockTemplate, params) {
    try {
        ...
        return global.coinFuncs.constructNewBlob(template, params.nonce, blockTemplate.port, params.mixhash);
    } catch (e) {
...
function submit_block(miner, job, blockTemplate, shareBuffer, resultHash, hashDiff, isTrustedShare, isParentBlock, isRetrySubmitBlock) {
    let reply_fn = function (rpcResult, rpcStatus) {
        if (rpcResult.error) {
            ...
            console.error(threadName + "Error submitting " + blockTemplate.coin + " (port " + blockTemplate.port + ") block at height " +
                ...
                ", error: " + JSON.stringify(rpcResult.error) + ", block hex: \n" + shareBuffer.toString('hex')
            );
            ...
            if (isNotifyAdmin) setTimeout(function() { // only alert if block height is not changed in the nearest time
                global.coinFuncs.getPortLastBlockHeader(blockTemplate.port, function(err, body) {
                    ...
                    if (blockTemplate.height == body.height + 1) global.support.sendEmail(global.config.general.adminEmail,
                        ...
                        ", isTrustedShare: " + isTrustedShare + " error (" + (typeof rpcResult.error) + "): " + JSON.stringify(rpcResult.error)
                    );
               });
            }, 2*1000);
        ...
        } else if (rpcResult && typeof rpcResult === 'string' && rpcStatus == 202 && blockTemplate.port == 11898) { // TRTL
            // Success! Submitted a block without an issue.
    ...
    if (blockTemplate.port == 11898) {
        global.support.rpcPortDaemon2(blockTemplate.port, 'block', shareBuffer.toString('hex'), reply_fn);
    } else if (global.coinFuncs.blobTypeDero(job.blob_type_num)) {
        global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [ blockTemplate.blocktemplate_blob, shareBuffer.toString('hex') ], reply_fn);
    } else {
        global.support.rpcPortDaemon(blockTemplate.port, 'submitblock', [ shareBuffer.toString('hex') ], reply_fn);
    }
}
...
function hash_buff_diff(hash, diff1) {
  return diff1.div(bignum.fromBuffer(hash, {endian: 'little', size: 32}));
}
...
function processShare(miner, job, blockTemplate, params, processShareCB) {
    ...
    // can be undefined for global.coinFuncs.blobTypeGrin(blob_type_num) or global.coinFuncs.blobTypeRaven(blob_type_num)
    // (if undefined will not be used in submit_block since isTrustedShare = false)
    ...
    let verifyShare = function(verifyShareCB) {
        if (global.coinFuncs.blobTypeGrin(blob_type_num)) {
            ...
            return verifyShareCB(hash_buff_diff(global.coinFuncs.c29_cycle_hash(params.pow, blob_type_num), baseDiff), shareBuffer, false, null);
        }

        if (global.coinFuncs.blobTypeRaven(blob_type_num)) {
            const shareBuffer = getShareBuffer(miner, job, blockTemplate, params);
            const convertedBlob = global.coinFuncs.convertRavenBlob(shareBuffer);
            if (params.header_hash !== convertedBlob) {
                report_miner_share(miner, job);
                return processShareCB(invalid_share(miner));
            }
            const hash = global.coinFuncs.slowHash(convertedBlob, blockTemplate, params.nonce, params.mixhash);
            return verifyShareCB(hash_buff_diff(hash, baseRavenDiff), shareBuffer, false, null);
        }
        ...
        const hashDiff = hash_buff_diff(resultBuffer, baseDiff);
        ...
        if ( global.config.pool.trustedMiners &&
            ...
            const isBlockDiffMatched = hashDiff.ge(blockTemplate.difficulty);
            if (isBlockDiffMatched) {
                ...
                return verifyShareCB(hashDiff, shareBuffer, false, isBlockDiffMatched);
            } else global.coinFuncs.slowHashAsync(convertedBlob, blockTemplate, function(hash) {
                if (hash === null) {
                  return processShareCB(null);
                } else if (hash === false) {
    ...
    verifyShare(function(hashDiff, shareBuffer, isTrustedShare, isBlockDiffMatched) {
        isBlockDiffMatched = isBlockDiffMatched === null ? hashDiff.ge(blockTemplate.difficulty) : isBlockDiffMatched;
    
        if (isBlockDiffMatched) { // Submit block to the RPC Daemon.
            if (!shareBuffer) {
                shareBuffer = getShareBuffer(miner, job, blockTemplate, params);
        ...
        if (is_mm && hashDiff.ge(blockTemplate.child_template.difficulty)) { // Submit child block to the RPC Daemon.
            ...
            isBlockDiffMatched = true;
        }
    
        if (isBlockDiffMatched) return processShareCB(true);
...
function handleMinerData(socket, id, method, params, ip, portData, sendReply, sendReplyFinal, pushMessage) {
    switch (method) {
	    ...
	    sendReply(null, [ null, "" ]); // empty extranonce (extra once is specificed in coinbase tx)
            break;
        }
