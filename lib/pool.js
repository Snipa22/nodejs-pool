...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent, algos, algos_hr) {
    ...
    this.selectBestAlgo = function() {
        ...
        ALGOS.forEach(function(algo) {
            if (!(global.coinFuncs.algoTypeStr(global.config.daemon[activePortAlgo(algo)]) in miner.algos)) return;
            if (!(global.coinFuncs.algoShortTypeStr(global.config.daemon[activePortAlgo(algo)]) in miner.algos)) return;
            if (algo in miner.algos_hr && miner.algos_hr[algo] * global.config.daemon[algoHashFactor(algo)] > best_algo_hr) {
                best_algo = algo;
                best_algo_hr = miner.algos_hr[algo] * global.config.daemon[algoHashFactor(algo)];
            }
        });
        return best_algo;
    }
...
if (cluster.isMaster) {
    ...
    if (global.config.daemon.activePortHeavy && global.config.daemon.activePortLight) {
        ALGOS.forEach(function(algo) { templateUpdate(algo); });
    }
    anchorBlockUpdate();
