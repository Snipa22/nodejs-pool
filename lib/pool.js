...
function get_miner_agent_warning_notification(agent) {
    let m;
    if (m = reXNP.exec(agent)) {
        let majorv = parseInt(m[1]) * 1000;
        let minorv = parseInt(m[2]) * 100;
        let minorv2 = parseInt(m[3]);
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        case 'login':
            ...
            let miner_notification = miner_agent_notification ? miner_agent_notification : get_miner_agent_warning_notification(params.agent);
