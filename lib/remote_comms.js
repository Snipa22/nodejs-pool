...
function remoteSender(task_body){
    request.post({url: global.config.general.shareHost, body: Buffer.from(task_body, 'hex'), forever: true}, function (error, response, body) {
        if (error || response.statusCode !== 200) {
            global.database.sendQueue.push({body: Buffer.from(task_body, 'hex')}, function () {
            });
        }
    });
}
...
function Database() {
    ...
    this.sendQueue = async.queue(function (task, callback) {
        pool.any.eval('global.database.remoteSender(' + task.body.toString('hex') + ')', function(){
            callback();
        });
    }, 32);
