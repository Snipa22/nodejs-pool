...
function get_cmc_price(symbol, callback) {
    const COIN = global.config.coin.name.toLowerCase();
    global.support.https_get("https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest?slug=" + COIN + "&convert=" + symbol + "&CMC_PRO_API_KEY=" + global.config.general.cmcKey, function (res) {
       return callback(
           res instanceof Object && "data" in res && COIN in res.data && "quote" in res.data[COIN] && symbol in res.data[COIN].quote
           ? parseFloat(res.data[COIN].quote[symbol].price) : 0
       );
    });
}
...
function updatePoolStats2(poolType) {
    ...
    blockList = blockList.length ? blockList.slice(max_blocks).concat(global.database.getBlockList(poolType, 0, max_blocks))
                                 : global.database.getBlockList(poolType);
    altblockList = altblockList.length ? altblockList.slice(max_altblocks).concat(global.database.getAltBlockList(poolType, null, 0, max_altblocks))
...
setInterval(get_cmc, 15*60*1000);
