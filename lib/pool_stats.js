...
function updatePoolStats2(poolType) {
    ...
    if (blockList.length) {
        ...
        for (let block of newBlocks) {
            if (block.hash == blockList[0].hash) break;
            addBlocks.unshift(block);
        }
        blockList = blockList.concat(addBlocks);
    } else {
    ...
    if (altblockList.length) {
        ...
        let addBlocks = [];
        for (let block of newBlocks) {
            if (block.hash == altblockList[0].hash) break;
            addBlocks.unshift(block);
        }
        altblockList = altblockList.concat(addBlocks);
    } else {
