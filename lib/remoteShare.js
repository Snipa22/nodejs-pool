...
function messageHandler(message) {
    if (message.type === 'newShare') {
        console.log('New share', message.body);
        shareData.push(message.body);
    }
}
        ...
        switch (msgData.msgType) {
            case global.protos.MESSAGETYPE.SHARE:
                try {
                    ...
                    process.send({ type: 'newShare', body: global.protos.Share.decode(msgData.msg) });
                } catch (e) {
                    console.error(e);
                }
            ...
            case global.protos.MESSAGETYPE.ALTBLOCK:
                ...
                global.database.storeAltBlock(msgData.exInt, msgData.msg, function (err, data) {
                    if (!data || err) {
                        console.error(err);
                        return res.status(400).end();
                    } else {
...
function storeShares() {
    debug(`storeBulkShares: ${Object.keys(shareData).length}`);
