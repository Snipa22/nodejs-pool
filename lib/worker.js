        ...
        function (height, callback) {
            ...
            async.doUntil(function (callback_until) {
                ...
                ['pplns', 'pps', 'solo', 'prop', 'global'].forEach(function (key) {
                    ...
                    if (cachedData !== false) {
                        cachedData.hash = Math.floor(localStats[key] / (hashrate_avg_min*60));
                        ...
                        cachedData = {
                            hash: Math.floor(localStats[key] / (hashrate_avg_min*60)),
                            totalHashes: 0,
                ...
                minerList.forEach(function (miner) {
                    ...
                    if (cachedData !== false) {
                        cachedData.hash = Math.floor(localStats.miners[miner] / (hashrate_avg_min*60));
                        ...
                        cachedData = {
                            hash: Math.floor(localStats.miners[miner] / (hashrate_avg_min*60)),
                            totalHashes: 0,
                ...
                globalMinerList.forEach(function (miner) {
                    ...
                    let is_stop_hashing  = false;
                    let is_start_hashing = false;
	            if (minerList.indexOf(miner) === -1) {
                        is_stop_hashing  = minerStats.hash !== 0;
                    } else {
                        is_start_hashing = cachedData !== false && minerStats.hash === 0;
                    }
                    if (is_stop_hashing || is_start_hashing) {
                        if (is_stop_hashing)  console.log("Removing: " + miner + " as an active miner from the cache.");
                        if (is_start_hashing) console.log("Adding: " + miner + " as an active miner to the cache.");
                        if (miner.indexOf('_') > -1) {
                            ...
                            global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_email IS true limit 1", [address]).then(function (rows) {
                                ...
                                global.support.sendEmail(rows[0].email,
                                    sprintf(is_stop_hashing ? global.config.email.workerNotHashingSubject : global.config.email.workerStartHashingSubject, emailData),
                                    sprintf(is_stop_hashing ? global.config.email.workerNotHashingBody    : global.config.email.workerStartHashingSubject, emailData));
                            });
                        }
                        if (is_stop_hashing) minerStats.hash = 0;
