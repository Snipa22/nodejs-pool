    ...
    async.waterfall([
        ...
        function (height, callback) {
			...
			if ( stats.hashHistory.length < global.config.general.statsBufferLength ||
                ...
                console.log("Processed " + minerCount + " workers (" + history_update_count + " history) for " + ((Date.now() - currentTime) / 1000) + " (" + ((Date.now() - db_write_start_time) / 1000) + " db write) seconds. Pool hashrate is: " + pool_hashrate);
