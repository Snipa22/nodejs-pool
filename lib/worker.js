        ...
        rows.forEach(function (row) {
            ...
            global.coinFuncs.getPortLastBlockHeader(port, function (err, block) {
                ...
                if (row.blockID < block.height - 3) {
                    global.support.sendEmail(global.config.general.adminEmail, "Pool server behind in blocks", "The pool server: "+row.hostname+" with IP: "+row.ip+" is "+(block.height - row.blockID)+ " blocks behind for " + port + " port");
                }
            });
        });
    });
