        ...
        function (height, callback) {
            ...
            async.doUntil(function (callback_until) {
	            ...
	            if (minerList.indexOf(miner) === -1) {
                        ...
                        is_start_hashing = minerStats !== false && cache_updates[miner].hash !== 0 && minerStats.hash === 0;
                    }
                        ...
                        if (miner.indexOf('_') > -1) {
                            ...
                            global.mysql.query("SELECT email FROM users WHERE username = ? AND enable_email IS true limit 1", [address]).then(function (rows) {
                                ...
                                global.support.sendEmail(rows[0].email,
                                    ...
                                    sprintf(is_stop_hashing ? global.config.email.workerNotHashingBody    : global.config.email.workerStartHashingBody, emailData),
                                    address
                                );
                            });
                        }
...
function updatePoolStats(poolType) {
    ...
    if (typeof(poolType) !== 'undefined') {
        cache = global.database.getCacheSum(poolType + "_stats");
    } else {
        cache = global.database.getCacheSum("global_stats");
    }
    async.series([
        function (callback) {
