...
function updateShareStats() {
    ...
    const history_power =
        Math.log(global.config.general.statsBufferLength) / Math.log(global.config.general.statsBufferHours * 60);
        ...
        [
            function (callback) {
                global.coinFuncs.getLastBlockHeader(function (err, body) {
                    ...
                    callback(null, body.block_header.height + 1);
                });
            },
            function (height, callback) {
                console.log(
                    'Starting stats collection for ' + height + ' height (history power: ' + history_power + ')',
                );
                ...
                async.doUntil(
                    function (callback_until) {
                        ...
                        for (
                            ...
                            found;
                            ++count, found = cursor.goToNextDup()
                        ) {
                            cursor.getCurrentBinary(function (key, share) {
                                // jshint ignore:line
                                try {
                                ...
                                let minerID =
                                    typeof share.paymentID !== 'undefined' && share.paymentID.length > 10
                                ...
                                const port =
                                    typeof share.port !== 'undefined' && share.port
                                        ? share.port
                    ...
                    function (err) {
                            ...
                            if (cachedData !== false) {
                                ...
                                if (cycleCount === 0) {
                                    ...
                                    if (cachedData.hashHistory.length > global.config.general.statsBufferLength) {
                                        while (
                                            cachedData.hashHistory.length > global.config.general.statsBufferLength
                                        ) {
                                    ...
                                    if (cachedData.minerHistory.length > global.config.general.statsBufferLength) {
                                        while (
                                            cachedData.minerHistory.length > global.config.general.statsBufferLength
                                        ) {
                        ...
                        for (let port in localPortHashes)
                            localPortHashes[port] = localPortHashes[port] / (hashrate_avg_min * 60);
                            ...
                            if (cycleCount === 0) {
                                ...
                                if (stats.hashHistory.length > global.config.general.statsBufferLength) {
                                    while (stats.hashHistory.length > global.config.general.statsBufferLength) {
                                        ...
                                        if (
                                            (currentTime - stats.hashHistory[last_index].ts) / 1000 / 3600 >
                                            global.config.general.statsBufferHours
                                        ) {
                                            ...
                                            const index_to_remove = (
                                                last_index *
                                                (1 - Math.pow(Math.random(), history_power))
                                            ).toFixed();
                        ...
                        for (let miner in prevMinerSet) {
                            ...
                            if (typeof worker !== 'undefined' && !worker.includes('silent')) {
                                ...
                                get_address_email(address, function (email) {
                                    setTimeout(
                                        delayed_send_worker_stopped_hashing_email,
                                        5 * 60 * 1000,
                                        miner,
                                        email,
                                        currentTime,
                        ...
                        try {
                            ...
                            console.error("Can't wite to pool DB: " + e);
                            global.support.sendEmail(
                                global.config.general.adminEmail,
                                'FYI: Pool DB is overflowed!',
                                "Can't wite to pool DB: " + e,
                            );
                        }
                        cache_updates = null;
                        ...
                        console.log(
                            'Processed ' +
                                ...
                                ' seconds. Pool hashrate is: ' +
                                pool_hashrate,
                        );
                        if (
                            !prev_pool_state_time ||
                            currentTime - prev_pool_state_time > hashrate_avg_min * 60 * 1000
                        ) {
                            ...
                            if (
                                pool_hashrate_ratio < 1 - stat_change_alert ||
                                pool_hashrate_ratio > 1 + stat_change_alert ||
                                ...
                                global.support.sendEmail(
                                    ...
                                    'Pool hashrate changed from ' +
                                        ...
                                        pool_hashrate_ratio +
                                        ')\n' +
                                        'Pool number of workers changed from ' +
                                        prev_pool_workers +
                                        ' to ' +
                                        pool_workers +
                                        ' (' +
...
function get_address_email(address, callback) {
    ...
    if (!(address in minerEmailTime) || currentTime - minerEmailTime[address] > 10 * 60 * 1000) {
        ...
        global.mysql
            .query('SELECT email FROM users WHERE username = ? AND enable_email IS true limit 1', [address])
            .then(function (rows) {
                if (rows.length === 0) {
