...
function updatePoolStats2(poolType) {
    ...
    async.series([
	    ...
	    let coin_comment = global.database.getCache('coin_comment');
            return callback(null, coin_comment ? coin_comment.value : {});
        },
        function (callback) {
	    ...
	    altBlocksFound: result[10] || {},
            ...
            coinProfit: result[13] || {},
            coinComment: result[14] || {},
            minBlockRewards: result[15] || {},
            pending: result[16] || 0,
            price: result[17] || {},
            currentEfforts: result[18] || {},
        });
    });
}
    ...
    global.coinFuncs.getLastBlockHeader(function(main_err, main_body){
        ...
        global.coinFuncs.getPortLastBlockHeader(global.config.daemon.activePort, function(err, body){
            ...
            if (body.hash !== lastBlockHash || main_body.height !== lastBlockHeight) {
                ...
                let info = global.database.getCache('networkBlockInfo');
		if (info === false) info = {};
                info.difficulty  = body.difficulty;
                info.hash        = body.hash;
                info.height      = body.height;
                info.main_height = main_body.height;
                info.value       = body.reward;
                info.ts          = body.timestamp;
                info[global.config.daemon.activePort] = {
