    ...
    async.waterfall([
        ...
        function (height, callback) {
			...
			const identifier = share.identifier;
                ...
                // need to clean stats_cache only after we update all current miner histories to avoid stats gaps
                const is_clean_stats_cache = cycleCount === 0 && Object.keys(miner_history_update_time).length === 0;
			...
			if ( stats.hashHistory.length < global.config.general.statsBufferLength ||
                             ...
                             (history_update_count < 5000 && currentTime - miner_history_update_time[miner] > 30*60*1000)
                           ) {
                ...
                console.log("Processed " + minerCount + " workers (" + history_update_count + " history) for " + ((Date.now() - currentTime) / 1000) + " seconds (" + ((Date.now() - db_write_start_time) / 1000) + " seconds DB write). Pool hashrate is: " + pool_hashrate);
...
setInterval(function() {
    console.log("Cleaning miner history cache");
