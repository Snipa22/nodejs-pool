...
function updateBlockHeader() {
    ...
    global.support.rpcPortDaemon(global.config.daemon.activePort, 'getlastblockheader', [], function (body) {
...
function monitorNodes() {
    global.mysql.query("SELECT blockID, hostname, ip, port FROM pools WHERE last_checkin > date_sub(now(), interval 30 minute)").then(function (rows) {
        rows.forEach(function (row) {
            global.coinFuncs.getPortLastBlockHeader(row.port, function (err, block) {
                if (err !== null){
                    console.error("Issue in getting block header for " + row.port + " port. Skipping node monitor");
                    return;
                }
                if (row.blockID < block.height - 3) {
                    global.support.sendEmail(global.config.general.adminEmail, "Pool server behind in blocks", "The pool server: "+row.hostname+" with IP: "+row.ip+" is "+(block.height - row.blockID)+ " blocks behind for " + row.port + " port");
                }
            });
        });
    });
