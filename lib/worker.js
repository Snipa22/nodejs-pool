            ...
            async.doUntil(function (callback_until) {
                ...
                for (let found = (cursor.goToRange(height) === height); found; found = cursor.goToNextDup()) {
                    cursor.getCurrentBinary(function (key, share) {  // jshint ignore:line
                        ...
                        if (minerID in localStats.miners) {
                        ...
                        if (minerIDWithIdentifier in localStats.miners) {
                ...
                if (Array.isArray(globalMinerList)){
                    globalMinerList.forEach(function(minerID){
                        globalMinerObject[minerID] = true;
                    });
                } else {
                    globalMinerObject = globalMinerList;
                }
                let cache_updates = {};
                ...
                for (let miner in localStats.miners){
                    if (localStats.miners.hasOwnProperty(miner)){
                        ...
                        local_miner_cache[miner] = cachedData.hash;
                    }
                }
                globalMinerList.forEach(function (miner) {
                    if (!(miner in localStats.miners)) {
                        let minerStats = {};
                        let loaded_from_cache = false;
                        if (!(miner in local_miner_cache)){
                            minerStats = global.database.getCache(miner);
                            local_miner_cache[miner] = minerStats.hash;
                        } else {
                            loaded_from_cache = true;
                        }
                        if (local_miner_cache[miner] !== 0) {
                            if(loaded_from_cache) {
                                // We need to get the newest data.  Oh teh noez.
                                minerStats = global.database.getCache(miner);
                            }
