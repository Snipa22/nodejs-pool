    ...
    do {
        ...
        for (let found = cursor.goToRange(height) === height; found; ++ count, found = cursor.goToNextDup()) {
            cursor.getCurrentBinary(function (key, share) {  // jshint ignore:line
                ...
                if (share.timestamp <= identifierTime) continue;
		...
		const identifier = share.identifier;
                ...
                if (share.timestamp <= locTime) continue;
                ...
                if (!shares2) continue; // use virtual shares from child block mining only for global pool stats
    ...
    for (let miner in prevMinerSet) {
        ...
        if (typeof(worker) !== 'undefined' && !worker.includes('silent')) {
            ...
            get_address_email(address, function (email) {
                setTimeout(delayed_send_worker_stopped_hashing_email, 10*60*1000, miner, email, currentTime);
            });
        }
    }
...
setInterval(function() {
    //console.log("TODO: Investigate why missive slowdown happens unless I restart this");
    //process.exit();
