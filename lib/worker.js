    ...
    async.waterfall([
        ...
        function (height, callback) {
            ...
            async.doUntil(function (callback_until) {
                ...
                if (is_use_cache) {
                    ...
                    for (let minerID in ci.identifiers) {
                        if (minerID in identifiers) {
                           for (let i in ci.identifiers[minerID]) {
                               let id = ci.identifiers[minerID][i];
                               if (identifiers[minerID].indexOf(id) < 0) {
                                   identifiers[minerID].push(id);
                                   ++ minerCount;
                               }
                           }
                        } else {
                    ...
                    for (let minerID in ci.minerSet) {
                    ...
                    for (; found; ++ count, found = cursor.goToNextDup()) cursor.getCurrentBinary(function (key, share) {  // jshint ignore:line
                        ...
                        if (is_add_new_share_cache_item) {
                            ...
                            if (minerID in ci.minerSet) {
                                ...
                                ci.minerSet[minerID] = 1;
                            }
                            if (minerIDWithIdentifier in ci.minerSet) {
                                ...
                                ci.minerSet[minerIDWithIdentifier] = 1;
                            }
                        }
                    });
                ...
                minerSet.forEach(function (miner) {
                ...
                // remove old workers
                prevMinerSet.forEach(function (miner) {
                    if (miner in minerSet) return; // we still have this miner in current set
                ...
                // find new workers
                minerSet.forEach(function (miner) {
                    if (miner in prevMinerSet) return; // we still have this miner in previous set
