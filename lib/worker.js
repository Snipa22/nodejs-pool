...
function updatePoolStats(poolType) {
    ...
    async.series([
        function (callback) {
            debug(threadName + "Checking Influx for last 5min avg for pool stats");
            return callback(null, cache.hash || 0);
        },
        function (callback) {
            debug(threadName + "Checking Influx for last 5min avg for miner count for pool stats");
        ...
        function (callback) {
            debug(threadName + "Checking LMDB for last block find time for pool stats");
            if (blockList.length === 0) {
                return callback(null, 0);
            }
            return callback(null, Math.floor(blockList[0].ts / 1000));
        },
        function (callback) {
            debug(threadName + "Checking LMDB for last block find time for pool stats");
            if (blockList.length === 0) {
        ...
        function (callback) {
            debug(threadName + "Checking LMDB for block count for pool stats");
        ...
        function (callback) {
            ...
            global.support.getActivePort(function (newActivePort) {
                return callback(null, newActivePort);
            });
        },
        function (callback) {
	    ...
	    let xmr_profit = global.database.getCache('xmr_profit_comment');
