...
function jsonRequest(host, port, data, callback, path, timeout) {
    ...
    let client = request.createClient(uri, {timeout: timeout});
    client.headers["Content-Type"] = "application/json";
    client.headers["Accept"]       = "application/json";
    if (global.config.daemon.basicAuth)    client.headers["Authorization"] = global.config.daemon.basicAuth;
    if (global.config.daemon["X-API-KEY"]) client.headers["X-API-KEY"]     = global.config.daemon["X-API-KEY"];
    ...
    if (data) {
        ...
        client.headers["Content-Length"] = data2.length;
        debug("JSON URI: " + uri + path + " Args: " + data2);
        client.post(path, data2, reply_fn);
    } else {
        debug("JSON URI: " + uri + path);
        client.get(path, reply_fn);
    }
}
