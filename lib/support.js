...
function sendEmailReal(toAddress, subject, email_body, retry) {
    ...
    const options = {
        ...
        agentOptions: {
            rejectUnauthorized: !global.config.general.mailgunNoCert,
        },
    };
    console.log(`MAIL SEND OPTIONS: ${JSON.stringify(options)}`);
    request.post(global.config.general.mailgunURL + '/messages', options, function (err, response, body) {
...
function sendEmail(toAddress, subject, body, wallet) {
    console.log(toAddress, subject, body, wallet);
    if (toAddress === global.config.general.adminEmail && !subject.includes('FYI')) {
        ...
        if (!(key in emailAcc)) {
            ...
            setTimeout(
                function (email_address, email_subject, wallet) {
...
function jsonRequest(host, port, data, callback, path, timeout) {
    ...
    let reply_fn = function (err, res, body) {
        if (err) {
            ...
            return callback(err, null, res ? res.statusCode : null);
        }
        let json;
        try {
            json = JSON.parse(body);
        } catch (e) {
            debug(`JSON parse exception: ${err}:` + body);
            return callback('JSON parse exception: ' + body);
        }

        debug('JSON result: ' + JSON.stringify(json));
        return callback(null, json, res.statusCode);
    };
    debug('JSON REQUEST: ' + JSON.stringify(options));
...
function getCoinHashFactor(coin, callback) {
    global.mysql
        .query("SELECT item_value FROM config WHERE module = 'daemon' and item = 'coinHashFactor" + coin + "'")
        .then(function (rows) {
            if (rows.length != 1) {
...
function setCoinHashFactor(coin, coinHashFactor) {
    global.mysql.query(
        "UPDATE config SET item_value = ? WHERE module = 'daemon' and item = 'coinHashFactor" + coin + "'",
        [coinHashFactor],
    );
...
function coinToDecimal(amount, coin = 'xmr') {
...
function decimalToCoin(amount, coin = 'xmr') {
    switch (coin.toLowerCase()) {
        case 'xtr':
            return Math.round(amount * 1000000.0);
        case 'xmr':
        case '':
