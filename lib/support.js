...
function jsonRequest(host, port, data, is_wallet, callback, path, timeout) {
    ...
    if (global.config.daemon.basicAuth) client.headers["Authorization"] = global.config.daemon.basicAuth;
...
function rpc_wallet(host, port, method, params, callback, timeout) {
    let data = {
        id: "0",
        jsonrpc: "2.0",
        method: method,
    ...
    return jsonRequest(host, port, data, true, callback, 'json_rpc', timeout);
}
...
module.exports = function () {
    return {
        ...
        rpcWallet: function (method, params, callback) {
            rpc_wallet(global.config.wallet.address, global.config.wallet.port, method, params, callback, 30*60*1000);
        },
        rpcPortWallet: function (port, method, params, callback) {
            rpc_wallet(global.config.wallet.address, port, method, params, callback, 30*60*1000);
        },
        rpcPortWalletShort: function (port, method, params, callback) {
            rpc_wallet(global.config.wallet.address, port, method, params, callback, 10*1000);
        },
