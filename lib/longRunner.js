	...
	console.log("Cleaning up the block balance table");
        ...
        global.mysql.query("SELECT DISTINCT hex FROM block_balance").then(function (rows) {
            rows.forEach(function (row) {
                if (row.hash in locked_block_hashes) {
                    console.log("Block hash is currently locked: " + row.hash); return;
                }
                if (row.hash in saw_block_hash_before) {
                    //global.mysql.query("DELETE FROM block_balance WHERE hex = ?", [row.hash]).then(function () {});
                    delete saw_block_hash_before[row.hash];
                    ++ deleted_row_count;
                } else {
                   saw_block_hash_before[row.hash] = 1;
                }
            });
        });
