    ...
    for (let found = cursor.goToFirst(); found; found = cursor.goToNext()) {
        cursor.getCurrentString(function (key, data) {
            ...
            if (key.includes('identifiers:')) {
                // remove frozen worker names after 24h
                let parts = key.split(/:(.+)/);
                let key2 = parts[1];
            ...
            } else if (key.includes('_') && !key.includes('history:') && !key.includes('stats:')) {
                // remove week old workers
            ...
            } else if (!key.includes('_') && key.includes('stats:')) {
                // zero frozen account hashrate after 24h
                try {
                    let data2 = JSON.parse(data);
...
let cleanBlockBalanceTableQueue = async.queue(function (task, callback) {
    global.mysql.query('DELETE FROM block_balance WHERE hex = ?', [task.hex]).then(function () {
...
function cleanBlockBalanceTable() {
    ...
    global.database.getValidLockedBlocks().forEach(function (block) {
        locked_block_hashes[block.hash] = 1;
    });
    global.database.getValidLockedAltBlocks().forEach(function (block) {
        locked_block_hashes[block.hash] = 1;
    });
...
async function cleanPendingPayouts() {
    try {
        const result = await db.pendingPayouts.deleteOldEntries();
        console.log(`Successfully deleted old pending payout entries ${JSON.stringify(result)}`);
    } catch (e) {
        console.error(e);
    }
}
...
setInterval(cleanPendingPayouts, 24 * 60 * 60 * 1000);
