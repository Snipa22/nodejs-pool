    ...
    for (let found = cursor.goToFirst(); found; found = cursor.goToNext()) {
        cursor.getCurrentBinary(function (key, data) {
            ...
            if (blockData.hash === hash) {
                is_found = true;
                global.coinFuncs.getBlockHeaderByHash(hash, (err, body) => {
                    if (err !== null) {
                        console.log(
                            'Altblock with ' +
                                hash +
                                ' hash still has invalid hash for ' +
                                blockData.port +
                                ' port! Exiting!',
                        );
                        cursor.close();
                    ...
                    if (blockData.value !== body.reward) {
                        console.log('Changing alt-block value from ' + blockData.value + ' to ' + body.reward);
                    }
