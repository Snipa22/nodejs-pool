...
require('../init_mini.js').init(function () {
    ...
    const old_where_str =
        old_payment_id === null
            ? "payment_address = '" + old_address + "' AND payment_id IS NULL"
            : "payment_address = '" + old_address + "' AND payment_id = '" + old_payment_id + "'";

    const new_where_str =
        new_payment_id === null
            ? "payment_address = '" + new_address + "' AND payment_id IS NULL"
    ...
    async.waterfall([
        function (callback) {
            global.mysql.query('SELECT * FROM balance WHERE ' + old_where_str).then(function (rows) {
                ...
                const row = rows[0];
                console.log('Old address amount: ' + global.support.coinToDecimal(row.amount, row.coin));
                console.log('Old address last update time: ' + row.last_edited);
                callback();
            });
        },
        function (callback) {
            global.mysql.query('SELECT * FROM balance WHERE ' + new_where_str).then(function (rows) {
                ...
                const row = rows[0];
                console.log('New address amount: ' + global.support.coinToDecimal(row.amount, row.coin));
                callback();
            });
        },
        function (callback) {
        ...
        function (callback) {
            global.mysql
                .query('UPDATE balance SET amount = amount + ' + old_amount + ' WHERE ' + new_where_str)
                .then(function (rows) {
