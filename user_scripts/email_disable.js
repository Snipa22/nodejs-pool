...
require("../init_mini.js").init(function() {
	async.waterfall([
		...
		function (callback) {
			global.mysql.query("UPDATE users SET enable_email = '0' WHERE username = ?", [user]).then(function (rows) {
				console.log("UPDATE users SET enable_email = '0' WHERE username = " + user);
				callback();
			});
		},
		function (callback) {
